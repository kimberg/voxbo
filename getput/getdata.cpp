...
#define MSG(AAA)                                       \
  {                                                    \
    fprintf(stderr, "Getdata fatal error: %s\n", AAA); \
    fflush(stderr);                                    \
  }
...
#define COMPLAIN(AAA)                     \
  {                                       \
    MSG(errstr[AAA]);                     \
    if (return_fifo) {                    \
      fprintf(return_fifo, "-%d\n", AAA); \
      fclose(return_fifo);                \
    } else {                              \
      if (!return_fifo_opened) {          \
...
#define DEBUG_MESSAGE(AAA)                       \
  {                                              \
    fprintf(stderr, "Getdata debug: %s\n", AAA); \
    fflush(stderr);                              \
  }
#else
#define DEBUG_MESSAGE(AAA) \
...
void deconstruct_voxel_position(int pos, int &x, int &y, int &z, int dimx,
                                int dimy, int dimz) {
  x = pos % dimx;
  pos /= dimx;
  y = pos % dimy;
...
void send_userhdr(FILE *f, string s) { fprintf(f, "%%%s\n", s.c_str()); }
...
int send_4d(char *path_, char *fifopath, short op) {
  ...
  int i;
  for (i = 0; i < total; i++) send_userhdr(f, t->header[i]);
  ...
  if (!t->mask)
    ...
    for (z = 0; z < dimz; z++) {
      for (y = 0; y < dimy; y++) {
        for (x = 0; x < dimx; x++) {
          if (t->GetMaskValue(x, y, z)) n_voxels++;
        }
      }
    }
  }
        ...
        for (x = 0; x < dimx; x++) {
          if (t->GetMaskValue(x, y, z)) {
            ...
            for (t2 = 0; t2 < dimt; t2++) {
              ...
              long ix =
                  t2 + x * dimt + y * dimt * dimx + z * dimt * dimx * dimy;
...
bool send_3d(char *path_, char *fifopath, short op) {
  ...
  if (do_data) {
    err = c->ReadFile(path);
    if (err) DEBUG_MESSAGE("Child's attempt to read file failed.")
  } else {
    err = c->ReadHeader(path);
    if (err) DEBUG_MESSAGE("Child's attempt to read header failed.")
  }
  ...
  int i;
  for (i = 0; i < total; i++) send_userhdr(f, c->header[i]);
...
int main(int argc, char **argv) {
  ...
  if (op < 0) COMPLAIN(BADOP)
  if (len < 1) COMPLAIN(BADPATH)
  ...
  if (op & OP_TSRA)
    ...
    if (total_potentials) {
      ...
      fprintf(stderr, "Getdata: VBFFF says type= %s, dims= %d\n",
              potentials[0].name.c_str(), potentials[0].dimensions);
      fflush(stderr);
#endif
  ...
  if (!(child = fork())) {
    ...
    if (!op) {
      ...
      string name = "UNKN";
      if (total_potentials) name = potentials[0].getName();

      char *s = (char *)name.c_str();
      if (s && !strncmp(s, "VoxBo ", 6)) s += 6;
      ...
      switch (type) {
        case TYPE_3D:
          ...
          break;
        case TYPE_TSRA:
          if ((err = send_timeseries(fifopath, op))) COMPLAIN(err)
          break;
        case TYPE_4D:
          if ((err = send_4d(path, fifopath, op))) COMPLAIN(err)
          break;
        default:
          COMPLAIN(NOSUPPORT)
      }
