...
#define MSG(AAA)                                       \
  {                                                    \
    fprintf(stderr, "Putdata fatal error: %s\n", AAA); \
    fflush(stderr);                                    \
  }

#define WARNING(AAA)                               \
  {                                                \
    fprintf(stderr, "Putdata warning: %s\n", AAA); \
    fflush(stderr);                                \
  }
...
int readline(FILE *f, char *buf, int lim) {
  ...
  buf[i] = 0;
  if (!i && ch == EOF) return -1;
  return i;
}

int receive_3d(char *fifo, char *path, int dimx, int dimy, int dimz,
               int datatype) {
  ...
  // Check params
  //
  if (dimx <= 0 || dimy <= 0 || dimz <= 0) complain(ERROR_BADDIMS);
  if (datatype < 0) complain(ERROR_BADTYPE);
  ...
  if (0 > readline(f, line, 199)) complain(ERROR_EOF);
  int total_header_items = atoi(line);
  if (total_header_items < 0) complain(ERROR_BADHEADER);
  ...
  for (i = 0; i < total_header_items; i++) {
    ...
    if (s2) {
      *s2++ = 0;
      if (!strcasecmp(s, "VoxSizes(xyz)")) {
      ...
      } else if (!strcasecmp(s, "Origin(xyz)")) {
        int x, y, z;
        ...
        c->orient = s2;
      } else if (!strcasecmp(s, "Scale")) {
        // float f = atof(s2);
        // c->scalefactor = f ? f : 1.0;
      } else if (!strcasecmp(s, "ByteOrder")) {
  ...
  if (!chunk) complain(ERROR_NOMEM);
  if (chunk_size != fread(chunk, 1, chunk_size, f)) complain(ERROR_READFAIL);
...
int receive_4d(char *fifo, char *path, int dimx, int dimy, int dimz, int dimt,
               int datatype) {
  char fifo_from_idl[PATH_MAX];
  char fifo_to_idl[PATH_MAX];
  char line[200];
  ...
  // Check params
  //
  if (dimx <= 0 || dimy <= 0 || dimz <= 0 || dimt <= 0) complain(ERROR_BADDIMS);
  if (datatype < 0) complain(ERROR_BADTYPE);
...
int main(int argc, char **argv) {
  ...
  if (argc != 10) complain(NOARG);

  return_path = fifopath = argv[2];

  char *path = argv[1];
  char len = strlen(path);
  if (len < 4) complain(BADPATH);
