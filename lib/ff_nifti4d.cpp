...
vf_status
test_n14d_4D(unsigned char *,int,string filename)
{
  string xt=xgetextension(filename);
  if (xt!="nii" && xt!= "img" && xt != "hdr" && xt != "gz")
    return vf_no;
  NIFTI_header nn;
...
int
read_data_n14d_4D(Tes *ts,int,int)  // start and count arguments removed
{
  if (!ts->header_valid) {
    if (nifti_read_header(ts->GetFileName(),NULL,ts))
      return 101;
  }
...
int
read_ts_n14d(Tes &ts,int x,int y,int z)
{
  if (!ts.header_valid) {
    if (nifti_read_header(ts.GetFileName(),NULL,&ts))
      return 101;
...
int
read_vol_n14d(Tes &ts,Cube &cb,int t)
{
  if (!ts.header_valid) {
    if (nifti_read_header(ts.GetFileName(),NULL,&ts))
      return 101;
