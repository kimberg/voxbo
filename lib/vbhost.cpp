...
int VBHost::ReadFile(const string &fname) {
  init();
  char line[STRINGLEN];
  ...
  for (i = 0; i < 168; i++) pri[i] = ncpus[i] = 0;
  while (fgets(line, STRINGLEN, infile) != NULL) {
    if (args.ParseLine(line) < 1) continue;
    ...
    if (args[0] == "hostname")  // full hostname
      ...
      rank = strtol(args[1]);
    } else if (args[0] == "resource" || args[0] == "globalresource" ||
               args[0] == "provides") {
      if (args.size() == 3) {
        VBResource vbr;
        vbr.name = args[1];
...
void VBHost::setnames(const string nn, const string hn) {
  nickname = nn;
  hostname = hn;
}

void VBHost::initaddress(uint16 serverport) {
  memset(&addr, 0, sizeof(struct sockaddr_in));
  addr.sin_family = AF_INET;
  ...
  if (hp)
    if (memcpy(&addr.sin_addr, hp->h_addr_list[0], hp->h_length)) valid = 1;
}

string VBHost::tobuffer(map<jobid, VBJobSpec> &runningmap) {
  stringstream ss;
  ...
  for (int j = 0; j < (int)reservations.size(); j++) {
    ss << "[reservation " << reservations[j].owner << " "
       << reservations[j].start << " " << reservations[j].end << " "
       << reservations[j].reason << "]";
  }
  for (RI rr = resources.begin(); rr != resources.end(); rr++)
    ss << "[resource '" << rr->second.name << "' '" << rr->second.f_global
       << "' " << rr->second.cnt << "]";

  for (map<jobid, VBJobSpec>::iterator j = runningmap.begin();
       j != runningmap.end(); j++) {
    if (j->second.hostname != nickname) continue;
    char info[STRINGLEN];
    sprintf(info, "[job %d %d %d %d %ld \"%s\"]", j->second.snum,
            j->second.jnum, j->second.actualcpus, j->second.percentdone,
...
int VBHost::frombuffer(string buf) {
  tokenlist args, argx;
  args.SetQuoteChars("[<(\"'");
  argx.SetQuoteChars("[<(\"'");
  args.ParseLine(buf);
...
float VBHost::LoadAverage() const { return loadaverage; }

void VBHost::SetLoadAverage(float load) { loadaverage = load; }

short VBHost::Priority() const { return currentpri; }
...
void VBHost::DeadOrAlive() {}

void VBHost::CheckSchedule() {
  time_t now;
  struct tm *nowtm;
  int ind;
...
void VBHost::Ping(map<jobid, VBJobSpec> &runningmap) {
  ...
  for (map<jobid, VBJobSpec>::iterator j = runningmap.begin();
    ...
    pinglist += (string) " " + strnum(j->second.snum) + " " +
                strnum(j->second.jnum) + " " + strnum(j->second.pid);
  }
...
int VBHost::SendMsg(string msg) {
  ...
  args.ParseLine(info);
  if (args[0] != "ACK") printf("[E] bad acknowledgment for msg: %s\n", args(0));
  close(s);
  return 0;
}
...
bool cmp_host_pri_taken(const VBHost &h1, const VBHost &h2) {
  ...
  if (h1.rank > h2.rank) return 0;
  // same rank, prioritize hosts that are less busy
  if (h1.taken_cpus < h2.taken_cpus) return 1;
  if (h1.taken_cpus > h2.taken_cpus) return 0;
  // beyond that it's random
  if (h1.rand < h2.rand) return 1;
  return 0;
}
...
void VBHost::print() {
  ...
  for (RI rr = resources.begin(); rr != resources.end(); rr++)
    printf("  + resource %s %d\n", rr->second.name.c_str(), rr->second.cnt);
}

void VBHost::updateresources() {
  // FIXME
