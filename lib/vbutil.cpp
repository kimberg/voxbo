...
string
xdirname(const string &str,int times)
{
  string res(str);
...
string
xsetextension(const string &str,const string newextension,bool multiflag)
{
  size_t pos,slashpos;
  if (!multiflag) {
    slashpos=str.rfind("/");
...
xgetcwd()
{
  char buf[PATH_MAX*2];
  buf[PATH_MAX*2-1]=0;
  if (getcwd(buf,PATH_MAX*2-1))
    return (string)buf;
  return ".";
}

string
xabsolutepath(string path)
{
...
string
xgetextension(const string &str,bool multiflag)
{
  size_t pos,slashpos;
  if (!multiflag) {
    slashpos=str.rfind("/");
...
bool
wildcard_compare(char *pTameText,char *pWildText,bool bCaseSensitive=FALSE,char cAltTerminator='\0')
{
  bool bMatch = TRUE;
...
string
textnumberlist(const vector<int32>nums)
{
  string ret;
  if (!(nums.size()))
    return "";
...
string
textnumberset(const set<int32>nums)
{
  string ret;
  if (!(nums.size()))
    return "";
...
void
stripchars(string &str,const char *chars)
{
  string::size_type first;
  first=str.find_first_of(chars);
  if (first != string::npos)
    str=str.substr(0,first);
}
...
struct timeval
operator+(struct timeval tv1,int usec)
{
  timeval ret;
...
int
morethan(const void *first,const void *second)
{
...
void
dblock::init(uint8 *indata,uint32 insize)
{
  if (data) delete [] data;
  data=new uint8[size];
  size=insize;
...
dblock::dblock(const dblock &src)
{
  *this=src;
}
...
{
  if (t1.v1<t2.v1) return 1;
  else if (t1.v1>t2.v1) return 0;
  if (t1.v2<t2.v2) return 1;
  return 0;
}

void
printErrorMsg(const VB_ERROR_LEVEL level,string theMsg)
{
  string msg;
  switch(level) {
  case VB_INFO:
...
void
printErrorMsgAndExit(const VB_ERROR_LEVEL level,string theMsg, const unsigned short exitValue)
{
		   ...
		   const unsigned int lineNo, const char *func, const char *file)
{
  ...
  theMsg2 << "LINE [" << lineNo << "] FUNCTION [" << func << "] FILE [" << file << "] " << theMsg;
...
int
interleavedorder(int ind,int total)
{
  if (total & 1) {     // ODD
    if (ind & 1)
...
int
arghandler::parseArgs(int argc, char **argv)
{
  errmsg="";
  for (int i=1; i<argc; i++) {
    if (argv[i][0]!='-') {
    ...
    for (size_t j=0; j<miniargs.size(); j++) {
      if (miniargs[j].flag==argv[i] || miniargs[j].flag2==argv[i]) {
        if ((i+miniargs[j].argcnt)<argc) {
          ...
          errmsg="bad argument structure -- not enough args for " + (string)argv[i];
        }
      }
    }
    if (!foundit)
      errmsg="bad argument structure -- flag " + (string)argv[i];
  }
  return 0;
}

tokenlist
arghandler::getFlaggedArgs(string flag)
{
  for (size_t i=0; i<miniargs.size(); i++) {
    if (miniargs[i].flag==flag)
      return miniargs[i].args;
  }
  tokenlist tmp;
  return tmp;
}
...
tokenlist
arghandler::getUnflaggedArgs()
{
  return unflagged;
}

void
arghandler::setArgs(string flag, string flag1, int size)
{
  miniarg newArg;
  newArg.clear();
  newArg.flag=flag;
...
int
arghandler::getSize(string flag)
{
  for (size_t i = 0; i < miniargs.size(); i++)
...
int
arghandler::flagPresent(string flag)
{
  for (size_t i=0; i<miniargs.size(); i++)
    if (miniargs[i].flag==flag || miniargs[i].flag2==flag)
      if (miniargs[i].present)
          return 1;
  return 0;
}
...
void
bitmask::operator=(const bitmask &old)
{
  if (mask) free(mask);
  bytes=old.bytes;
  sz=old.sz;
...
operator==(const bitmask &a,const bitmask &b)
{
  ...
  if (a.mask==NULL || b.mask==NULL)
    return 0;
  if (memcmp(a.mask,b.mask,a.bytes)==0)
    return 1;
  return 0;
}
...
bool
zfile::open(string fname,const char *mode,int8 compressed)
{
  filename=fname;
  if (compressed==-1) {
    if (xgetextension(fname)=="gz") compressed=1;
    else compressed=0;
  }
  if (compressed) {
...
size_t
zfile::write(const void *ptr,size_t bytes)
{
  if (zflag)
...
int
zfile::seek(off_t offset,int whence)
{
  if (zflag)
...
zfile::operator bool() const
{
  return ((zflag && zfp)||(!zflag && fp));
}
...
vglob::vglob(const string pat,uint32 flags)
{
  load(pat,flags);
}

void
vglob::load(const string pat,uint32 flags)
{
  clear();
  append(pat,flags);
}
...
void
vglob::append(const string pat,uint32 flags)
{
  glob_t gb;
  ...
  for (size_t i=0; i<gb.gl_pathc; i++) {
    if (flags) {
      ...
      if (err) continue;  // if we can't stat it, it doesn't definitely satisfy the flags
...
{
  tokenlist tt;
  vbforeach(string s,names)
    tt.Add(s);
  return tt;
}

string
vglob::operator[](size_t ind)
{
  return names[ind];
}
...
bool
operator==(const jobid &j1,const jobid &j2)
{
  if (j1.snum!=j2.snum || j1.jnum!=j2.jnum)
    return 0;
  return 1;
}
...
operator<(const jobid &j1,const jobid &j2)
{
  if (j1.snum<j2.snum || (j1.snum==j2.snum && j1.jnum<j2.jnum))
    return 1;
  return 0;
}
...
vbrect
vbrect::operator&(const vbrect &rr)
{
  int32 l=x;
  int32 r=x+w-1;
  int32 t=y;
