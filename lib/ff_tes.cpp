...
vf_status tes1_test(unsigned char *buf, int bufsize, string) {
  tokenlist args;
  args.SetSeparator("\n");
  if (bufsize < 40) return vf_no;
  args.ParseLine((char *)buf);
  if (args[0] != "VB98" || args[1] != "TES1") return vf_no;
  return vf_yes;
}

int tes1_read_ts(Tes &ts, int x, int y, int z) {
  gzFile fp;
  string keyword;
  tokenlist args;
  int cnt;
  ...
  if (!(ts.GetMaskValue(x, y, z))) {
    ...
    for (int i = 0; i < ts.dimt; i++) ts.timeseries.setElement(i, 0.0);
    return (0);  // no error!
  }
  ...
  gzclose(fp);
  if (cnt != ts.dimt * ts.datasize) return 101;

  if (my_endian() != ts.filebyteorder) swapn(tmpdata, ts.datasize, ts.dimt);
  ts.timeseries.resize(ts.dimt);
...
int tes1_read_vol(Tes &ts, Cube &cb, int t) {
  ...
  // includes the mask
  if (!ts.header_valid) return (100);
  if (t < 0 || t > ts.dimt - 1) return 101;
  ...
  gzclose(fp);
  if (my_endian() != ts.filebyteorder) cb.byteswap();
  if (ts.f_scaled) {
...
int tes1_write(Tes *mytes) {
  ...
  hdr += buf;
  if ((mytes->origin[0] + mytes->origin[1] + mytes->origin[2]) >
      0) {  // FIXME could be bad
    buf = (format("Origin(XYZ): %d %d %d\n") % mytes->origin[0] %
           mytes->origin[1] % mytes->origin[2])
              .str();
    hdr += buf;
  }
  // force big-endian
  ...
  if (mytes->f_scaled) {
    ...
    *mytes /= mytes->scl_slope;
    if (mytes->altdatatype == vb_byte || mytes->altdatatype == vb_short ||
        mytes->altdatatype == vb_long)
      mytes->convert_type(mytes->altdatatype);
  }
  if (my_endian() != mytes->filebyteorder) mytes->byteswap();
  int sz, cnt;
  for (int i = 0; i < mytes->dimx * mytes->dimy * mytes->dimz; i++) {
    if (mytes->mask[i] == 0) continue;
  ...
  if (mytes->f_scaled) {
    if (mytes->datatype == vb_byte || mytes->datatype == vb_short ||
        mytes->datatype == vb_long)
...
int tes1_read_head(Tes *mytes) {
  ...
  if (mytes->dimt == 0 || mytes->dimx == 0 || mytes->dimy == 0 ||
      mytes->dimz == 0) {
    gzclose(fp);
    return (100);
  }
  ...
  if ((fabs(mytes->scl_slope) > FLT_MIN &&
       fabs(mytes->scl_slope - 1.0) > FLT_MIN) ||
      (fabs(mytes->scl_slope - 1.0) < FLT_MIN &&
       fabs(mytes->scl_inter) > FLT_MIN)) {
    mytes->f_scaled = 1;
  ...
  mytes->SetVolume(mytes->dimx, mytes->dimy, mytes->dimz, mytes->dimt,
                   mytes->datatype);
  // read the mask
  if (mytes->InitMask(0)) {
    gzclose(fp);
...
int tes1_read_data(Tes *mytes, int start, int count) {
  gzFile fp;
  string keyword;
  tokenlist args;
  int cnt;

  if (!mytes->header_valid) return 101;
  if (mytes->InitData()) return 102;
  ...
  for (int i = 0; i < mytes->dimx * mytes->dimy * mytes->dimz; i++) {
    ...
    if (start > 0) gzseek(fp, start * mytes->datasize, SEEK_CUR);
    ...
    if (endskip > 0) gzseek(fp, endskip * mytes->datasize, SEEK_CUR);
  }
  gzclose(fp);
  if (my_endian() != mytes->filebyteorder) mytes->byteswap();
  if (mytes->f_scaled) {
