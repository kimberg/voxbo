...
bool recursive_contains(container& ct, const shared_ptr<Exec>& ep) {
  ...
  for (typename container::const_iterator iter = ct.begin(); iter != ct.end();
       ++iter) {
    const EP& sub_ep = *iter;

    if (sub_ep == ep || sub_ep->contains(ep)) return true;
  }
  return false;
}
...
bool Job::contains(const shared_ptr<Exec>&) const { return false; }
...
const shared_ptr<Block>& Sequence::root_block() const { return m_root_block; }
...
void Sequence::root_block_def(BlockDef* root_bdp) {
  if (m_root_block) m_root_block.reset();

  if (root_bdp) m_root_block = static_pointer_cast<Block>(root_bdp->declare());
}

const map<string, string>& Sequence::vars() const { return m_vars; }

map<string, string>& Sequence::vars() { return m_vars; }
...
void Sequence::name(const string& n) { m_vars[SEQUENCE_NAME_STRING] = n; }
...
void Sequence::priority(const string& p) { m_vars["PRIORITY"] = p; }
...
bool Sequence::execs_are_sorted() const {
  ...
  vbforeach(ExecPointer exec, execs) {
    ...
    vbforeach(ExecPointer wait_exec, wait_execs) {
      if (seen_execs.find(wait_exec) == seen_execs.end()) return false;
    }
...
list<shared_ptr<Exec> >& Sequence::sort_execs() {
  ...
  if (!execs_are_sorted()) {
    ...
    // Let multimap do the work of semi-efficiently sorting the table according
    // to the Job depths.
    multimap<int, shared_ptr<Exec> > sorted_depth_table;

    typedef pair<shared_ptr<Exec>, int> ExecIntPair;
    vbforeach(ExecIntPair table_pair, depths) sorted_depth_table.insert(
        pair<int, shared_ptr<Exec> >(table_pair.second, table_pair.first));

    // Now, I don't actually care what the depths are; I just needed them to
    // sort the Jobs topologically.  So now let's get rid of the depths and just
...
void Sequence::build_depth_table(map<shared_ptr<Exec>, int>* table) {
  ...
  for (list<ExecPointer>::iterator iter = execs.begin(); iter != execs.end();
       ++iter) {
...
void Sequence::build_depth_table_helper(shared_ptr<Exec> exec,
                                        map<shared_ptr<Exec>, int>* table) {
  typedef shared_ptr<Exec> ExecPointer;

  // We don't need to build what we've already built, so check whether we've
  // done this Job.  Get out if we have.
  if (table->find(exec) != table->end()) return;
  ...
  if (!deps.empty()) {
    ...
    for (list<ExecPointer>::iterator iter = deps.begin(); iter != deps.end();
         ++iter) {
    ...
    for (list<ExecPointer>::iterator iter = deps.begin(); iter != deps.end();
         ++iter) {
      ExecPointer& wait_exec = *iter;
      if (depth <= (*table)[wait_exec]) depth = (*table)[wait_exec] + 1;
    }
  }
  ...
  while (in.good()) {
    ...
    if (token == "set") {
      ...
      if (eqpos == string::npos) {
        DEBUG_OUT(
            "  error: Line with 'set' directive but no '=' sign.  Skipping.\n");
        continue;
      }
...
shared_ptr<Exec> VB::read_inst_line(istream& in_line, ExecDef* edp,
  ...
  while (in_line.good()) {
    ...
    // Exec argument setting
      ...
      if (eqpos == string::npos) {
        DEBUG_OUT("  error: Unrecognized token -- ");
        DEBUG_OUT(token);
        DEBUG_OUT(".  Ignoring.\n");
        continue;
      }
  ...
  if (!does_not_wait && ep->depends.empty() && prev_ep) {
...
void build_final_jobs_map(const shared_ptr<VB::Exec>& ep) {
  DEBUG_OUT("  Processing final_jobs for exec inst ");
  DEBUG_OUT(ep->name + "\n");
  ...
  if (bp) {
    ...
    for (list<EP>::iterator iter = bp->execs.begin(); iter != bp->execs.end();
         ++iter) {
      EP& child_ep = *iter;
      ...
      if (child_ep->depends.empty()) {
        ...
        for (list<EP>::iterator iter = child_ep->depends.begin();
          ...
          EP& wait_ep = *iter;
          DEBUG_OUT("    this executable depends on ");
          DEBUG_OUT(wait_ep->name + ".\n");

          set<JP>& wait_fj = final_jobs[wait_ep];
          for (set<JP>::iterator iter = wait_fj.begin(); iter != wait_fj.end();
               ++iter) {
            JP wait_jp = *iter;
            fj.erase(wait_jp);
          }
  ...
  vbforeach(JP fj, final_jobs[ep]) DEBUG_OUT(fj->name + " ");
  DEBUG_OUT("\n");
}
 ...
 * dataset node id.
 */
int VB::calc_job_num(const shared_ptr<VB::Job>& jp,
                     const VB::DataSet* const dsp) {
  static map<int, int> job_num_map;

  pair<map<int, int>::iterator, bool> insert_value = job_num_map.insert(
      pair<int, int>(jp->id * 1000 + dsp->get_id(), max_jnum + 1));
...
BP common_ancestor(const JP& jp1, const JP& jp2) {
  ...
  while (parent2) {
    if (ancestors1.find(parent2) != ancestors1.end()) return parent2;
...
set<DataSet*>& get_all_descendant_nodes(DataSet* dsp) {
  ...
  if (iter == node_map.end()) {
    iter = node_map.insert(pair<DataSet*, set<DataSet*> >(dsp, set<DataSet*>()))
               .first;
...
int fill_file_vars(string& str, const string& infilename,
  ...
  if ((pos = str.find(setext_string)) != string::npos) {
    ...
    if (paren_pos != string::npos)
      ...
      DEBUG_OUT("no closing paren for ");
    DEBUG_OUT(setext_string + "\n");
    ...
    if (comma_pos != string::npos && comma_pos != rest.length() - 1)
      ...
      DEBUG_OUT("no new extension specified for ");
    DEBUG_OUT(setext_string + "\n");
    ...
    str.replace(pos, str.length() - pos - 1,
                xsetextension(oldfilename, newext));
    ++value;
  }
 ...
 * job).
 */
vector<VBJobSpec> create_single_jobspec(const shared_ptr<VB::Job>& jp,
                                        VB::DataSet* dsp) {
  ...
  vbforeach(JobType::File f, jtp->files()) {
    ...
    for (list<FileSnapShot>::reverse_iterator iter =
             global_file_info_map[f.id].rbegin();
    ...
    if (informat != "" && informat != prevformat) {
      conversion_jp = static_pointer_cast<VB::Job>(
          VB::Definitions::Get("x_vbconv")->declare());
      ...
      infilename =
          prevfilename + "." + my_itoa(calc_job_num(jp, dsp)) + "." + informat;
  ...
  vbforeach(JobType::Argument arg, jtp->args()) {
    ...
    if (variables.find(arg.name) != variables.end()) {
      ...
      DEBUG_OUT("  error: argument ");
      DEBUG_OUT(arg.name);
      DEBUG_OUT(" has no value.\n");
    }
  }
  ...
  if (jp->depends.empty()) {
    ...
    while (parent_bp) {
      jp->depends.insert(jp->depends.end(), parent_bp->depends.begin(),
                         parent_bp->depends.end());
      if (!jp->depends.empty()) break;
  ...
  for (list<EP>::iterator iter = jp->depends.begin(); iter != jp->depends.end();
       ++iter) {
    EP& wait_ep = *iter;
    ...
    for (set<shared_ptr<Job> >::const_iterator iter =
             final_jobs[wait_ep].begin();
      ...
      if (iterated_nodes.find(final_jp) == iterated_nodes.end()) {
        ...
        list<DataSet*> final_iterated_nodes =
            dsp->get_root()->get_children(wait_path);
        iterated_nodes[final_jp].insert(final_iterated_nodes.begin(),
                                        final_iterated_nodes.end());
      }

      DEBUG_OUT("    The ID of this job is ");
      DEBUG_OUT(calc_job_num(jp, dsp));
      DEBUG_OUT("\n");
      DEBUG_OUT("    The job waits for jobs that iterate over: DataSet");
      DEBUG_OUT(wait_path + "\n");
      ...
      if (common_ancestor_block)
        ancestor_path = common_ancestor_block->full_path();
      else
        DEBUG_OUT(
            "error: jobs do not have a common ancestor; must not be from the "
            "same sequence.\n");
      string ds_path = dsp->get_long_name();
      if (ds_path.substr(0, 7) == "DataSet") ds_path = ds_path.substr(7);
      list<string> ancestor_path_list = split(ancestor_path, ":");
      list<string> ds_path_list = split(ds_path, ":");

      DEBUG_OUT("    The path of the current dataset node is DataSet");
      DEBUG_OUT(ds_path + "\n");
      DEBUG_OUT(
          "    The path of the nearest common ancestor's node is DataSet");
      DEBUG_OUT(ancestor_path + "\n");

      DataSet* specific_ancestor_node = dsp->get_root();
      DEBUG_OUT("    The root node of this dataset is ");
      DEBUG_OUT(dsp->get_root()->get_long_name() + "\n");
      while (!ds_path_list.empty() && !ancestor_path_list.empty()) {
        ...
        if (!matches(ds_node_name, ancestor_node_name)) break;

        DEBUG_OUT("      Getting node child ");
        DEBUG_OUT(ds_node_name + "\n");
        specific_ancestor_node =
            specific_ancestor_node->get_child(ds_node_name);

        ds_path_list.pop_front();
        ancestor_path_list.pop_front();
      }

      DEBUG_OUT("    The specific path of the ancestor node is ");
      DEBUG_OUT(specific_ancestor_node->get_long_name() + "\n");

      set<DataSet*>& all_descendants =
          get_all_descendant_nodes(specific_ancestor_node);
      set<DataSet*>& all_wait_iterated_nodes = iterated_nodes[final_jp];
      set<DataSet*> wait_children;

      // ::NOTE:: Not sure if this should be the case, but if the two
      // executables
      ...
      if (final_jp->parent.lock() == jp->parent.lock() &&
        ...
        set_intersection(all_descendants.begin(), all_descendants.end(),
                         all_wait_iterated_nodes.begin(),
                         all_wait_iterated_nodes.end(),
      ...
      for (set<DataSet*>::iterator iter = wait_children.begin();
        ...
        DEBUG_OUT("      This job waits for ");
        DEBUG_OUT(calc_job_num(final_jp, wait_dsp));
        DEBUG_OUT("\n");
      }
      DEBUG_OUT("\n");
    }
 ...
 */
vector<VBJobSpec> create_jobspecs(const shared_ptr<VB::Job>& jp,
                                  VB::DataSet* dsp) {
  ...
  /*  if (datasets.empty())
    ...
    DEBUG_OUT(jp->full_path());DEBUG_OUT(")\n"); dsp->spit_tree_to_stdout();
    }
  ...
  vbforeach(VBJobSpec & js, js_and_conv) { jobspecs.push_back(js); }
  /*  }
    if (jobspecs.empty())
    {
 ...
 */
vector<VBJobSpec> process_block(const shared_ptr<VB::Block>& this_bp,
                                VB::DataSet* ds,
                                set<shared_ptr<VB::Job> >& prev_jobs) {
  vector<VBJobSpec> jobspecs;
  if (ds) {
    DEBUG_OUT("  Begin processing the block '");
    DEBUG_OUT(this_bp->name + "'\n");
    //  DataSet my_ds = ds;
    ...
    vbforeach(EP ep, this_bp->execs) {
      ...
      vbforeach(DataSet * dspi, datasets) {
        // If the instance is a block, it needs to be unfolded as well.  So
        // here's a bit of recursion for you.
        if (bdp) {
          vector<VBJobSpec> block_jobspecs = process_block(
              static_pointer_cast<Block>(ep), dspi, local_prev_jobs);
          jobspecs.insert(jobspecs.end(), block_jobspecs.begin(),
                          block_jobspecs.end());
          continue;
        }
        ...
        if (jp) {
          ...
          jobspecs.insert(jobspecs.end(), job_jobspecs.begin(),
                          job_jobspecs.end());
        ...
        DEBUG_OUT(
            "ok, this exec instance appears to neither be a block or a job.\n");
      }
    }

    prev_jobs = final_jobs[static_pointer_cast<Exec>(this_bp)];

    DEBUG_OUT("  End processing the block '");
    DEBUG_OUT(this_bp->name + "' (returning VBJobSpecs)\n");
...
bool jobspec_depends_on(int jnum1, int jnum2, VBSequence& seq,
  ...
  else if (find(js.waitfor.begin(), js.waitfor.end(), jnum2) !=
           js.waitfor.end())
 ...
 */
void VB::do_output(const VB::Sequence& my_seq, VB::DataSet my_ds,
                   VBSequence& seq, ExecPoint beginpoint, ExecPoint endpoint) {
  ...
  vector<VBJobSpec> tmpj =
      process_block(my_seq.root_block(), temp_ds, last_set);
  vbforeach(VBJobSpec & js, tmpj) seq.specmap[js.jnum] = js;
  delete temp_ds;
  ...
  if (beginpoint.first && beginpoint.second) {
    ...
    for (SMI iter = seq.specmap.begin(); iter != seq.specmap.end(); ++iter) {
      ...
      bool in_set = false;
      for (list<shared_ptr<Job> >::iterator ji = begin_job_list.begin();
           ji != begin_job_list.end(); ++ji) {
        int begin_jnum = calc_job_num(*ji, ds);
        if (jobspec_depends_on(js.jnum, begin_jnum, seq, jobnum_map,
                               jobspec_depend_map)) {
          in_set = true;
          break;
        }
      }
      if (!in_set) jnums_to_remove.insert(js.jnum);
    }
  }
  ...
  if (endpoint.first && endpoint.second) {
    ...
    for (SMI iter = seq.specmap.begin(); iter != seq.specmap.end(); ++iter) {
      VBJobSpec& js = iter->second;
      bool in_set = false;
      for (list<shared_ptr<Job> >::iterator ji = end_job_list.begin();
           ji != end_job_list.end(); ++ji) {
        int end_jnum = calc_job_num(*ji, ds);
        if (jobspec_depends_on(end_jnum, js.jnum, seq, jobnum_map,
                               jobspec_depend_map)) {
          in_set = true;
          break;
        }
...
void Sequence::export_to_disk(const VB::DataSet&, const string&, ExecPoint,
                              ExecPoint) const {
  //   // In case multiple sequences are being exported during the same
  //   process... static int num = -1;
...
void Sequence::submit(const VB::DataSet&, ExecPoint, ExecPoint) const {
  //   VBSequence seq;
