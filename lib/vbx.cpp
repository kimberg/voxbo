...
void talk2child(VBJobSpec &js, vector<string> myscript, int &logpipe,
                int &cmdpipe);
...
int run_voxbo_job(VBPrefs &vbp, VBJobSpec &js) {
  ...
  if (js.jt.commandlist.size()) {
    ...
    js.error = -1;
    js.errorstring =
        str(format("jobtype %s has no commands") % js.jt.shortname);
  }
  return 0;
}
...
int fork_command(VBJobSpec &js, int ind) {
  ...
  if (js.f_cluster)
    tell_scheduler(js.queuedir, js.hostname,
                   (string) "jobrunning " + js.hostname + " " +
                       strnum(js.snum) + " " + strnum(js.jnum) + " " +
                       strnum(js.pid) + " " + strnum(pid) + " " +
                       strnum(time(NULL)));
  ...
  if (cmdpipe[1] > 0) close(cmdpipe[1]);

  return 0;
}

void run_command(VBJobSpec &js, int ind) {
  ...
  for (int i = 0; i < (int)js.jt.setenvlist.size(); i++) {
    ...
    fill_vars2(jtenv,
               envmap(environ));  // jobtype env could depend on global env!
...
void talk2child(VBJobSpec &js, vector<string> myscript, int &logpipe,
  ...
  if (js.f_cluster)
    qlogfname =
        js.queuedir + "/" + js.seqdirname() + "/" + js.basename() + ".log";
  if (js.logdir.size()) ulogfname = js.logdir + "/" + js.basename() + ".log";
  ...
  while (1) {
    ...
    FD_ZERO(&readset);
    FD_ZERO(&writeset);
    if (logpipe > -1) FD_SET(logpipe, &readset);
    if (cmdpipe > -1) FD_SET(cmdpipe, &writeset);
    ...
    for (size_t i = 0; i < bufset.size(); i++) {
      ...
      // handle message lines
        ...
        if (js.jt.msg_tag.size() < bufstr.size()) js.errorstring = bufstr;
      }
      // handle warning lines
        ...
        if (js.jt.warn_tag.size() < bufstr.size()) js.errorstring = bufstr;
        if (js.GetState() == XNone) js.SetState(XWarn);
      }
      // handle retry lines
...
void test_outputline(VBJobSpec &js, string &line) {
  for (TGI tgi = js.jt.triggerlist.begin(); tgi != js.jt.triggerlist.end();
       tgi++) {
    if (tgi->cond == "match") {
...
void parse_status(VBJobSpec &js, int status) {
  char tmp[STRINGLEN];
...
void do_internal(VBJobSpec &js) {
  ...
  if (js.jobtype == "timewaster") {
    ...
    int i = 0;
    while (environ[i]) fprintf(stdout, "%s\n", environ[i++]);
    ...
    msg += "Subject: VoxBo Sequence \"" + js.seqname + "\" (" +
           strnum(js.snum) + ")\n";
...
void exec_command(VBJobSpec &js, vector<string> script, int ind) {
  ...
  if (js.arguments.size()) fprintf(stderr, "arguments:\n");
  ...
  fprintf(stderr,
          "below is the output of your job, with script input tagged [S]\n");
...
int runseq(VBPrefs &vbp, VBSequence &seq, uint32 njobs) {
  ...
  for (SMI js = seq.specmap.begin(); js != seq.specmap.end(); js++) {
    ...
    if (vbp.jobtypemap.count(js->second.jobtype))
      ...
      cout << format(
                  "[E] your sequence has at least one unrecognized jobtype "
                  "(%s)\n") %
                  js->second.jobtype;
  ...
  while (1) {
    ...
    if (WIFEXITED(mystatus) && WEXITSTATUS(mystatus) == 0) {
      ...
      if (f_quit == 0) {
        ...
        string resp =
            vb_tolower(vb_getchar("\n[v]iew log, [e]dit log, [s]kip job, "
                                  "[r]etry job, [c]ontinue, or [q]uit: "));
        printf("\n\n");
        if (resp == "v") {
          printf(
              "======================BEGIN LOG FILE======================\n");
...
set<int32> readyjobs(VBSequence &seq, uint16 max) {
  ...
  for (SMI j = seq.specmap.begin(); j != seq.specmap.end(); j++) {
    ...
    if (!unmet) {
      ...
      if (readyset.size() >= max) return readyset;
    }
  }
  return readyset;
}
