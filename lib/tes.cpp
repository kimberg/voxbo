...
Tes::Tes(int in_dimx, int in_dimy, int in_dimz, int in_dimt,
         VB_datatype in_type) {
  mask = (unsigned char *)NULL;
  data = (unsigned char **)NULL;
...
int Tes::init(int in_dimx, int in_dimy, int in_dimz, int in_dimt,
              VB_datatype in_type) {
  init();
  return (SetVolume(in_dimx, in_dimy, in_dimz, in_dimt, in_type));
}

void Tes::SetVoxSizes(float v1, float v2, float v3) {
  voxsize[0] = v1;
  voxsize[1] = v2;
  voxsize[2] = v3;
}

int Tes::SetVolume(uint32 in_dimx, uint32 in_dimy, uint32 in_dimz,
                   uint32 in_dimt, VB_datatype in_type) {
  if (in_dimx > MAX_DIM || in_dimy > MAX_DIM || in_dimz > MAX_DIM ||
      in_dimt > MAX_DIM)
    return 101;
  dimx = in_dimx;
  dimy = in_dimy;
...
int Tes::DimsValid() {
  if (dimx < 1 || dimy < 1 || dimz < 1 || dimt < 1) return 0;
  // FIXME might want to rule out some improbably large sizes here
  return 1;
}

int Tes::InitMask(short val) {
  if (!DimsValid()) return 101;
  if (mask && !f_mirrored) delete[] mask;
  f_mirrored = 0;
  mask = new unsigned char[dimx * dimy * dimz];
  if (!mask) return 102;
  for (int i = 0; i < dimx * dimy * dimz; i++) mask[i] = val;
  return 0;
}
...
int Tes::InitData() {
  ...
  if (!data) return 102;
  for (int i = 0; i < dimx * dimy * dimz; i++) data[i] = (unsigned char *)NULL;
  data_valid = 1;
  return 0;
}
...
int Tes::ReadFile(const string &fname, int start, int count) {
  ...
  if ((err = ReadData(fname, start, count))) return err;
  return 0;
}

int Tes::ReadHeader(const string &fname) {
  // never call reparse here, because it will end up converting
  // foo.tes:3 to foo.tes, and we'll end up reading it as 4D
  init();
...
int Tes::ReadData(const string &fname, int start, int count) {
  // never call reparse here, because it will end up converting
  // foo.tes:3 to foo.tes, and we'll end up reading it as 4D
  ...
  if (!fileformat.read_data_4D) return 102;
  err = fileformat.read_data_4D(this, start, count);
  return err;
}

int Tes::ReadTimeSeries(const string &fname, int x, int y, int z) {
  int err;
  if (!header_valid) {
    if ((err = ReadHeader(fname))) return err;
  }
  if (!fileformat.read_ts_4D) return 102;
  err = fileformat.read_ts_4D(*this, x, y, z);
  return err;
}

int Tes::ReadVolume(const string &fname, int t, Cube &cb) {
  int err;
  if (!header_valid) {
    if ((err = ReadHeader(fname))) return err;
  }
  if (!fileformat.read_vol_4D) return 102;
  cb.init();
  err = fileformat.read_vol_4D(*this, cb, t);
  return err;
}

int Tes::WriteFile(const string fname) {
  VBFF original;
  ...
  if (!fileformat.write_4D) fileformat = findFileFormat(filename, 4);
  // if not, try original file's format
  if (!fileformat.write_4D) fileformat = original;
  // if not, try cub1
  if (!fileformat.write_4D) fileformat = findFileFormat("tes1");
  // if not (should never happen), bail
  if (!fileformat.write_4D) return 200;
  int err = fileformat.write_4D(this);
  return err;
}

int Tes::SetCube(int t, const Cube &cube) {
...
int Tes::GetTimeSeries(int x, int y, int z) {
  if (!inbounds(x, y, z)) return 101;
  timeseries.resize(dimt);
  for (int i = 0; i < dimt; i++) timeseries[i] = GetValue(x, y, z, i);
  return 0;
}

int Tes::MergeTes(Tes &src) {
  Tes *dest = this;
  ...
  if (src.datatype != dest->datatype) return 102;
  for (int i = 0; i < dimx * dimy * dimz; i++) {
    if (src.data[i] == NULL) continue;
    if (data[i] == NULL) buildvoxel(i);
    memcpy(dest->data[i], src.data[i], datasize * dimt);
  }
  return 0;
}

Cube Tes::getCube(const int index) {
  Cube cb;
  getCube(index, cb);
  return cb;
}

int Tes::getCube(int index, list<Cube> &cubelist) {
  Cube tmpc;
  list<Cube> tmplist;
...
Cube Tes::operator[](const int index) {
  Cube c;
  getCube(index, c);
  return c;
}

int Tes::getCube(const int index, Cube &c) {
  c.init();
  if (!data_valid) return 101;
  c.dimx = dimx;
  c.dimy = dimy;
  c.dimz = dimz;
  c.datatype = datatype;
  ...
  if (c.data) {
    ...
    // long pos=0;
    // once we have a better Tes::getValue(), we can rewrite the following to be
    // a lot faster
    for (int i = 0; i < dimx * dimy * dimz; i++) {
      if (mask[i]) memcpy(pos, data[ord] + cpos, datasize);
      pos += datasize;
      ord++;
    }
  } else {
...
double Tes::GetValue(VBVoxel &v, int t) const {
  return GetValue(v.x, v.y, v.z, t);
}

double Tes::GetValue(int x, int y, int z, int t) const {
  if (!inbounds(x, y, z) || t > dimt - 1) return 0.0;
...
double Tes::GetValueUnsafe(int x, int y, int z, int t) const {
  int index = voxelposition(x, y, z);
  if (!data[index])  // not stored, so presumed 0
    return 0.0;
...
template <class T>
T Tes::getValue(VBVoxel &v, int t) const {
  return getValue<T>(v.x, v.y, v.z, t);
}

template <class T>
T Tes::getValue(int x, int y, int z, int t) const {
  if (!inbounds(x, y, z) || t > dimt - 1) return 0.0;
  int index = voxelposition(x, y, z);
  if (!data)  // application error
    return 0.0;
...
bool Tes::GetMaskValue(int x, int y, int z) const {
  if (!inbounds(x, y, z)) return 0;
  int index = voxelposition(x, y, z);
  return (mask[index]);
}

bool Tes::GetMaskValue(int index) const { return (mask[index]); }
...
int Tes::VoxelStored(int x, int y, int z) {
  if (!inbounds(x, y, z)) return 0;
  int index = voxelposition(x, y, z);
  if (data[index])
    return 1;
...
void Tes::SetValue(int x, int y, int z, int t, double val) {
  if (!inbounds(x, y, z) || t > dimt - 1) return;
  int index = voxelposition(x, y, z);
  if (!data[index] && fabs(val) < DBL_MIN)  // already set!
    return;
  if (!data[index]) buildvoxel(index);
  unsigned char *ptr = data[index] + (t * datasize);
  switch (datatype) {
    case vb_byte:
...
unsigned char *Tes::buildvoxel(int x, int y, int z) {
  if (!data) return NULL;
  int index;
  if (y < 0 && z < 0)
    index = x;
  ...
  switch (datatype) {
    case vb_short:
      for (i = 0; i < dimx * dimy * dimz; i++) {
        if (data[i]) swap((int16 *)data[i], dimt);
      }
      break;
    case vb_long:
      for (i = 0; i < dimx * dimy * dimz; i++) {
        if (data[i]) swap((int32 *)data[i], dimt);
      }
      break;
    case vb_byte:
      // no action necessary!
    ...
    case vb_float:
      for (i = 0; i < dimx * dimy * dimz; i++) {
        if (data[i]) swap((float *)data[i], dimt);
      }
      break;
    case vb_double:
      for (i = 0; i < dimx * dimy * dimz; i++) {
        if (data[i]) swap((double *)data[i], dimt);
      }
      break;
  }
}

double Tes::GrandMean() {
...
void Tes::print() const { cout << *this; }

ostream &operator<<(ostream &os, const Tes &ts) {
  os << endl
     << "+- 4D Image file " << xfilename(ts.GetFileName()) << " ("
     << ts.fileformat.getName() << ")"
     << " (" << DataTypeName(ts.f_scaled ? ts.altdatatype : ts.datatype)
     << (ts.f_scaled ? ", scaled)" : ")") << endl;
  if (!ts.header_valid) {
    os << "+- invalid 4D data\n";
    return os;
  }
  ...
  os.precision(4);
  os << "| " << ts.voxsize[0] << " x " << ts.voxsize[1] << " x "
     << ts.voxsize[2] << " mm, TR=" << ts.voxsize[3] << "ms" << endl;
  os << "| realvoxels: " << ts.realvoxels << " of "
     << ts.dimx * ts.dimy * ts.dimz << endl;
  os.precision(1);
  os << "| " << ts.meglen() << "MB on disk ("
     << (ts.filebyteorder == ENDIAN_BIG ? "msbfirst" : "lsbfirst") << ")"
     << endl;
  os << "| origin: (" << ts.origin[0] << "," << ts.origin[1] << ","
     << ts.origin[2] << ")" << endl;
  os.precision(2);
  if (ts.voxsize[0] > FLT_MIN && ts.voxsize[1] > FLT_MIN &&
      ts.voxsize[1] > FLT_MIN) {
    string tmp =
        (format("[%g,%g,%g;%g,%g,%g]") % (ts.voxsize[0] * ts.origin[0]) %
         (ts.voxsize[1] * ts.origin[1]) % (ts.voxsize[2] * ts.origin[2]) %
  ...
  if (ts.f_scaled)
    os << "| slope: " << ts.scl_slope << ","
       << "intercept: " << ts.scl_inter << endl;
  if (ts.header.size() > 0) {
    os << "+--user header----------" << endl;
...
void Tes::printbrief(const string &flags) const {
  ...
  for (int i = 0; i < (int)myflags.size(); i++) {
    ...
    if (myflags[i] == 'f')
      ...
      cout << voxsize[0] << "x" << voxsize[1] << "x" << voxsize[2]
           << "mm, TR=" << voxsize[3];
    } else if (myflags[i] == 'o')
...
Tes &Tes::operator=(const Tes &ts) { return copytes(ts); }

Tes &Tes::copytes(const Tes &ts, bool mirrorflag) {
  if (!ts.header_valid) return *this;
  init();
  ...
  if (mirrorflag) {
    ...
    if (ts.mask) {
      mask = new unsigned char[dimx * dimy * dimz];
      if (mask) memcpy(mask, ts.mask, dimx * dimy * dimz);
    } else
      mask = (unsigned char *)NULL;
  }
...
float Tes::meglen() const {
  if (header_valid)
    return (float)(dimt * realvoxels * datasize) / (1024 * 1024);
  return 0.0;
}

const unsigned char *Tes::GetMaskPtr() { return mask; }

int Tes::ExtractMask(Cube &target) {
  if (!header_valid || !mask) return 101;
  target.SetVolume(dimx, dimy, dimz, vb_byte);
  int index = 0;
  for (int k = 0; k < dimz; k++) {
    for (int j = 0; j < dimy; j++) {
      for (int i = 0; i < dimx; i++) {
        if (GetMaskValue(i, j, k)) target.data[index] = 1;
        index++;
      }
    }
  }
...
int Tes::convert_type(VB_datatype newtype, uint16 flags) {
    ...
    for (int k = 0; k < dimz; k++) {
      for (int j = 0; j < dimy; j++) {
        for (int i = 0; i < dimx; i++) {
          ...
          unsigned char *tmp =
              convert_buffer(data[ind], dimt, datatype, newtype);
          if (tmp == NULL) {
            invalidate();
            return 120;
          }
...
void Tes::zerovoxel(int index) {
  delete[] data[index];
  data[index] = (unsigned char *)NULL;
  mask[index] = 0;
}

void Tes::zerovoxel(int x, int y, int z) { zerovoxel(voxelposition(x, y, z)); }
    ...
    for (int j = 0; j < dimy; j++) {
      for (int i = 0; i < dimx; i++) {
        ...
        for (int t = 0; t < dimt; t++) {
          if (!(finite(GetValue(i, j, k, t)))) SetValue(i, j, k, t, 0.0);
        }
      }
    }
  }
...
void Tes::applymask(Cube &m) {
  for (int i = 0; i < m.dimx * m.dimy * m.dimz; i++) {
    if (data[i] && !(m.testValue(i))) zerovoxel(i);
  }
}
...
int Tes::resizeInclude(const set<int> &includeset) {
  if (includeset.empty()) return 101;
  if (*(includeset.begin()) < 0) return 102;
  if (*(includeset.rbegin()) > dimt - 1) return 103;
  Tes tmptes;
  tmptes = *this;
  ...
  for (int i = 0; i < dimt; i++) {
    if (includeset.count(i)) tmptes.SetCube(ind++, (*this)[i]);
  }
  *this = tmptes;
  return 0;
}

int Tes::resizeExclude(const set<int> &excludeset) {
  if (excludeset.size() > (size_t)dimt) return 101;
  if (*(excludeset.begin()) < 0) return 102;
  if (*(excludeset.rbegin()) > dimt - 1) return 103;
  Tes tmptes;
  tmptes = *this;
  ...
  int ind = 0;
  for (int i = 0; i < dimt; i++) {
    if (!(excludeset.count(i))) tmptes.SetCube(ind++, (*this)[i]);
  }
  *this = tmptes;
  return 0;
}
...
VB_Vector getTS(vector<string> &teslist, int x, int y, int z, uint32 flags) {
  ...
  for (int i = 0; i < (int)teslist.size(); i++) {
    ...
    if (flags & MEANSCALE) mytes.timeseries.meanNormalize();
    if (flags & DETREND) mytes.timeseries.removeDrift();
...
VBMatrix getRegionComponents(vector<string> &teslist, VBRegion &rr,
  ...
  for (int i = 0; i < (int)teslist.size(); i++) {
    if (tlist[i].ReadHeader(teslist[i])) return empty;
  ...
  for (int i = 0; i < (int)teslist.size(); i++) {
    ...
    for (VI myvox = rr.begin(); myvox != rr.end(); myvox++) {
      rr.getxyz(myvox->first, xx, yy, zz);
      if (tlist[i].ReadTimeSeries(noname, xx, yy, zz)) return empty;
      if (flags & MEANSCALE) tlist[i].timeseries.meanNormalize();
      if (flags & DETREND) tlist[i].timeseries.removeDrift();
      for (int m = row; m < row + tlist[i].dimt; m++) {
        gsl_matrix_set(&components.mview.matrix, m, j,
                       tlist[i].timeseries[m - row]);
      }
    }
    j++;
    row += tlist[i].dimt;
  ...
  VBMatrix tmp, E;
  VB_Vector lambdas;
  if (pca(components, lambdas, tmp, E)) return empty;
  return tmp;
}
...
VBRegion restrictRegion(vector<string> &teslist, VBRegion &rr) {
  ...
  for (size_t i = 0; i < teslist.size(); i++) {
    Tes ts;
    if (ts.ReadHeader(teslist[i])) continue;
    if (!ts.fileformat.f_headermask) {
      if (ts.ReadFile(teslist[i])) continue;
    }
    if (ts.ExtractMask(tesmask)) continue;
    if (!mask.data)
      mask = tesmask;
    else
