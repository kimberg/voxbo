...
class Sequence {
 public:
  ...
  void build_depth_table_helper(shared_ptr<Exec> exec,
                                std::map<shared_ptr<Exec>, int>* table);
  ...
  void export_to_disk(const VB::DataSet&, const string& dirname,
                      ...
                      ExecPoint endpoint = ExecPoint(shared_ptr<Exec>(),
                                                     0)) const;
...
/*  struct ExecNode
  {
    ...
    bool has_in_from(ExecNode* en_from) {
      ...
      for (list<ExecNode*>::iterator iter = in_from.begin(); !found && iter !=
  in_from.end(); ++iter) { ExecNode*& en = *iter; found =
  en->has_in_from(en_from);
      }
      return found;
    }
  };
  ...
  struct ExecNet
  {
    ...
    void clear() {
      for (list<ExecNode*>::iterator iter = nodes.begin();  iter != nodes.end();
  ++iter) { ExecNode* en = *iter; delete en;
      }
      nodes.clear();
    }
    ...
    void validate_all() {
      for (list<ExecNode*>::iterator iter = nodes.begin();  iter != nodes.end();
  ++iter) { ExecNode* en = *iter; en->validated = true;
      }
    }
    ...
    void invalidate(ExecNode* en_from)
    {
      ...
      for (list<ExecNode*>::iterator iter = nodes.begin();  iter != nodes.end();
  ++iter) { ExecNode* en = *iter; if (en->directly_in_from(en_from)) {
          invalidate(en);
        }
      }
    }
