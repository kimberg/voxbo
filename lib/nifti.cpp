...
int nifti_read_header(string fname, NIFTI_header *xhdr, VBImage *vol) {
  ...
  gzFile fp;
  bool f_swap = 0;
  if (!(fp = gzopen(fname.c_str(), "r"))) return (100);
  ...
  // byte swap if needed
  if (hdr->dim[0] < 1 || hdr->dim[0] > 7) f_swap = 1;
  if (f_swap) {
    if (bo == ENDIAN_BIG)
      bo = ENDIAN_LITTLE;
    else
  ...
  if ((string)hdr->magic != "ni1" && (string)hdr->magic != "n+1") {
    gzclose(fp);
    return 102;
  }
  ...
  if (vol && gztell(fp) == 348 && hdr->vox_offset >= 352) {
    ...
    if (cnt == 4 && exts[0] != 0) {
      while (gzread(fp, &n11ext, 8) == 8) {
        // swap if needed
        if (f_swap) swap(&n11ext.esize), swap(&n11ext.ecode);
        // is it voxbo ecode?
        ...
        tmpt.SetSeparator("\n\r");
        tmpt.ParseLine(buf);
        for (size_t i = 0; i < tmpt.size(); i++) vol->header.push_back(tmpt[i]);
      }
    }
  }
  ...
  if (hdr->sizeof_hdr != 348) return 106;
  if (hdr->dim[0] < 2 || hdr->dim[0] > 4) return 107;
  ...
  vol->AddHeader((string) "nifti_slice_duration " +
                 strnum(hdr->slice_duration));
  vol->AddHeader((string) "nifti_toffset " + strnum(hdr->toffset));

  vol->offset = (int)hdr->vox_offset;
  if (vol->offset < NIFTI_MIN_OFFSET_BOGUS)
    vol->offset = NIFTI_MIN_OFFSET_BOGUS;
  vol->header_valid = 1;
  return 0;
}
...
void print_nifti_header(NIFTI_header &ihead) {
  ...
  cout << format("                dims: %d %d %d %d %d %d %d %d\n") %
              ihead.dim[0] % ihead.dim[1] % ihead.dim[2] % ihead.dim[3] %
              ihead.dim[4] % ihead.dim[5] % ihead.dim[6] % ihead.dim[7];
  cout << format("         intent code: %d (params %g %g %g)\n") %
              ihead.intent_code % ihead.intent_p1 % ihead.intent_p2 %
              ihead.intent_p3;
  cout << format("         intent_name: %.16s\n") % ihead.intent_name;
  cout << format("            datatype: %d (%s)\n") % ihead.datatype %
              nifti_typestring(ihead.datatype);
  ...
  cout << format("%20s: %g %g %g\n") % tmps % ihead.pixdim[1] %
              ihead.pixdim[2] % ihead.pixdim[3];

  cout << format("                  TR: %g%s\n") % ihead.pixdim[4] % timeunits;

  cout << format("         pixdim[5-7]: %g %g %g\n") % ihead.pixdim[5] %
              ihead.pixdim[6] % ihead.pixdim[7];
  ...
  cout << format("       quatern b/c/d: %g/%g/%g\n") % ihead.quatern_b %
              ihead.quatern_c % ihead.quatern_d;
  cout << format("       qoffset x/y/z: %g/%g/%g\n") % ihead.qoffset_x %
              ihead.qoffset_y % ihead.qoffset_z;
  cout << format("              srow_x: %8.4g %8.4g %8.4g %8.4g\n") %
              ihead.srow_x[0] % ihead.srow_x[1] % ihead.srow_x[2] %
              ihead.srow_x[3];
  cout << format("              srow_y: %8.4g %8.4g %8.4g %8.4g\n") %
              ihead.srow_y[0] % ihead.srow_y[1] % ihead.srow_y[2] %
              ihead.srow_y[3];
  cout << format("              srow_z: %8.4g %8.4g %8.4g %8.4g\n") %
              ihead.srow_z[0] % ihead.srow_z[1] % ihead.srow_z[2] %
              ihead.srow_z[3];
...
int nifti_write_3D(string fname, Cube &im) {
  ...
  if (im.f_scaled) {
    im -= im.scl_inter;
    im /= im.scl_slope;
    if (im.altdatatype == vb_byte || im.altdatatype == vb_short ||
        im.altdatatype == vb_long)
  ...
  string buf;
  if (f_ext) {
    for (size_t i = 0; i < im.header.size(); i++) buf += im.header[i] + '\n';
    // the offset is an extra ecode+esize, plus the size of the extension, plus
    // the sentinel
    if (buf.size()) offset += buf.size() + 16;
  }
  ...
  if (f_ext && im.header.size()) {
    ...
    for (size_t i = 0; i < im.header.size(); i++) buf += im.header[i] + '\n';
    esize = buf.size();
    if (im.filebyteorder != my_endian()) swap(&ecode), swap(&esize);
  ...
  if (im.filebyteorder != my_endian()) im.byteswap();
...
int nifti_read_3D_data(Cube &im) {
  string fname = im.GetFileName();
  if (xgetextension(fname) == "hdr") fname = xsetextension(fname, "img");
  im.SetVolume(im.dimx, im.dimy, im.dimz, im.datatype);
  if (im.dimx < 1 || im.dimy < 1 || im.dimz < 1) {
    im.data_valid = 0;
    return 105;
  }
  if (!im.data) return 101;
  ...
  if (my_endian() != im.filebyteorder) im.byteswap();
  if (im.f_scaled) {
    if (im.altdatatype == vb_byte || im.altdatatype == vb_short ||
        im.altdatatype == vb_long)
      im.convert_type(vb_float);
...
int nifti_read_4D_data(Tes &im, int start, int count) {
  string fname = im.GetFileName();
  if (xgetextension(fname) == "hdr") fname = xsetextension(fname, "img");
  ...
  for (int i = 0; i < im.dimt; i++) {
    ...
    if (my_endian() != im.filebyteorder) cb.byteswap();
    im.SetCube(i, cb);
  }
  if (im.f_scaled) {
    if (im.altdatatype == vb_byte || im.altdatatype == vb_short ||
...
int nifti_read_ts(Tes &im, int x, int y, int z) {
  string fname = im.GetFileName();
  if (xgetextension(fname) == "hdr") fname = xsetextension(fname, "img");
  if (x < 0 || y < 0 || z < 0 || x > im.dimx - 1 || y > im.dimy - 1 ||
      z > im.dimz - 1)
    return 101;
  ...
  gzclose(fp);
  if (my_endian() != im.filebyteorder) swapn(tmpdata, im.datasize, im.dimt);
...
int nifti_read_vol(Tes &im, Cube &cb, int t) {
  string fname = im.GetFileName();
  if (xgetextension(fname) == "hdr") fname = xsetextension(fname, "img");
  if (t < 0 || t > im.dimt - 1) return 101;
  cb.SetVolume(im.dimx, im.dimy, im.dimz, im.datatype);
  gzFile fp = gzopen(fname.c_str(), "r");
  if (!fp) {
    cb.invalidate();
