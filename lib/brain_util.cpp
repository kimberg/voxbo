 ...
 */
int chkRegionName(string dbHome, string rDbName, string rName, string name_space)
{
...
/* This function checks whether a certain synonym string exists in a certain namespace 
 * of synonym db.
 * Returns -1 or -2 for exceptions;
 * returns 0 if it does not exist;
 * returns 1 if it does exist;
 */
int chkSynonymStr(string dbHome, string sDbName, string sName, string name_space)
{
...
int getAllRegions(string dbHome, string rName, 
		  vector <string> &nameList, vector <string> &spaceList)
{
...
/* This function assigns all region names in a certain namespace in region_name.db 
 * to the last argument.
 * Returns 0 if database read process is correct. 
 * Returns -1 or -2 for db exceptions. */ 
int getRegions(string dbHome, string rName, string name_space_in, vector <string> &nameList)
{
  mydb regionDB(dbHome, rName, DB_RDONLY);
  Dbc *cursorp;
  Dbt key, data;
 ...
 * Returns 0 if database read process is correct. 
 * Returns negative values if db read exception shows up. */ 
int getAllSynonyms(string dbHome, string sName,
		   vector <string> &nameList, vector <string> &spaceList)
{
 ...
 * Returns 0 if database read process is correct. 
 * Returns negative values if db read exception shows up. */ 
int getSynonyms(string dbHome, string sName, string name_space_in, vector <string> &nameList)
{
  mydb synonymDB(dbHome, sName, DB_RDONLY);
  Dbc *cursorp;
  Dbt key, data;
...
int getRegionRec(string dbHome, string rDbName, string inputName, string name_space, regionRec &outputRec)
{
...
/* This function searches region name db according to relation name and namespace
 * and returns the region ID.
 * Returns -1 or -2 for exceptions;
 * returns 0 if it is not found;
 */
long getRegionID(string dbHome, string rDbName, string rName, string name_space)
{
  mydb rDB(dbHome, rDbName, DB_RDONLY);  
  Dbc *cursorp;
  Dbt key, data;
...
/* Seach a synonym name in synonym db file and put primary structure name in pName. 
 * Returns -1 or -2 if exceptions are met;
 * returns 0 if parent name is not found;
 * returns 1 if parent name is found. */
int getPrimary(string dbHome, string sDbName, string sName_in, string name_space_in, string &pName)
{
  ...
  try {
    ...
    while ((ret = cursorp->get(&key, &data, DB_NEXT)) == 0 ) {
      synonymRec sData(data.get_data());
      if (sData.getName() == sName_in && sData.getNameSpace() == name_space_in) {
...
/* This function collects a input region's synonyms and put in a string list argument.
 * Returns -1 or -2 if exceptions are met;
 * returns 0 if db file is read in w/0 error. */
int getSynonym(string dbHome, string sDbName, string rName_in, string name_space_in, 
	       vector<string> &sList) 
{
  ...
  try {
    ...
    while ((ret = cursorp->get(&key, &data, DB_NEXT)) == 0 ) {
      synonymRec sData(data.get_data());
      if (sData.getPrimary() == rName_in && sData.getNameSpace() == name_space_in) 
...
*/
long getRelationID(string dbHome, string rrDbName, long r1, long r2, string relStr)
{
  ...
  try {
    ...
    while ((ret = cursorp->get(&key, &data, DB_NEXT)) == 0) {
      regionRelationRec rrData(data.get_data());
      if ((r1 == rrData.getRegion1() && r2 == rrData.getRegion2() && relStr == rrData.getRelationship()) ||
	  (relStr == "overlap" && r1 == rrData.getRegion2() && r2 == rrData.getRegion1() 
...
int getRel_noChild(string dbHome, string rrDbName, long excludeID,
 ...
 * in relationship db. Note the difference between this function and previous one. 
 * Return -1 or -2 when exceptions are met;
 * Return 0 if reading w/o error. */
int getRel_all(string dbHome, string rrDbName, 
	       long r1, long r2, vector <string> &relList)
{
  mydb relationDB(dbHome, rrDbName, DB_RDONLY);  
  string relStr;
 ...
 */
int findRegionNames(string dbHome, string dbName, string key_in, bool ik_flag, 
		    string ns_in, bool ins_flag, vector <regionRec> &outputList)
{
  string keyStr = key_in;
  if (ik_flag)
    keyStr = toLowerCase(key_in);
  ...
  try {
	...
	rName = toLowerCase(rData.getName());
      ...
      if (ins_flag)
	nsName = toLowerCase(rData.getNameSpace());
		 ...
		 vector <synonymRec> &outputList)
{
  ...
  try {
	...
	sName = toLowerCase(sData.getName());
      if (sName.find(keyStr) != string::npos) 
