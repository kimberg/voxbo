...
VBFF findFileFormat(const string &signature) {
  ...
  if (VBFF::filetypelist.size() == 0) VBFF::LoadFileTypes();
...
VBFF findFileFormat(const string &fname, int dims) {
  ...
  if (ext == "gz") ext = xgetextension(xsetextension(fname, ""));
...
VBFF *EligibleFileTypesByExtension(string extn, int dims) {
...
vector<VBFF> EligibleFileTypes(string fname, int dims) {
  ...
  if (infile) {
    ...
    buf[BUFSIZE - 1] = '\0';
    if (slugsize < BUFSIZE) buf[slugsize] = '\0';
    gzclose(infile);
...
int VBFF::LoadFileTypes() {
  ...
  for (size_t i = 0; i < vg.size(); i++) LoadFiletype(vg[i]);
...
void VBFF::printffblurb() {
  printf(
      "  The format for image output files is determined by the file "
      "extension.\n");
  printf(
      "  To force a specific byteorder, append [big] or [little] to the "
      "filename\n");
  printf(
      "  (e.g., foo.cub[big]).  VoxBo cub and tes formats and NIfTI 3D and "
      "4D\n");
  printf(
      "  volumes may be stored compressed by tacking .gz onto the filename.\n");
  // printf("  To force...\n");
}
...
int VBFF::LoadFiletype(string) {
  return 0;
  //   void *filehandle;
  //   filehandle=dlopen(dlfile.c_str(),RTLD_NOW);
  //   // FIXME below appropriate use of error msg?
  //   if (!filehandle) {
  //     printf("[E] vbff.cpp: error opening %s:
  //     %s\n",dlfile.c_str(),dlerror()); return 101;
  //   }
  //   ffinitfn initfn;
  //   VBFF tmpff;
...
void VBFF::install_filetype(VBFF newff) {
...
void VBFF::LoadBuiltinFiletypes() {
  VBFF tmp;
...
string VBFF::getName() const { return (string)name; }

string VBFF::getSignature() const { return (string)signature; }

string VBFF::getPath() const { return path; }

int VBFF::getDimensions() const { return dimensions; }
...
string GetHeader(vector<string> headers, string tag) {
  ...
  for (int i = 0; i < (int)headers.size(); i++) {
    ...
    if (args[0][args[0].size() - 1] == ':') args[0].erase(args[0].end() - 1);
    if (vb_tolower(args[0]) == vb_tolower(tag)) return (headers[i]);
  }
  string tmp;
  return (tmp);
}
