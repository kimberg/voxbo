...
int
smoothCube_m(Cube &cube,Cube &mask,double s0,double s1,double s2)
{
  if (mask.dimx!=cube.dimx || mask.dimy!=cube.dimy ||
      mask.dimz!=cube.dimz)
    return 101;
  Cube smask=mask;
  if (smoothCube(smask,s0,s1,s2))
    return 102;
  if (smoothCube(cube,s0,s1,s2))
    return 103;
  double val;
  for (int i=0; i<cube.dimx; i++) {
    for (int j=0; j<cube.dimy; j++) {
      for (int k=0; k<cube.dimz; k++) {
        ...
        if (val>0.0)
          cube.SetValue(i,j,k,cube.GetValue(i,j,k)/val);
      }
    }
  }
  return 0;
...
int
smoothCube(Cube &cube,double s0,double s1,double s2,bool f_correct)
{
  int i;
...
void
conv3d(Cube &cube,VB_Vector &kx,VB_Vector &ky,VB_Vector &kz)
{
  int i,j,k,pad;
  ...
  for (i=0; i<cube.dimx; i++) {
    for (j=0; j<cube.dimy; j++) {
      ...
      for (k=0; k<cube.dimz; k++)
        v[k]=cube.GetValue(i,j,k);
      // convolve
      v.convolve(kz);
      // transfer back
      for (k=0; k<cube.dimz; k++)
        cube.SetValue(i,j,k,v[k+pad]);
    }
  }
  ...
  for (j=0; j<cube.dimy; j++) {
    for (k=0; k<cube.dimz; k++) {
      ...
      // transfer data
      for (i=0; i<cube.dimx; i++)
        v[i]=cube.GetValue(i,j,k);
      // convolve
      v.convolve(kx);
      // transfer back
      for (i=0; i<cube.dimx; i++)
        cube.SetValue(i,j,k,v[i+pad]);
    }
  }
  ...
  for (i=0; i<cube.dimx; i++) {
    for (k=0; k<cube.dimz; k++) {
      ...
      // transfer data
      for (j=0; j<cube.dimy; j++)
        v[j]=cube.GetValue(i,j,k);
      // convolve
      v.convolve(ky);
      // transfer back
      for (j=0; j<cube.dimy; j++)
        cube.SetValue(i,j,k,v[j+pad]);
    }
  }
}
...
void
convolvex(VB_Vector &v1,VB_Vector &kernel)
{
...
void
conv3dx(Cube &cube,VB_Vector &kx,VB_Vector &ky,VB_Vector &kz)
{
  int i,j,k,pad;
  ...
  for (int i=0; i<tes.dimx; i++) {
    for (int j=0; j<tes.dimy; j++) {
      for (int k=0; k<tes.dimz; k++) {
        if (!(tes.data[tes.voxelposition(i,j,k)]))
          continue;
...
int
vbOrientTes(Tes &tes, Tes &outTes, string in, string out, int interleaved)
{
  int err = 0;
  Cube reorientedCube;
  Cube initialCube;
  for(int i = 0; i < tes.dimt; i++) {
    ...
    if (err) {
      ...
      //resize the tes file to match the possibly rotated cube. Only do this once (don't waste the cpu's time.)
      if (i == 0)
	outTes.SetVolume(reorientedCube.dimx, reorientedCube.dimy, reorientedCube.dimz, tes.dimt, tes.datatype);
      err = outTes.SetCube(i, reorientedCube);
      //SetCube returns true(=1) on success, so this makes us check for 1 and reset err = 0 below
      if (err != 1) {
	return 7;
      }
      else
...
int
vbOrient(Cube &inCube, Cube &outCube, string in, string out, int interleaved)
{
  ...
  if (validateOrientation(in) < 0) 
    return 2;

  if (validateOrientation(out) < 0) 
    return 3;  
  ...
  //the initial and desired orientation strings affect the variables below, and,
  //ultimately, how we transcribe the data from the initial cube (cube) to the
  //desired orientation cube (outCube). Briefly, we scan the initial string with
  //each letter from the desired string. Whether the same letter exists in both
  //strings, whether the letter's inverse A<->P, I<->S, L<->R exists, and the location
  //of the letter or its inverse in the initial string tells us which axes in the
  //first cube map to which axes in the second, from which direction we should be copying
  //the voxels, and how many voxels line each side of the cube.
  ...
  //resize the new cube
  if (xfrom == 0 && yfrom == 1 && zfrom == 2) outCube.resize(inCube.dimx, inCube.dimy, inCube.dimz);
  if (xfrom == 0 && yfrom == 2 && zfrom == 1) outCube.resize(inCube.dimx, inCube.dimz, inCube.dimy);
  if (xfrom == 1 && yfrom == 2 && zfrom == 0) outCube.resize(inCube.dimy, inCube.dimz, inCube.dimx); 
  if (xfrom == 1 && yfrom == 0 && zfrom == 2) outCube.resize(inCube.dimy, inCube.dimx, inCube.dimz);
  if (xfrom == 2 && yfrom == 0 && zfrom == 1) outCube.resize(inCube.dimz, inCube.dimx, inCube.dimy);
  if (xfrom == 2 && yfrom == 1 && zfrom == 0) outCube.resize(inCube.dimz, inCube.dimy, inCube.dimx);
  ...
  //based on the initial and desired orientations, we know how to copy the data from the old cube
  //to the new. In short, we transcibe the voxels from the old cube (cube) to the new cube (outCube)
  //by modifying the old cube's origin (initx, inity, initz) and traversing the old cube in the
  //directions necessary to fetch voxels to populate the new cube in the desired way.
  ...
  for (int i=0; i<inCube.dimx; i++) {
    if (xfrom == 1) srcx=initx;
    else if (yfrom == 1) srcy=inity;
    else if (zfrom == 1) srcz=initz;
    for (int j=0; j<inCube.dimy; j++) {
      if (xfrom == 2) srcx=initx;
      else if (yfrom == 2) srcy=inity;
      else if (zfrom == 2) srcz=initz;
      for (int k=0; k<inCube.dimz; k++) {
        if (interleaved) {
           if (zfrom == 2) 
             outCube.SetValue(srcx,srcy,interleavedorder(srcz, outCube.dimz),inCube.GetValue(i,j,k));
           else if (yfrom == 2) 
             outCube.SetValue(srcx,interleavedorder(srcy, outCube.dimy),srcz,inCube.GetValue(i,j,k));
           else if (xfrom == 2) 
             outCube.SetValue(interleavedorder(srcx, outCube.dimx),srcy,srcz,inCube.GetValue(i,j,k));
        }
        else
	   outCube.SetValue(srcx,srcy,srcz,inCube.GetValue(i,j,k));
        if (xfrom == 2) srcx+=xincr;
        else if (yfrom == 2) srcy+=yincr;
        else if (zfrom == 2) srcz+=zincr;
      }
      if (xfrom == 1) srcx+=xincr;
      else if (yfrom == 1) srcy+=yincr;
      else if (zfrom == 1) srcz+=zincr;
    }
    if (xfrom == 0) srcx+=xincr;
    else if (yfrom == 0) srcy+=yincr;
    else if (zfrom == 0) srcz+=zincr;
  }
...
int
returnReverseOrientation(string &s) {
...
VBMatrix
affine_pitch(VBMatrix &m,double pitch)
{
  VB_Vector row(4);
  VBMatrix out(4,4);
  VBMatrix tr(4,4);
...
VBMatrix
affine_roll(VBMatrix &m,double roll)
{
  VB_Vector row(4);
  VBMatrix out(4,4);
  VBMatrix tr(4,4);
...
VBMatrix
affine_yaw(VBMatrix &m,double yaw)
{
  VB_Vector row(4);
  VBMatrix out(4,4);
  VBMatrix tr(4,4);
...
VBMatrix
affine_translate(VBMatrix &m,double dx,double dy,double dz)
{
  VB_Vector row(4);
  VBMatrix out(4,4);
  VBMatrix tr(4,4);
...
void
createresampledvolume(Cube &dest,Cube &src)
{
  double q_mag=1.0;
  // get corners
  ...
  for (int i=0; i<newvolume.dimx; i++) {
    ...
    for (int j=0; j<newvolume.dimy; j++) {
      ...
      for (int k=0; k<newvolume.dimz; k++) {
        newvolume.SetValue(i,j,k,src.GetValue((int)round(xx),(int)round(yy),(int)round(zz)));
        zz+=iz;
      }
      yy+=iy;
    }
...
// THE BELOW IS USED TO DO PSEUDO-GAUSSIAN SMOOTHING (DOESN'T INCLUDE EXTRA-MASK VOXELS)

// s0/s1/s2 are the fwhm kernel in voxels

int
buildGaussianKernel(Cube &cube,double s0,double s1,double s2)
{
  short x1,y1,z1;
...
int
maskKernel(Cube &kernel,Cube &mask,int x,int y,int z)
{
  ...
  if (kernelsum>0.0)
    kernel*=((double)1.0/kernelsum);
  return 0;
}

int
smooth3D(Cube &cube,Cube &mask,Cube &rawkernel)
{
...
double
getKernelAverage(Cube &cube,Cube &kernel,int x,int y,int z)
{
  int cx=kernel.dimx/2;
  int cy=kernel.dimy/2;
  int cz=kernel.dimz/2;
...
void
Resample::SetXX(double xx1,double xxstep,int nxx)
{
  x1=xx1; xstep=xxstep; nx=nxx;
}

void
Resample::SetYY(double yy1,double yystep,int nyy)
{
  y1=yy1; ystep=yystep; ny=nyy;
}

void
Resample::SetZZ(double zz1,double zzstep,int nzz)
{
  z1=zz1; zstep=zzstep; nz=nzz;
}
...
int
Resample::UseZ(Cube &cb,Cube &refcube,double zsize)
{
...
int
Resample::UseDims(const Cube &cb,const Cube &refcube)
{
  nx=refcube.dimx;
  ny=refcube.dimy;
  nz=refcube.dimz;
...
int
Resample::UseSpecifiedDims(const Cube &cb,int dimx,int dimy,int dimz)
{
  nx=dimx;
  ny=dimy;
  nz=dimz;
...
int
Resample::UseTLHC(const Cube &cb,const Cube &refcube)
{
...
int
Resample::UseCorner(const Cube &cb,const Cube &refcube)
{
  stringstream tmps;
...
int
Resample::UseCorner2(const Cube &cb,const Cube &refcube)
{
  stringstream tmps;
  // first get the corner coordinates of both images
  tokenlist ourline,refline;
...
void
Resample::SetupDims(Cube &cb,const Cube &ref)
{
  x1=y1=z1=0;
  nx=ref.dimx;
  ny=ref.dimy;
...
int
Resample::NNResampleCube(const Cube &mycube,Cube &newcube)
{
  int i,j,k;
...
int
Resample::SincResampleCube(const Cube &mycube,Cube &newcube)
{
  int i,j,k;
...
void
zero_smallregions(Cube &cb,double thresh)
{
...
void
rotatecube(Cube &cb,float pitch,float roll,float yaw)
{
...
//         oldcoord.set(2,0,(double)k);
        ...
        oldcoord2=transform;
        oldcoord2*=oldcoord;
        c1(0)=oldcoord2(0,0)+1+cb.origin[0];   // +1 because resample_sinc expects 1-indexed coords
        ...
        switch (mycube.datatype) {
        case vb_byte:
          resample_sinc(1,(unsigned char *)cb.data,out,c1,c2,c3,
                        cb.dimx,cb.dimy,cb.dimz,5,
                        0.0,1.0);
          break;
        case vb_short:
          resample_sinc(1,(int16 *)cb.data,out,c1,c2,c3,
                        cb.dimx,cb.dimy,cb.dimz,5,
                        0.0,1.0);
          break;
        case vb_long:
          resample_sinc(1,(int32 *)cb.data,out,c1,c2,c3,
                        cb.dimx,cb.dimy,cb.dimz,5,
                        0.0,1.0);
          break;
        case vb_float:
          resample_sinc(1,(float *)cb.data,out,c1,c2,c3,
                        cb.dimx,cb.dimy,cb.dimz,5,
                        0.0,1.0);
          break;
        case vb_double:
          resample_sinc(1,(double *)cb.data,out,c1,c2,c3,
                        cb.dimx,cb.dimy,cb.dimz,5,
                        0.0,1.0);
          break;
        }
...
void
enlarge(Cube &cb,uint32 radius,uint32 xradius,uint32 yradius,uint32 zradius)
{
  ...
  for (int v=0; v<cb.dimx*cb.dimy*cb.dimz; v++) {
    ...
    x1=xx-xradius; if (x1<0) x1=0;
    y1=yy-yradius; if (y1<0) y1=0;
    z1=zz-zradius; if (z1<0) z1=1;
    x2=xx+xradius; if (x2>cb.dimx-1) x2=cb.dimz-1;
    y2=yy+yradius; if (y2>cb.dimy-1) y2=cb.dimy-1;
    z2=zz+zradius; if (z2>cb.dimz-1) z2=cb.dimz-1;
    for (int i=x1; i<=x2; i++) {
      for (int j=y1; j<=y2; j++) {
        for (int k=z1; k<=z2; k++) {
  ...
  vbforeach(VBVoxel &vv,hitlist)
    cb.setValue(vv.x,vv.y,vv.z,1.0);
}
