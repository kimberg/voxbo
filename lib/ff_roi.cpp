...
vf_status test_roi_3D(unsigned char *, int, string filename) {
  if (filename.find(".roi") != string::npos) return vf_yes;
  return vf_no;
}

int read_head_roi_3D(Cube *cb) {
  // if we already have dims, great!
  if (cb->dimx > 0 && cb->dimy > 0 && cb->dimz > 0) return 0;
...
int read_data_roi_3D(Cube *cb) {
  ...
  while (1) {
    ...
    if (my_endian() != ENDIAN_LITTLE) swap(shdr, 2);
    ...
    for (int i = 0; i < (shdr[1] - 2) / 2; i++) {
      ...
      if (my_endian() != ENDIAN_LITTLE) swap(sdat, 2);
      sdat[0]--;
      xx = sdat[0] % cb->dimx;
      yy = sdat[0] / cb->dimx;
