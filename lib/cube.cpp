...
Cube::init()
{
  ...
  minval=1; maxval=0;  // invalid values, forces min and max to be calculated
}

int
Cube::init(int in_dimx,int in_dimy,int in_dimz,VB_datatype in_type)
{
  init();
...
int
Cube::SetVolume(const Cube &cb)
{
  return SetVolume(cb.dimx,cb.dimy,cb.dimz,cb.datatype);
}

int
Cube::SetVolume(const Cube &cb,VB_datatype dt)
{
  return SetVolume(cb.dimx,cb.dimy,cb.dimz,dt);
}

int
Cube::SetVolume(uint32 in_dimx,uint32 in_dimy,uint32 in_dimz,VB_datatype in_type)
{
  if (data && !f_mirrored)
    delete [] data;
...
int
Cube::ReadFile(const string &fname)
{
  int err;
...
int
Cube::ReadHeader(const string &fname)
{
  if (fname.size()==0)
    return 104;
  ...
  if (subvolume>-1) {
    ...
    if (ftypes.size()==0)
      return 101;
    fileformat=ftypes[0];
    if (!fileformat.read_head_4D || !fileformat.read_vol_4D)
      return 102;
  ...
  fileformat=ftypes[0];
  if (!fileformat.read_head_3D)
    return 102;
  int err=fileformat.read_head_3D(this);
  if (!err)
    ReadLabels();   // ignore errors here
  return err;
}

int
Cube::ReadData(const string &fname)
{
  filename=fname;
  data_valid=0;
  int err;
...
int
Cube::WriteFile(const string fname)
{
  VBFF original;
  ...
  if (!fileformat.write_3D)
    fileformat=findFileFormat(filename,3);
  // if not, try original file's format
  if (!fileformat.write_3D)
    fileformat=original;
  // if not, try cub1
  if (!fileformat.write_3D)
    fileformat=findFileFormat("cub1");
...
ostream&
operator<<(ostream &os,const Cube &cb)
{
  os << endl << "+- 3D Image file " << xfilename(cb.GetFileName()) << " ("
  ...
  os << "| " << cb.dimx << "x" << cb.dimy << "x" << cb.dimz << " voxels" << endl;
  os.setf(ios::fixed,ios::floatfield);
  os.precision(4);
  os << "| " << cb.voxsize[0] << " x " << cb.voxsize[1] << " x " << cb.voxsize[2] << " mm" << endl;
  os.precision(1);
  os << "| " << cb.meglen() << "MB on disk (" <<
    (cb.filebyteorder == ENDIAN_BIG ? "msbfirst" : "lsbfirst") << ")" << endl;
  os << "| origin: (" << cb.origin[0] << "," << cb.origin[1] << "," << cb.origin[2] << ")" << endl;
  os.precision(2);
  if (cb.voxsize[0]>FLT_MIN && cb.voxsize[1]>FLT_MIN && cb.voxsize[1]>FLT_MIN) {
    string tmp=(format("[%g,%g,%g;%g,%g,%g]")
                %(cb.voxsize[0]*cb.origin[0])
                %(cb.voxsize[1]*cb.origin[1])
                %(cb.voxsize[2]*cb.origin[2])
  ...
  os.precision(4);
  if (cb.f_scaled) os << "| slope: " << cb.scl_slope << "," << "intercept: " << cb.scl_inter << endl;
  if (cb.header.size() > 0) {
    os << "+--user header----------" << endl;
...
void
Cube::printbrief(const string &flags) const
{
...
double
Cube::GetValue(VBVoxel &v) const
{
  return GetValue(v.x,v.y,v.z);
}

double
Cube::GetValue(int x,int y,int z) const
{
  switch (datatype) {
  case vb_byte:
...
bool
Cube::testValue(int x,int y,int z) const
{
  switch (datatype) {
  case vb_byte:
...
bool
Cube::testValue(int index) const
{
  switch (datatype) {
  case vb_byte:
...
void
Cube::SetValue(int x,int y,int z,double val)
{
  if (x<0 || y<0 || z<0)
    return;
...
Cube &
Cube::copycube(const Cube &cb,bool mirrorflag)
{
  init();
  if (!cb.header_valid)
    return *this;
  ...
  dimy=cb.dimy;
  dimz=cb.dimz;
  dimt=cb.dimt;   // not needed for cubes, but we might sometimes stash something here
...
int
operator==(const Cube &c1,const Cube &c2)
{
  int differences=0;
...
{
  if (header_valid)
    return (float)(voxels*datasize)/(1024*1024);
  return 0;
}
...
void
Cube::zero(int x1,int x2,int y1,int y2,int z1,int z2)
{
  if (dimx<1) return;
  if (dimy<1) return;
  if (dimz<1) return;
...
Cube::leftify()
{
  zero(((dimx+1)/2),dimx-1,0,0,0,0);
}

void
Cube::rightify()
{
  zero(0,((dimx+1)/2)-1,0,0,0,0);
}

void
Cube::thresh(double val)
{
  double thisval;
  for (int i=0; i<dimx; i++) {
    for (int j=0; j<dimy; j++) {
...
int
Cube::is_surface(int x,int y,int z)
{
  if (x==0 || y==0 || z==0)
    return 1;
  if (x==dimx-1 || y==dimy-1 || z==dimz-1)
...
Cube::header2string()
{
  ...
  vbforeach(pp,maskspecs)
    hdr+=(format("vb_maskspec: %d %d %d %d %s\n")%pp.first%pp.second.r%pp.second.g
          %pp.second.b%pp.second.name).str();
...
Cube::string2header(string &hdr)
{
  ...
  for (size_t i=0; i<wholeline.size(); i++) {
    ...
    if (args[0]=="VoxDims(XYZ):" && args.size()>3) {
      ...
      addMaskSpec(strtol(args[1]),strtol(args[2]),strtol(args[3]),strtol(args[4]),args[5]);
    }
    else {
...
int
Cube::convert_type(VB_datatype newtype,uint16 flags)
{
  if (!data)
    return 100;
  if (datatype!=newtype) {
