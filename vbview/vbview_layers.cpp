...
VBLayer::tooltipinfo()
{
  ...
  if (mask) fstring+="\n(currently masked)";
  format ff(fstring);
  QString tmp=(ff%xfilename(filename)
               %cube.dimx%cube.dimy%cube.dimz
               %cube.voxsize[0]%cube.voxsize[1]%cube.voxsize[2]).str().c_str();
  return tmp;
}
...
void
VBLayer::setAlignment(VBLayerI base)
{
  VBImage *im;
  if (tes)
    im=&tes;
  ...
  if (base->dimx==im->dimx && base->dimy==im->dimy && base->dimz==im->dimz) {
...
inline int32
scaledvalue(float val,float low,float,float factor)
{
  ...
  if (q_clustersize>1) {
    ...
    vbforeach(VBRegion &rr,regions) {
      ...
      for (VI myvox=rr.begin(); myvox!=rr.end(); myvox++) {
        rendercube.setValue(myvox->second.x,myvox->second.y,myvox->second.z,1);
      }
    }
  }
  for (int i=0; i<cube.dimx*cube.dimy*cube.dimz; i++) {
    ...
    if (q_clustersize>1 && rendercube.getValue<int32>(i)==0)
      continue;
    aval=fabs(val);
...
uint32
VBLayer::overlayvalue(double val,double aval)
{
  ...
  if (aval-q_thresh>FLT_MIN) {
    ...
    int rr,gg,bb;
    if (pct>1.0)pct=1.0;
    if (val>(0-FLT_MIN)) {    // zeros (or neg values < epsilon) are always positive
      rr=q_poscolor1.red();
...
VBMatrix
VBLayer::calcFullTransform(float xscale,float yscale,float zscale,
