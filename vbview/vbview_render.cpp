...
void VBView::RenderAll() {
  if (!q_update) return;
  for (int i = 0; i < (int)viewlist.size(); i++) RenderSingle(viewlist[i]);
  drawarea->update();
}

void VBView::RenderSingle(MyView &view) {
  if (!q_update) return;
...
void VBView::UniRender(vbrect rrect) {
  // iterate through views, find ones whose
  for (int i = 0; i < (int)viewlist.size(); i++) {
    vbrect vrect(viewlist[i].xoff, viewlist[i].yoff, viewlist[i].width,
                 viewlist[i].height);
    vrect = vrect & rrect;  // intersection of view rect and render rect
    if (vrect) RenderSingle(viewlist[i]);
  }
}

void VBView::UniRender(MyView &view) {
  UniRenderView(view);
  return;
  if (!q_update) return;
...
void VBView::UniRenderView(MyView &view) {
  if (!q_update) return;
  if (!layers.size()) return;
  for (VBLayerI l = layers.begin(); l != layers.end(); l++) {
    if (l == layers.begin() || (l->q_visible && l->alpha > 0))
      UniRenderLayer(view, l,
                     vbrect(view.xoff, view.yoff, view.width, view.height));
  }
}

void VBView::UniRenderLayer(MyView &view, VBLayerI layer, vbrect rrect) {
  if (!q_update) return;
  ...
  layer->calcFullTransform(q_xscale, q_yscale, q_zscale, view.xoff, view.yoff,
                           view.width, view.height, view.orient, pos, q_fliph,
                           q_flipv);
  ...
  for (int j = rrect.y; j < rrect.y + rrect.h; j++) {
    ...
    for (int i = rrect.x; i < rrect.x + rrect.w; i++) {
      ...
      if (q_showorigin && xx == layer->rendercube.origin[0] &&
          yy == layer->rendercube.origin[1] &&
  ...
  if (myorigin.size()) {
    ...
    for (int j = y1; j <= yn; j++) {
      ...
      for (int i = x1; i <= xn; i++) {
        double dist =
            sqrt((double)((i - x) * (i - x)) + (double)((j - y) * (j - y)));
        if (dist > 9.5 && dist < 10.5) *p = qRgb(255, 255, 0);
        p++;
      }
    }
  }
...
inline QRgb alphablend(QRgb oldval, QRgb newval, int alpha) {
  ...
  int newfact = alpha;
  int rr = (((oldval >> 16) & 0xff) * oldfact / 100) +
           (((newval >> 16) & 0xff) * newfact / 100);
  int gg = (((oldval >> 8) & 0xff) * oldfact / 100) +
           (((newval >> 8) & 0xff) * newfact / 100);
  int bb =
      (((oldval)&0xff) * oldfact / 100) + (((newval)&0xff) * newfact / 100);
...
int32 VBView::fillmask(MyView &view, VBVoxel loc, VBLayerI masklayer,
                       uint32 maskcolor) {
  // calculate the transformation from window coordinates to this
  // layer's rendercube coordinates
  int pos = view.position;
  ...
  masklayer->calcFullTransform(q_xscale, q_yscale, q_zscale, view.xoff,
                               view.yoff, view.width, view.height, view.orient,
                               pos, q_fliph, q_flipv);
  ...
  for (VI mv = maskreg.begin(); mv != maskreg.end(); mv++) {
    ...
    if (!(masklayer->undo.front().contains(mv->second.x, mv->second.y,
                                           mv->second.z)))
      masklayer->undo.front().add(
          mv->second.x, mv->second.y, mv->second.z,
          masklayer->cube.GetValue(mv->second.x, mv->second.y, mv->second.z));
    // now write the new value
    masklayer->cube.SetValue(mv->second.x, mv->second.y, mv->second.z,
                             (maskcolor ? q_maskindex : 0));
    masklayer->rendercube.SetValue(mv->second.x, mv->second.y, mv->second.z,
                                   maskcolor);
  }
...
int32 VBView::colormask(MyView &view, VBVoxel loc, VBLayerI masklayer,
                        uint32 maskcolor) {
  // calculate the transformation from window coordinates to this
  // layer's rendercube coordinates
  int pos = view.position;
  ...
  for (int i = xx - q_radius; i <= xx + q_radius; i++) {
    ...
    for (int j = yy - q_radius; j <= yy + q_radius; j++) {
      ...
      for (int k = zz - zr; k <= zz + zr; k++) {
        if (i < 0 || i > masklayer->cube.dimx - 1) continue;
        dist = sqrt(((xx - i) * (xx - i)) + ((yy - j) * (yy - j)) +
                    ((zz - k) * (zz - k)));
        if (dist <= q_radius) maskreg.add(i, j, k);
      }
    }
  }
  for (VI mv = maskreg.begin(); mv != maskreg.end(); mv++) {
...
int32 VBView::colorpixels(MyView &view, VBVoxel cvox, VBRegion &maskreg,
                          VBLayerI masklayer) {
  // calculate the transformation from window coordinates to this
  // layer's rendercube coordinates
  int pos = view.position;
  ...
  int xpos, ypos, zpos;
  VBVoxel vv;
  int32 xmin = position(0, 0), xmax = position(0, 0), ymin = position(1, 0),
        ymax = position(1, 0);
  while (queue.size()) {
    // pop the first voxel
    ...
    position.set(3, 0, 1);
    position ^= full;  // premultiply to get layer coordinate
    xpos = (int)position(0, 0);
    ypos = (int)position(1, 0);
    zpos = (int)position(2, 0);
