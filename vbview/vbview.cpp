...
enum {C_VIS=0,C_NAME=1,C_TYPE=2,C_X=3,C_Y=4,C_Z=5,C_VAL=6,C_LABEL=7};
  ...
  : QWidget(parent,name)
{
  ...
  // things that are in the top layout: image scrollview, button grid layout, control widget
  ...
  // things that are in the control widget: layer table and numerous context-sensitive widgets
  ...
  QObject::connect(layertable,SIGNAL(cellClicked(int,int)),this,SLOT(layertableclicked(int,int)));
  QObject::connect(layertable,SIGNAL(cellDoubleClicked(int,int)),this,SLOT(layertabledclicked(int,int)));
  QObject::connect(layertable,SIGNAL(customContextMenuRequested(QPoint)),this,SLOT(layertablerightclicked(QPoint)));
  ...
  colbox->hide();
  QObject::connect(tmps,SIGNAL(valueChanged(int)),this,SLOT(SetColumns(int)));
  ...
  // layertable->setStyle("selection-background-color:qlineargradient(x1: 0, y1: 0, x2: 0.5, y2: 0.5, stop: 0 #FF92BB, stop: 1 white)");
  layertable->setColumnCount(8);
  layertable->setSizePolicy(QSizePolicy::MinimumExpanding,QSizePolicy::Minimum);
  ...
  QObject::connect(tsliceslider,SIGNAL(valueChanged(int)),this,SLOT(NewTSlice(int)));
  ...
  cbox->addWidget(tmph);
  QObject::connect(q_trans,SIGNAL(valueChanged(int)),this,SLOT(newtrans(int)));
  ...
  QObject::connect(maskview,SIGNAL(changed(QWidget *)),this,SLOT(handle_maskchanged(QWidget *)));
  QObject::connect(maskview,SIGNAL(selected(int)),this,SLOT(handle_maskselected(int)));
  QObject::connect(maskview,SIGNAL(copied(int)),this,SLOT(handle_maskcopied(int)));
  QObject::connect(maskview,SIGNAL(inforequested(int)),this,SLOT(handle_inforequested(int)));
  QObject::connect(maskview,SIGNAL(savemask()),this,SLOT(handle_savemask()));
  QObject::connect(maskview,SIGNAL(newmask(QTMaskWidget *)),this,SLOT(handle_newmask(QTMaskWidget *)));
  QObject::connect(maskview,SIGNAL(newradius(int)),this,SLOT(NewRadius(int)));
  QObject::connect(maskview,SIGNAL(newzradius(int)),this,SLOT(NewZRadius(int)));
  ...
  QObject::connect(lowedit,SIGNAL(textEdited(const QString&)),this,SLOT(HighlightLow()));
  QObject::connect(lowedit,SIGNAL(returnPressed()),this,SLOT(SetLow()));
  QObject::connect(highedit,SIGNAL(textEdited(const QString&)),this,SLOT(HighlightHigh()));
  QObject::connect(highedit,SIGNAL(returnPressed()),this,SLOT(SetHigh()));
  QObject::connect(clusteredit,SIGNAL(textEdited(const QString&)),this,SLOT(HighlightCluster()));
  ...
  q_scale->setscale(3.5,5,QColor(0,0,200),QColor(100,100,255),QColor(255,0,0),QColor(255,255,0));
  ...
  q_scale2->hide();
  QObject::connect(q_scale2,SIGNAL(newcolors(QColor,QColor,QColor,QColor)),this,SLOT(newcolors2(QColor,QColor,QColor,QColor)));
  ...
  QObject::connect(tslist,SIGNAL(itemSelectionChanged()),this,SLOT(UpdateTS()));
  ...
  ts_maskbox->setToolTip("graph the average of the current layer's visible region (if un-checked, you see the same voxel displayed in the layer table");
...
void
VBView::newcolors2(QColor,QColor,QColor p1,QColor p2)
{
  if (currentlayer==layers.end()) return;  // actually shouldn't happen
...
void
VBView::layertableclicked(int row,int col)
{
  ...
  if (col==C_VIS) {
    ...
    if (li->q_visible) {
      ...
      li->q_visible=1;
      QTableWidgetItem *twi=new QTableWidgetItem(QIcon(QPixmap(":/icons/visible.png")),"");
      twi->setFlags(Qt::ItemIsEnabled|Qt::ItemIsSelectable);
      layertable->setItem(row,col,twi);
    }
    RenderAll();
...
void
VBView::layertabledclicked(int row,int)
{
  // which layer did we click on?
  VBLayerI li=layers.begin();
  int rowx=row;
...
void
VBView::updatewidgets()
{
  // FIXME update menu items too, like graying out mask save when a
  // mask layer isn't selected

  // stat-only stuff
  if (currentlayer->type==VBLayer::vb_stat || currentlayer->type==VBLayer::vb_corr || currentlayer->type==VBLayer::vb_glm) {
    statbox->show();
    q_scale->setscale(currentlayer->q_thresh,currentlayer->q_high,currentlayer->q_negcolor1,currentlayer->q_negcolor2,currentlayer->q_poscolor1,currentlayer->q_poscolor2);
    q_scale2->setscale(0,currentlayer->q_thresh,currentlayer->q_nscolor1,currentlayer->q_nscolor2);
  ...
  if (currentlayer->type==VBLayer::vb_mask) {
    if (currentlayer!=lastmasklayer) {
      ...
      vbforeach(mm,currentlayer->cube.maskspecs) {
        maskview->addMask(mm.first,mm.second.name,QColor(mm.second.r,mm.second.g,mm.second.b));
      }
...
void
VBView::layertablerightclicked(QPoint p)
{
  ...
  if (currentlayer->type==VBLayer::vb_stat) mm.addAction("sign flip layer",this,SLOT(fliplayer()));
  if (currentlayer->type==VBLayer::vb_corr) mm.addAction("sign flip layer",this,SLOT(fliplayer()));
...
VBView::GoToOrigin()
{
  ...
  if (xx<0 || xx>ll->cube.dimx-1)
    return;
  if (yy<0 || yy>ll->cube.dimy-1)
    return;
  if (zz<0 || zz>ll->cube.dimz-1)
    return;
...
VBView::LayerInfo()
{
  ...
  for (size_t i=0; i<rlist.size(); i++) {
    ...
    regioninfo+=(format("region %02d: count: %d\n")%i%(rlist[i].size())).str();
    if (vfactor>FLT_MIN)
    ...
    regioninfo+=(format("note: peakcnt (or lowcnt) is the total number of voxels in\n")).str();
    regioninfo+=(format("the region with the same peak (or min) value\n")).str();
    regioninfo+="\n";
  }
...
int
VBView::NewTSlice(int newval)
{
  if (layers.size()==0) return 1;
  if (!(layers.begin()->tes.data))
    return 2;
  int nv=newval;
...
VBView::SavePNG()
{
  QString s=Q3FileDialog::getSaveFileName(".","All (*.*)",this,"save image file","Choose a filename for your snapshot");
...
int
VBView::ShowCrosshairs(bool state)
{
  if (state)
    q_crosshairs=1;
  else
...
{
  if (layers.size()==0) return 1;
  if (layers.front().cube.voxsize[0]<FLT_MIN || layers.front().cube.voxsize[1]<FLT_MIN || layers.front().cube.voxsize[2]<FLT_MIN)
    return 0;
  q_isotropic=state;
  UpdateScaling();
  return 0;
...
VBView::UpdateScaling()
{
  ...
  if (q_isotropic) {
    ...
    if (layers.front().cube.voxsize[1]<minsize) minsize=layers.front().cube.voxsize[1];
    if (layers.front().cube.voxsize[2]<minsize) minsize=layers.front().cube.voxsize[2];
...
VBView::SetTails(bool twotailed)
{
  ...
  if (twotailed)
    ...
    q_scale->setscale(currentlayer->q_thresh,currentlayer->q_high,currentlayer->q_poscolor1,currentlayer->q_poscolor2);
  currentlayer->render();
  RenderAll();
  UpdateTS();
}
...
VBView::GetThreshold()
{
  ...
  if (ret==QDialog::Accepted && finite(tc.getbestthresh())) {
    ...
    if (v.denomdf)
      tt+=(format("F(%g,%g), %d comparisons")%v.effdf%v.denomdf%v.numVoxels).str();
    else
...
VBView::SaveFullMap()
{
  if (currentlayer->type!=VBLayer::vb_glm && currentlayer->type!=VBLayer::vb_stat)
    return 0;
  VBLayerI ll=currentlayer;  // convenience;
  int err;
  QString s=Q3FileDialog::getSaveFileName("Filename for map","All (*.*)",this,"save map","Map filename");
  if (s==QString::null) return 0;
...
VBView::reload_averages()
{
  ...
  if (li==layers.end() || li->type!=VBLayer::vb_glm)
    li=firstStatLayer();
  if (li==layers.end())
    return;
...
VBView::update_status()
{
  ...
  char tmp[STRINGLEN];
  if (base_dims==4)
    sprintf(tmp,"%d x %d x %d, %d time points",base_dimx,base_dimy,base_dimz,base_dimt);
  else if (base_dims==3)
  ...
  if (ll!=layers.end()) {
    ...
    if (ll->glmi.teslist.size()) {
      if (ll->glmi.glmflags & DETREND)
        statstring+=" DETRENDED";
      if (ll->glmi.glmflags & MEANSCALE)
...
void
VBView::handle_newmask(QTMaskWidget *mw)
{
  VBMaskSpec ms;
  ms.r=mw->color.red();
  ms.g=mw->color.green();
...
void
VBView::handle_maskselected(int i)
{
...
void
VBView::handle_maskcopied(int ind)
{
...
void
VBView::handle_inforequested(int i)
{
  return;
  VBMaskSpec &ms=lastmasklayer->cube.maskspecs[i];
  cout << ms.name << endl;
...
VBView::ByteSwap()
{
  if (currentlayer==layers.end())
    return;
  if (currentlayer->cube)
    currentlayer->cube.byteswap();
  if (currentlayer->tes)
    currentlayer->tes.byteswap();
  currentlayer->render();
  RenderAll();
}
...
{
  QString s;
  s=Q3FileDialog::getOpenFileName(q_currentdir.c_str(),"All (*.*)",this,"open averages file","Choose the file that contains your averages");
...
VBView::handle_savemask()
{
  ...
  QString s=Q3FileDialog::getSaveFileName(".","All (*.*)",this,"save mask","Choose a filename for your mask");
  if (s==QString::null) return;
...
//   if (!q_filename.size()) {
//     QString s=Q3FileDialog::getSaveFileName(".","All (*.*)",this,"save image","Choose a filename for your image");
//     if (s==QString::null) return (0);
...
VBView::Paste()
{
  ...
  if (!q_mask.dimsequal(rc)) {
    QMessageBox::warning(this,"Dimension mis-match","The mask you copied and your current layer have different dimensions.","Okay");
    return 1;
  }
  rm=q_mask;
  for (int i=0; i<rc.dimx*rc.dimy*rc.dimz; i++) {
...
VBView::ApplyMask()
{
  ...
  if (previouslayer==currentlayer || previouslayer==layers.end())
    return 1;
...
VBView::FindCurrentRegion()
{
  ...
  if (currentlayer->type==VBLayer::vb_mask) {
    ...
    VBRegion rr=growregion(xx,yy,zz,currentlayer->rendercube,mask,vb_eq,val);
  ...
  if (currentlayer->type==VBLayer::vb_glm || currentlayer->type==VBLayer::vb_stat) {
    ...
    VBRegion rr=growregion((int32)coord(0,0),(int32)coord(1,0),(int32)coord(2,0),
...
VBView::CreateCorrelationMap()
{
  ...
  int32 sz=coord(2,0);
  // just in case...
  if (sx<0 || sx>cl->dimx-1 || sy<0 || sy>cl->dimy-1 || sz<0 || sz>cl->dimz-1)
    return;
...
void
VBView::wheelEvent(QWheelEvent *we)
{
...
VBView::keyPressEvent(QKeyEvent *ke)
{
  ...
  if (ke->ascii()=='a' && base_dims==4)
    ...
    ShowOrigin(q_crosshairs);  // so that if they were out of sync, now they're not
  }
...
VBView::PopMask()
{
  ...
  for (VI vox=r->begin(); vox!=r->end(); vox++) {
    // vox->second.print();
    currentlayer->cube.SetValue(vox->second.x,vox->second.y,vox->second.z,vox->second.val);
  }
  currentlayer->render();
...
VBView::MousePressEvent(QMouseEvent me)
{
  ...
  if (me.button()==Qt::LeftButton) {
    effectivemode=mode;
    if (mode==vbv_mask && me.buttons()&(Qt::ShiftButton|Qt::ControlButton))
  ...
  // MASK DRAWING
  if (me.button()&(Qt::LeftButton|Qt::RightButton) && q_drawmask && currentlayer->type==VBLayer::vb_mask) {
    ...
    colormask(*currentview,VBVoxel(me.x(),me.y(),0),currentlayer,(me.buttons()&Qt::LeftButton?q_maskcolor:0));
    return 0;
  }
  if (me.button()&(Qt::MidButton) && q_drawmask && currentlayer->type==VBLayer::vb_mask) {
    // we have initiated a mask draw, let's push back the undo stuff
    // and start a new undo region
    if (currentlayer->undo.size()>100) currentlayer->undo.pop_back();
    VBRegion rr;
    currentlayer->undo.push_front(rr);
    fillmask(*currentview,VBVoxel(me.x(),me.y(),0),currentlayer,q_maskcolor);
    return 0;
  }
...
VBView::MouseMoveEvent(QMouseEvent me)
{
  ...
  // MASK DRAWING
  if (me.buttons() & (Qt::LeftButton|Qt::RightButton) && q_drawmask && currentlayer->type==VBLayer::vb_mask) {
    colormask(*currentview,VBVoxel(me.x(),me.y(),0),currentlayer,(me.buttons()&Qt::LeftButton?q_maskcolor:0));
    return 0;
  }

  // FIXME otherwise, for now, when any mouse button is down, we're moving the cross
...
int
VBView::win2base(int x,int y,double &xx,double &yy,double &zz)
{
  ...
  for (view=viewlist.begin(); view!=viewlist.end(); view++) {
    if (x>=view->xoff && x<view->xoff+view->width &&
        y>=view->yoff && y<view->yoff+view->height)
      break;
  }
  currentview=view;  // for later use
  // not in any view?
  if (view==viewlist.end())
    return 101;
  VBMatrix coord(4,1);
  VBMatrix newcoord;
  ...
  newcoord=layers.begin()->calcFullTransform(q_xscale,q_yscale,q_zscale,
...
void
VBView::setAllLayerCoords(double x,double y,double z)
{
  int row=0;
    ...
    if (li->cube) {
      ...
      if (li->type==VBLayer::vb_mask)
        if (li->cube.maskspecs.count((uint32)val))
          layertable->item(row,C_LABEL)->setText(li->cube.maskspecs[(uint32)val].name.c_str());
    }
    row++;
  }
  UpdateTS();
...
void
VBView::FillPixel(int,int,int,int)
{
  // q_update=0;
...
void
VBView::FillPixelXY(int,int,int,int)
{
}

void
VBView::FillPixelYZ(int,int,int,int)
{
}

void
VBView::FillPixelXZ(int,int,int,int)
{
}
...
void
VBView::ToggleOverlays()
{
  if (currentlayer==layers.end() || previouslayer==layers.end())
    return;
  if (currentlayer==previouslayer)
    return;
  if (currentlayer==layers.begin() || previouslayer==layers.begin())
    return;
...
VBLayerI
VBView::firstStatLayer()
{
  VBLayerI ll;
  for (ll=layers.begin(); ll!=layers.end(); ll++)
    if (ll->type==VBLayer::vb_stat || ll->type==VBLayer::vb_glm)
      break;
  return ll;
}

VBLayerI 
VBView::firstMaskLayer()
{
  VBLayerI ll;
  for (ll=layers.begin(); ll!=layers.end(); ll++)
    if (ll->type==VBLayer::vb_mask)
      break;
  return ll;
}

VBLayerI 
VBView::firstCorrLayer()
{
  VBLayerI ll;
  for (ll=layers.begin(); ll!=layers.end(); ll++)
    if (ll->type==VBLayer::vb_corr)
      break;
  return ll;
}
