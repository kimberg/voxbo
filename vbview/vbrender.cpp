...
int main(int argc, char **argv) {
  ...
  if (vfbpid == 0) execlp("Xvfb", ":11", NULL);
  setenv("DISPLAY", ":11", 1);
...
int RenderMe::Go(tokenlist &args) {
  if (!args.size()) {
    help();
    return 102;
  }
  ...
  if (overlayname.size()) {
    ...
    if (!overlay.data_valid) {
      sprintf(tmp, "vbrender: statistical overlay file %s not valid",
              overlayname.c_str());
      printErrorMsg(VB_ERROR, tmp);
      return 102;
    }
  ...
  QImage im;
  if (vbr.RenderSlices()) return 103;
  if (vbr.SavePNG(outfilename)) return 103;
  return 0;
}
...
void RenderMe::help() {
  ...
  printf(
      "  -t <low> <high> below low not rendered, high and above get max "
      "value\n");
  printf("notes:\n");
  printf(
      "  if -o is specified, either -a or -d (preferably both) must also be "
      "specified\n");
  printf("\n");
}
