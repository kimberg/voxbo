...
VBQTnewmasklayer::VBQTnewmasklayer(QWidget *parent,int basex,int basey,int basez) : QDialog(parent)
{
  init(basex,basey,basez);
}

VBQTnewmasklayer::VBQTnewmasklayer(QWidget *parent,const Cube &cb) : QDialog(parent)
{
  init(cb.dimx,cb.dimy,cb.dimz);
}

void
VBQTnewmasklayer::init(int basex,int basey,int basez)
{
  // set<int> xs;
  // set<int> ys;
  // set<int> zs;
  ...
  QObject::connect(acceptbutton,SIGNAL(clicked()),this,SLOT(acceptclicked()));
...
void
VBQTnewmasklayer::acceptclicked()
{
  pair<bool,int32> val;
  val=strtolx(w_xv->text().toStdString());
  if (!val.first) retx=val.second; else return;
  val=strtolx(w_yv->text().toStdString());
  if (!val.first) rety=val.second; else return;
  val=strtolx(w_zv->text().toStdString());
  if (!val.first) retz=val.second; else return;
  accept();
}

void
VBQTnewmasklayer::cancelclicked()
{
  reject();
}

void
VBQTnewmasklayer::resizeEvent(QResizeEvent *re)
{
...
void
VBQTnewmasklayer::arrangeChildren()
{
  ...
  acceptbutton->setGeometry(hpos,vpos,acceptbutton->width(),acceptbutton->height());
  hpos+=10+acceptbutton->width();
  cancelbutton->setGeometry(hpos,vpos,cancelbutton->width(),cancelbutton->height());
}
