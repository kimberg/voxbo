...
VBScalewidget::VBScalewidget(QWidget *parent) : QFrame(parent) {
...
void VBScalewidget::paintEvent(QPaintEvent *) {
  QPainter pp(this);
  drawcolorbars(&pp, QRect(0, 0, width(), height()), 12);
}

void VBScalewidget::setscale(float low, float high, QColor negcolor1,
                             QColor negcolor2, QColor poscolor1,
                             QColor poscolor2) {
  q_low = low;
  q_high = high;
...
void VBScalewidget::setscale(float low, float high, QColor poscolor1,
                             QColor poscolor2) {
  q_low = low;
  q_high = high;
...
void VBScalewidget::drawContents(QPainter *paint) {
  drawcolorbars(paint, contentsRect(), 12);
}

void VBScalewidget::drawcolorbars(QPainter *paint, QRect myrect, int fontsize) {
  f_vertical = 0;
  ...
  if (f_vertical && f_neg) {
    ...
    posrect.setRect(xoff + SPACE, yoff + SPACE,
                    myrect.width() - SPACE - fontwidth, barsize);
    negrect.setRect(xoff + SPACE, yoff + SPACE + barsize + SPACE,
                    myrect.width() - SPACE - fontwidth, barsize);
  ...
  if (!f_vertical && f_neg) {
    ...
    negrect.setRect(xoff + SPACE, yoff + SPACE, barsize,
                    myrect.height() - SPACE - fontsize);
    posrect.setRect(xoff + SPACE + barsize + SPACE, yoff + SPACE, barsize,
                    myrect.height() - SPACE - fontsize);
  }
  if (!f_vertical && !f_neg) {
    int barsize = myrect.width() - SPACE - SPACE;
    posrect.setRect(xoff + SPACE, yoff + SPACE, barsize,
                    myrect.height() - SPACE - fontsize);
  }
  ...
  if (f_neg && f_vertical) {
    ...
    paint->drawText(negrect.right() + ISPACE, negrect.top(),
                    myrect.width() - negrect.right(), negrect.height(),
                    Qt::AlignLeft | Qt::AlignBottom, QString(tmp));
    sprintf(tmp, "%g", q_low * -1);
    paint->drawText(negrect.right() + ISPACE, negrect.top(),
                    myrect.width() - negrect.right(), negrect.height(),
                    Qt::AlignLeft | Qt::AlignTop, QString(tmp));
  } else if (f_neg && !f_vertical) {
    shaderect(paint, negrect, q_negcolor2, q_negcolor1);
    paint->setPen(QColor(0, 0, 0));
    sprintf(tmp, "%g", q_high * -1);
    paint->drawText(negrect.left(), negrect.bottom(), negrect.width(),
                    myrect.height() - negrect.bottom(),
                    Qt::AlignLeft | Qt::AlignTop, QString(tmp));
    sprintf(tmp, "%g", q_low * -1);
    paint->drawText(negrect.left(), negrect.bottom(), negrect.width(),
                    myrect.height() - negrect.bottom(),
                    Qt::AlignRight | Qt::AlignTop, QString(tmp));
  }
  ...
  if (f_vertical) {
    ...
    paint->setPen(QColor(0, 0, 0));
    sprintf(tmp, "%g", q_low);
    paint->drawText(posrect.right() + ISPACE, posrect.top(),
                    myrect.width() - posrect.right(), posrect.height(),
                    Qt::AlignLeft | Qt::AlignBottom, QString(tmp));
    ...
    paint->setPen(QColor(0, 0, 0));
    sprintf(tmp, "%g", q_low);
    paint->drawText(posrect.left(), posrect.bottom(), posrect.width(),
                    myrect.height() - posrect.bottom(),
                    Qt::AlignLeft | Qt::AlignTop, QString(tmp));
    sprintf(tmp, "%g", q_high);
    paint->drawText(posrect.left(), posrect.bottom(), posrect.width(),
                    myrect.height() - posrect.bottom(),
...
void VBScalewidget::shaderect(QPainter *paint, QRect &r, QColor &c1,
                              QColor &c2) {
  int rr, gg, bb;
  float pct;
...
void VBScalewidget::mousePressEvent(QMouseEvent *me) {
...
void VBScalewidget::mouseDoubleClickEvent(QMouseEvent *me) {
  if (negrect.contains(me->x(), me->y())) {
    if ((f_vertical &&
         me->y() < negrect.top() + (negrect.bottom() - negrect.top()) / 2) ||
        (!f_vertical &&
         me->x() < negrect.left() + (negrect.right() - negrect.left()) / 2)) {
    ...
    if ((f_vertical &&
         me->y() > posrect.top() + (posrect.bottom() - posrect.top()) / 2) ||
        (!f_vertical &&
         me->x() < posrect.left() + (posrect.right() - posrect.left()) / 2)) {
      // p1
...
void VBScalewidget::contextmenu() {
  QMenu foo(this);
  ...
  if (ret == copy1 || ret == copy2 || ret == copy3) {
    ...
    QString s =
        QFileDialog::getSaveFileName(".", "All (*.*)", this, "save image file",
                                     "Choose a filename for the scale widget");
    if (s != QString::null) pm.save(s.latin1(), "PNG");
    ;
  }
}
