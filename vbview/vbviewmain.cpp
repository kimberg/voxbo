...
VBViewMain::VBViewMain(QWidget *parent, const char *) : Q3VBox(parent) {
  ...
  if (volmap.size()) {
    ...
    vbforeach(tt, volmap) {
      ...
      if (tt.second.GetHeader("maptype") == "mask")
        ac = tmenu->addAction(
            QString("Open ") + QString(xfilename(tt.second.id3).c_str()),
            maskmapper, SLOT(map()), 0);
      else
        ac = tmenu->addAction(
            QString("Open ") + QString(xfilename(tt.second.id3).c_str()),
            templatemapper, SLOT(map()), 0);
      if (tt.first < 10) {
      ...
      if (tt.second.GetHeader("maptype") == "mask")
        ...
        templatemapper->setMapping(ac,
                                   QString(tt.second.GetFileName().c_str()));
    }
    QObject::connect(templatemapper, SIGNAL(mapped(const QString &)), this,
                     SLOT(OpenTemplate(const QString &)));
    QObject::connect(maskmapper, SIGNAL(mapped(const QString &)), this,
                     SLOT(OpenMask(const QString &)));
  }
  ...
  filemenu->addAction("Save &As", this, SLOT(SaveImage()),
                      Qt::CTRL + Qt::Key_A);
  ...
  // viewmenu->addAction("Show/Hide Display
  // Panel",this,SLOT(ToggleDisplayPanel()),Qt::Key_F5);
  // viewmenu->addAction("Show/Hide Mask
  // Panel",this,SLOT(ToggleMaskPanel()),Qt::Key_F6);
  // viewmenu->addAction("Show/Hide Stat
  // Panel",this,SLOT(ToggleStatPanel()),Qt::Key_F7);
  viewmenu->addAction("Toggle between overlays", this, SLOT(ToggleOverlays()),
                      Qt::CTRL + Qt::Key_L);
  viewmenu->addAction("Show/Hide TimeSeries Panel", this, SLOT(ToggleTSPanel()),
                      Qt::Key_F8);
  viewmenu->addAction("Show/Hide Header Info", this, SLOT(ToggleHeaderPanel()),
                      Qt::Key_F9);
  viewmenu->addAction("Layer Info", this, SLOT(LayerInfo()),
                      Qt::CTRL + Qt::Key_I);
...
void VBViewMain::LoadMaps() {
  ...
  for (size_t i = 0; i < config.size(); i++) {
    ...
    if (args[0] == "path") {
      ...
      if (vg.size() == 0) vg.load(vbp.rootdir + "/elements/templates/*");
...
VBView *VBViewMain::AddView() {
  VBView *qtv = new VBView(imagetab);
  if (!qtv) return qtv;
  imagetab->addTab(qtv, "<no image>");
  imagetab->setCurrentPage(imagetab->count() - 1);
  QObject::connect(qtv, SIGNAL(closeme(QWidget *)), this,
                   SLOT(ClosePanel(QWidget *)));
  QObject::connect(qtv, SIGNAL(renameme(QWidget *, string)), this,
                   SLOT(RenamePanel(QWidget *, string)));
  return (qtv);  // no error!
}
...
//   imagetab->setCurrentPage(imagetab->count()-1);
//   QObject::connect(qtv,SIGNAL(closeme(QWidget
//   *)),this,SLOT(ClosePanel(QWidget *)));
//   QObject::connect(qtv,SIGNAL(renameme(QWidget
//   *,string)),this,SLOT(RenamePanel(QWidget *,string))); return;
// }

void VBViewMain::NewOutline() {
  if ((VBView *)imagetab->currentPage())
    ((VBView *)imagetab->currentPage())->NewOutline();
}

void VBViewMain::ByteSwap() {
  if ((VBView *)imagetab->currentPage())
    ((VBView *)imagetab->currentPage())->ByteSwap();
}

void VBViewMain::LayerInfo() {
  if ((VBView *)imagetab->currentPage())
    ((VBView *)imagetab->currentPage())->LayerInfo();
}

void VBViewMain::LoadAverages() {
  if ((VBView *)imagetab->currentPage())
    ((VBView *)imagetab->currentPage())->LoadAverages();
}

void VBViewMain::GoToOrigin() {
  if ((VBView *)imagetab->currentPage())
    ((VBView *)imagetab->currentPage())->GoToOrigin();
}

void VBViewMain::NewPane() { AddView(); }

int VBViewMain::ClosePanel(QWidget *w) {
  if (imagetab->indexOf(w) == -1) return (101);
  imagetab->removePage(w);
  delete w;
  return (0);
}

int VBViewMain::RenamePanel(QWidget *w, string label) {
  if (imagetab->indexOf(w) == -1) return (101);
  imagetab->changeTab(w, label.c_str());
  return (0);
}

void VBViewMain::SaveImage() {
  if ((VBView *)imagetab->currentPage())
    ((VBView *)imagetab->currentPage())->SaveImage();
}

void VBViewMain::OpenFile() {
  fileview fv;
...
void VBViewMain::Help() {
  string text = (string) "The following key shortcuts can be used here:\n" +
                "-/= to zoom in or out\n" + "SPACE to toggle the crosshairs\n" +
...
void vbview_help() { cout << boost::format(myhelp) % vbversion; }

void vbview_version() { printf("VoxBo vbview (v%s)\n", vbversion.c_str()); }
