...
SequenceScene::SequenceScene(QObject* parent)
    : QGraphicsScene(parent),
      _sequence(VB::Sequence(new BlockDef())),
      is_settling(false),
      is_supressed(false) {
...
SequenceScene::SequenceScene(qreal x, qreal y, qreal width, qreal height,
                             QObject* parent)
...
SequenceScene::~SequenceScene() { delete _root_bd; }
...
void SequenceScene::itemMoved(SequenceItem* item) {
  ...
  qreal w = width();
  qreal h = height();
  if (w < bottomRight.x()) w = bottomRight.x();
  if (h < bottomRight.y()) h = bottomRight.y();
...
void SequenceScene::removeExecItem(ExecItem* ei) {
  ...
  vbforeach(QGraphicsItem * gi, items()) {
    if (qgraphicsitem_cast<SequenceItem*>(gi)->sequenceItemType() ==
        "waitfor") {
      WaitforItem* wi = qgraphicsitem_cast<WaitforItem*>(gi);
      if (wi->fromItem() == ei)
        removeItem(wi);
      else if (wi->toItem() == ei)
        removeItem(wi);
    }
  }
...
ExecItem* SequenceScene::beginPointItem() { return _beginPointItem; }

ExecItem* SequenceScene::endPointItem() { return _endPointItem; }

void SequenceScene::setBeginPointItem(ExecItem* ei) { _beginPointItem = ei; }

void SequenceScene::setEndPointItem(ExecItem* ei) { _endPointItem = ei; }
...
void SequenceScene::clear() {
  clearFocus();
  clearSelection();
  vbforeach(QGraphicsItem * item, items()) removeItem(item);
}
...
void SequenceScene::loadSequence(VB::Sequence& seq) {
  ...
  QHash<ExecItem*, QPair<int, int> >
      placement;  // think of this as a grid indexed by contents.
  int num_jobs = exec_item_list.size();
  ExecItem* grid[num_jobs][num_jobs];
  for (int i = 0; i < num_jobs; ++i)
    for (int j = 0; j < num_jobs; ++j) grid[i][j] = 0;
  ...
  vbforeach(ExecItem * job_item, exec_item_list) {
    ...
    if ((wait_set.size() == 1) &&
        (*(wait_set.begin()) == prev_job_item->exec())) {
  ...
  for (int r = 0; r < num_jobs; ++r) {
    ...
    for (int c = 0; c < num_jobs; ++c)
      if (grid[r][c]) {
        rowheight[r] =
            max(rowheight[r], grid[r][c]->boundingRect().height() + 10);
      }
  }
...
bool SequenceScene::settler_supressed() const { return is_supressed; }

void SequenceScene::occupy_settler() { is_settling = true; }

void SequenceScene::free_settler() { is_settling = false; }
...
void SequenceScene::settle_helper() {
  if (/*!is_settling && */ !is_supressed) {
    ...
    //		occupy_settler();
    //    QTimer::singleShot(int(1000 / CALLS_PER_SECOND), this,
    //    SLOT(free_setler()));
    ...
    QMap<ExecItem*, QPointF>
        displacement;  // should be one for each jobitem in item_list
    ...
    for (int i = 0; i < ITERATIONS_PER_CALL; ++i) {
      for (QList<QGraphicsItem*>::iterator iter = item_list.begin();
        ...
        // Two jobs that are not connected prefer to be on top of each other, if
        // they are within a given distance.
        if (si->isVisible() && (si->sequenceItemType() == "job" ||
                                si->sequenceItemType() == "block")) {
          ExecItem* ei1 = static_cast<ExecItem*>(si);

          QList<QGraphicsItem*>::iterator iter2 = iter;
          for (++iter2; iter2 != item_list.end(); ++iter2) {
            SequenceItem* si2 = static_cast<SequenceItem*>(*iter2);
            if (si2->isVisible() && si2->parentItem() == si->parentItem() &&
                (si2->sequenceItemType() == "job" ||
                 si2->sequenceItemType() == "block")) {
              ExecItem* ei2 = static_cast<ExecItem*>(si2);

              if (find(ei1->exec()->depends.begin(), ei1->exec()->depends.end(),
                       ei2->exec()) == ei1->exec()->depends.end() &&
                  find(ei2->exec()->depends.begin(), ei2->exec()->depends.end(),
                       ei1->exec()) == ei2->exec()->depends.end()) {
                QPointF dist_vector = ei2->pos() - ei1->pos();

                QPointF MY_NOWAITVECTOR = NOWAITVECTOR;
                if (dist_vector.y() < 0) MY_NOWAITVECTOR *= -1;

                qreal d_sq = dist_vector.x() * dist_vector.x() +
                             dist_vector.y() * dist_vector.y();
                if (d_sq == 0)
                  d_sq = 0.0000000001;  // <-- never zero, but "very small"
                //                qreal d = sqrt(d_sq);

                //                if (d < CRITICALDISTANCE)
                if (fabs(dist_vector.x()) < CRITICALDISTANCE / 2 &&
                    fabs(dist_vector.y()) < CRITICALDISTANCE) {
                  displacement[ei1] +=
                      STEPFACTOR * (dist_vector - MY_NOWAITVECTOR) / 2;
                  displacement[ei2] -=
                      STEPFACTOR * (dist_vector - MY_NOWAITVECTOR) / 2;
                }
              }
        ...
        // Waitfor lines prefer a perfectly horizontal orientation with legth
        // LENGTH
    ...
    //  	if (max_movement > 0) std::cerr << "max movement (distance^2): "
    //  << max_movement << std::endl;

    bool disequilibriated;
    if (max_movement > MOVETHRESHOLD)
      disequilibriated = true;
    else
      disequilibriated = false;
    ...
    if (disequilibriated) {
      ...
      QTimer::singleShot(int(1000 / (CALLS_PER_SECOND * time_factor)), this,
                         SLOT(settle_helper()));
    }
  }
}
