...
ExecItem::ExecItem(shared_ptr<VB::Exec> ei, SequenceItem* parent,
                   SequenceScene* scene)
    : SequenceItem(parent, scene), _exec(ei), _boundingRect(0, 0, 50, 20) {}

BlockItem::BlockItem(shared_ptr<VB::Block> b, SequenceItem* parent,
                     SequenceScene* scene)
...
JobItem::JobItem(shared_ptr<VB::Job> j, SequenceItem* parent,
                 SequenceScene* scene)
...
/* detailed()
 */
bool SequenceItem::detailed() const { return _detailed; }
...
/* paint (virtual)
 */
void WaitforItem::paint(QPainter* painter, const QStyleOptionGraphicsItem*,
                        QWidget*) {
  ...
  painter->drawEllipse(int(src_point.x()) - CONN_RAD,
                       int(src_point.y()) - CONN_RAD, CONN_RAD * 2,
                       CONN_RAD * 2);
  painter->drawEllipse(int(snk_point.x()) - CONN_RAD,
                       int(snk_point.y()) - CONN_RAD, CONN_RAD * 2,
                       CONN_RAD * 2);
}
...
QRectF WaitforItem::boundingRect() const {
  ...
  return rect.adjusted(-CONN_RAD, -CONN_RAD, CONN_RAD,
                       CONN_RAD);  //.adjusted(-10,-10,10,10);
}
...
QPointF TempWaitforDragItem::toEnd() const { return _toPoint; }
...
void BlockItem::paint(QPainter* painter, const QStyleOptionGraphicsItem*,
  ...
  if (selected)
    painter->setBrush(BLOCK_SELECT_TEXT_COLOR);
  else if (hovered)
    painter->setBrush(BLOCK_HOVER_TEXT_COLOR);

  painter->setPen(BLOCK_TEXT_COLOR);
  if (selected)
    painter->setPen(BLOCK_SELECT_TEXT_COLOR);
  else if (hovered)
    painter->setPen(BLOCK_HOVER_TEXT_COLOR);

  painter->drawText(x, y, w, h, Qt::AlignHCenter | Qt::AlignVCenter,
                    block()->name.c_str());
  ...
  vbforeach(ExecItem * job_item, exec_item_list) {
    ...
    if ((wait_set.size() == 1) &&
      ...
      vbforeach(shared_ptr<Exec> wait_job, wait_set) {
        ExecItem* wait_job_item =
            static_cast<SequenceScene*>(scene())->itemByExec(wait_job);
...
bool BlockItem::execsShown() { return _detailed; }

/* ExecItem ******************************************************************/

shared_ptr<VB::Exec> ExecItem::exec() const { return _exec; }
...
/* paint (virtual)
 */
void JobItem::paint(QPainter* painter, const QStyleOptionGraphicsItem*,
                    QWidget*) {
  int x = 0;
  int y = 0;
  int h = _boundingRect.height();
  ...
  if (selected)
    painter->setBrush(JOB_SELECT_TEXT_COLOR);
  else if (hovered)
    painter->setBrush(JOB_HOVER_TEXT_COLOR);

  painter->setPen(JOB_TEXT_COLOR);
  if (selected)
    painter->setPen(JOB_SELECT_TEXT_COLOR);
  else if (hovered)
    painter->setPen(JOB_HOVER_TEXT_COLOR);
 ...
 * itemChange
 */
QVariant SequenceItem::itemChange(GraphicsItemChange change,
...
QVariant ExecItem::itemChange(GraphicsItemChange change,
                              const QVariant& value) {
  if (change == ItemPositionChange) {
...
ExecItem* WaitforItem::fromItem() const { return _fromItem; }

ExecItem* WaitforItem::toItem() const { return _toItem; }
