...
VBscoregen::parseconfig(string configfile)
{
  ...
  while (infile.getline(buf,STRINGLEN,'\n')) {
    ...
    if (args[0]=="intact" && args.size()==3) {
      ...
      vector<string> ll;  ll.push_back(lesionfile);
...
void
VBscoregen::generate(string fname)
{
  if (scores.size()==0) {
...
void
VBscoregen::do_ifor(tokenlist &args)
{
...
void
VBscoregen::do_ifand(tokenlist &args)
{
  double thresh=max(strtod(args[3]),DBL_MIN);
  string v1name=args[1];
  string v2name=args[2];
...
void
VBscoregen::do_pct(tokenlist &args)
{
  string v1name=args[1];
...
void
VBscoregen::do_pctmin(tokenlist &args)
{
  string v1name=args[1];
  string v2name=args[2];
  string refname=args[3];
...
void
VBscoregen::do_pctmax(tokenlist &args)
{
  string v1name=args[1];
  string v2name=args[2];
  string refname=args[3];
