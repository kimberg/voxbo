...
int main(int argc, char *argv[]) {
  ...
  if (glmname.size() == 0 && teslist.size() == 0) {
    printf(
        "[E] vbxts: you must specify either a GLM directory or a tes file\n");
    exit(5);
  }
  if (averagelist.size() == 0) averagelist.push_back("none");
  ...
  for (int j = 0; j < (int)regionlist.size(); j++) {
    ...
    if (glmname.size()) {
      if (q_filterflag) glmi.filterTS(vv);
      if (q_removeflag) glmi.adjustTS(vv);
    }
    if (averages.size()) {
      for (int i = 0; i < (int)averages.size(); i++) {
        ...
        if (outfile.size())
          ...
          tmpx.WriteFile("vbxts_" + regionlist[j].name + "_" +
                         averages[i].name + ".ref");
      }
    } else {
      if (q_powerflag) {
...
void vbxts_help() { cout << boost::format(myhelp) % vbversion; }

void vbxts_version() { printf("VoxBo vbxts (v%s)\n", vbversion.c_str()); }
