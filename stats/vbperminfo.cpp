  ...
  if (mode == bytimeseries) {
    ...
    for (int i = 0; i < iterations.dimx; i++) {
      for (int j = 0; j < iterations.dimy; j++) {
        for (int k = 0; k < iterations.dimz; k++) {
          ...
          if (!iterations.GetMaskValue(i, j, k)) continue;
          voxelcount++;
          if (iterations.ReadTimeSeries(iterfile, i, j, k)) {
            cout << format(
                "[E] vbperminfo: couldn't read series data from iteration "
                "volume\n");
            exit(110);
          }
          ...
          for (uint32 m = 0; m < iterations.timeseries.size(); m++) {
            if (!(val > iterations.timeseries[m])) notexceededcnt++;
          }
    ...
    for (int t = 0; t < iterations.dimt; t++) {
      if (iterations.ReadVolume(iterfile, t, cb)) {
        cout << format(
                    "[E] vbperminfo: couldn't read volume data from iteration "
                    "volume %s\n") %
                    iterfile;
        exit(111);
      }
      for (int i = 0; i < cb.dimx * cb.dimy * cb.dimz; i++) {
        ...
        if (!iterations.GetMaskValue(i)) continue;
        val = cb.getValue(i);
        if (!mask.testValueUnsafe<char>(i)) {
          if (fabs(val) >= DBL_MIN) mask.setValue<char>(i, 1);
        }
        voxelcount++;
    ...
    if (fnames.empty()) {
      cout << format(
          "[E] vbperminfo: no iteration volumes found by that name or "
          "location\n");
      exit(112);
    }
    itercnt = fnames.size();
...
void calc_dist(tokenlist &args) {
  ...
  if (!vv.ReadFile(args[2]))
    if (vv.size() == 1) crit = vv[0];
  if (vv.size() != 1) {  // couldn't get it from a file
    pair<bool, double> critx = strtodx(args[2]);
    if (critx.first) {
      cout << "[E] vbperminfo: couldn't derive a valid true value from "
           << args[2] << endl;
      exit(147);
    }
    crit = critx.second;
  ...
  if (vv.size() < 100)
    cout << "[W] vbperminfo: number of elements in vector is only " << vv.size()
         << endl;
  vv.sort();
  int32 cnt = 0;
  for (uint32 i = 0; i < vv.size(); i++) {
    if (f_rev) {
      if ((vv[i] - crit) < DBL_MIN) cnt++;
    } else {
      if ((crit - vv[i]) < DBL_MIN) cnt++;
    }
  }
  cout << (format("[I] vbperminfo: your p value is %g\n") %
           ((double)cnt / (double)(vv.size())))
              .str();
}
...
void calc_cutoff(tokenlist &args) {
  ...
  if (myvec.ReadFile(args[1])) {
    cout << "[E] vbperminfo: couldn't open result vector file " << args[1]
         << endl;
    exit(121);
  }
  ...
  if (args.size() > 2) alpha = strtod(args[2]);
  int sz = myvec.size();
  ...
  if (alpha > 0) {
    ...
    cout << "[I] vbperminfo: Cutoff index: " << index + 1 << " (from 1 to "
         << myvec.size() << ")" << endl;
...
void calc_peak(tokenlist &args) {
  ...
  string maskfile;
  if (args.size() > 4) maskfile = args[4];
  double alpha = 0.05;
  Tes iterations;
  VB_Vector myvec;
  Cube mask;
  ...
  if (mode == byvolume) {
    ...
    for (int t = 0; t < iterations.dimt; t++) {
      ...
      if (mask) {
        if (!(cb.dimsequal(mask))) {
          cout << format(
                      "[E] vbperminfo: volume %s dimensions don't match "
                      "mask\n") %
                      iterfile;
          exit(177);
        }
    ...
    if (mask) {
      if (!(iterations.dimsequal(mask))) {
        cout << format(
                    "[E] vbperminfo: volume %s dimensions don't match mask\n") %
                    iterfile;
        exit(177);
      }
  ...
  myvec.sort();  // sorts ascending, luckily
  if (args.size() > 3) alpha = strtod(args[3]);
  int sz = myvec.size();
  int index = (int)ceil((1.0 - alpha) * sz) - 1;
...
void calc_peakmin(tokenlist &args) {
  ...
  for (int i = 0; i < nsets; i++) {
    ...
    vglob vg(stemlist[i] + "*");
    if (vg.size() < 2) vg.load(stemlist[i] + "/*.cub");
    if (vg.size() < 2) vg.load(stemlist[i] + "/*.img");
    if (vg.size() < 2) vg.load(stemlist[i] + "/*.nii");
    if (vg.size() < 2) vg.load(stemlist[i] + "/*");
    if (vg.size() > 1) {
  ...
  for (p = 0; p < nperms; p++) {
    ...
    if (xstart + xcount - 1 < p) {
      ...
      for (int i = 0; i < nsets; i++) {
        if (fnamelist[i].size() == 1) {
          ...
          if (!(dimsConsistent(xx, yy, zz, teslist[i].dimx, teslist[i].dimy,
                               teslist[i].dimz))) {
            printf("[E] vbperminfo: dimension mismatch\n");
            exit(240);
          }
    ...
    for (v = 0; v < nsets; v++) {
      ...
      if (!(dimsConsistent(xx, yy, zz, mycub[v].dimx, mycub[v].dimy,
                           mycub[v].dimz))) {
        printf("[E] vbperminfo: dimension mismatch\n");
        exit(240);
      }
    ...
    for (int i = 0; i < xx * yy * zz; i++) {
      ...
      if (voxelcount == 0) {
        ...
        if (minval > peakmin) peakmin = minval;
  ...
  alpha = 0.005;
  index = (int)ceil((1.0 - alpha) * sz) - 1;
  minvec.AddHeader("Cutoff for alpha " + strnum(alpha) + ": " +
                   strnum(minvec[index]));
...
void calc_cluster(tokenlist &args) {
  ...
  for (size_t i = 0; i < vg.size(); i++) {
    ...
    for (int j = 1; j < (int)rlist.size(); j++) {
      if ((int)rlist[j].size() > maxsize) maxsize = rlist[j].size();
    }
    myvec[i] = maxsize;
  }
  ...
  // now figure out the cutoff value for the supplied alpha or 0.5
  myvec.sort();  // sorts ascending, luckily
  if (args.size() > 4) alpha = strtod(args[4]);
  int sz = myvec.size();
  int index = (int)ceil((1.0 - alpha) * sz) - 1;
  string info;
...
double find_cluster_thresh(Cube &cb, int crit_type, int k) {
  if (crit_type != vb_agt && crit_type != vb_gt) return -1.0;
  ...
  for (int i = 0; i < cb.dimx; i++) {
    for (int j = 0; j < cb.dimy; j++) {
      for (int k = 0; k < cb.dimz; k++) {
        ...
        if (crit_type == vb_agt) val = fabs(val);
        if (crit_type == vb_gt && val < 0) continue;
  ...
  int highclusters = 0;
  middle = low +
           ((high - low) * 19 / 20);  // heuristic good starting point, avoid
                                      // too many cycles of whole-brain clusters
  while (TRUE) {
    rlist = findregions(cb, crit_type, cubevals[middle]);
    clusters = 0;
    for (int i = 0; i < (int)rlist.size(); i++) {
      if ((int)rlist[i].size() >= k) clusters++;
    }
    if (clusters) {
      low = middle;
  ...
  if (lowclusters && highclusters == 0) return cubevals[high];
  return -2.0;
}
...
vector<string> getfilenames(string fname) {
  ...
  vg.load(fname + "/*");
  if (vg.size() == 0) vg.load(fname + "*");

  for (size_t i = 0; i < vg.size(); i++)
    if (xgetextension(vg[i]) != "hdr") fnames.push_back(vg[i]);

  return fnames;
}

void vbperminfo_help() { cout << boost::format(myhelp) % vbversion; }
