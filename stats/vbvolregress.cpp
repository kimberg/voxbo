...
main(int argc,char *argv[])
{
  ...
  if (perminds.size()>1 && distname.size())
    unlink(distname.c_str());
  ...
  for (size_t permi=0; permi<perminds.size(); permi++) {
    ...
    for (int c=0; c<(int)contrastlist.size(); c++) {
      ...
      if (f_volume) {
        ...
        if (f_fdr) {
          vector<fdrstat> ffs=calc_multi_fdr_thresh(glmi.rawcube,glmi.statcube,tmask,q);
          if (ffs.size()) {
            cout << (format("[I] vbvolregress: FDR calculation included %d voxels with p values from %.4f to %.4f\n")
                     %ffs[0].nvoxels%ffs[0].low%ffs[0].high).str();
            vbforeach(fdrstat ff,ffs) {
              if (ff.maxind>=0)
                cout << (format("[I] vbvolregress: FDR threhsold for q=%.2f is %.4f\n")%ff.q%ff.statval).str();
              else
                cout << (format("[I] vbvolregress: no FDR threhsold could be identified for q=%.2f\n")%ff.q).str();
            }
          }
        }
        ...
        if (distname.size()) {
          if (appendline(distname,(format("%.20g")%glmi.statcube.get_maximum()).str())) {
