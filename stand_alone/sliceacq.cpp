...
*********************************************************************/
  ...
  {
    ostringstream errorMsg;
    errorMsg << "Line Number [" << __LINE__ << "] Invalid data for Tes file [" << tesFile << "].";
...
*********************************************************************/
  ...
  {
    ostringstream errorMsg;
    errorMsg << "Line Number [" << __LINE__ << "] Invalid header for Tes file [" << tesFile << "].";
    printErrorMsgAndExit(VB_ERROR, errorMsg.str(), 1);
  } // if
...
*********************************************************************/
 ...
 {
    ostringstream errorMsg;
    errorMsg << "Line Number [" << __LINE__ << "] The 4D data file ["
    << tesFile << "] has already been corrected.";
    printErrorMsgAndExit(VB_ERROR, errorMsg.str(), 1);
 } // if
...
*********************************************************************/
    int pauseFlag = 0;
    readParamFile(paramFilePath, reqHeaderInfo.pulseSeq, reqHeaderInfo.fieldStrength,
    &sliceTime, &pauseFlag, reqHeaderInfo.tr, dimZ);
...
*********************************************************************/
  if (!sanityCheckOfValues(sliceTime, firstSliceTime, reqHeaderInfo.tr, dimZ))
  {
    ...
    << "] Correction parameter values are not valid: TR = [" << reqHeaderInfo.tr
...
// DYK: added the following to handle interleaved or not
  ...
  for (unsigned int i = 0; i < dimZ; i++)
  {
    ...
    shiftAmount[i] = ((timeAcquired[i] * sliceTime) + firstSliceTime) / (double ) reqHeaderInfo.tr;
  } // for i
...
void usage(const unsigned short exitValue, char *progName)
{
  ...
  vbp.init();
  string tempStr("                          " + vbp.rootdir + "etc/sliceacq.cfg");

  genusage(exitValue, progName, "- Acquisition correction routine for VoxBo.",
           ...
           "-p                        Configuration file path. Default is", tempStr.c_str(),
...
*********************************************************************/
  if ((*sliceTime) == 0.0)
  {
    ostringstream errorMsg;
    errorMsg << "Line Number [" << __LINE__
    << "] Unable to determine slice time and time of first slice. Used SEQUENCE = ["
    << seq << "] FIELD STRENGTH = [" << fieldStrength << "] PARAMETER FILE = [" << paramFileName << "].";
    printErrorMsgAndExit(VB_ERROR, errorMsg.str(), 1);
  } // if
...
*********************************************************************/
  return (((absDiff <= epsilon) || (diff > 0.0)) && (tr > 0) && (firstSlice >= 0.0) && (sliceTime > 0.0) && (dimZ > 0));
...
*********************************************************************/
  ...
  {
    ostringstream errorMsg;
    errorMsg << "Line Number [" << __LINE__ << "]  Need length to be >= 2: length = [" << length << "].";
    printErrorMsgAndExit(VB_ERROR, errorMsg.str(), 1);
  } // if
...
*********************************************************************/
      realShifter[j] = (cos(phi[j]) * realPart[j]) - (sin(phi[j]) * imagPart[j]);
      imagShifter[j] = (cos(phi[j]) * imagPart[j]) + (sin(phi[j]) * realPart[j]);
    } // for j
...
*                                     stored here.                   *
********************************************************************/
void phaseShift(const VB_Vector& tSeries, const double timeShift, VB_Vector& shiftedSignal)
{
