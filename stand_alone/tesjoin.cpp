...
void writeMoveParamsFile(const double *paramValues, const string &outputFile,
                         const int origDimT, const string &firstSplitFile);
...
int main(int argc, char *argv[]) {
  ...
  if (printHelp) usage(0, argv[0]);

  if (printVersion) printf("\nVoxBo v%s\n", vbversion.c_str());

  if ((directoryName.size() == 0) && (fileName.size() == 0) &&
      (fileList.size() == 0)) {
  ...
  if ((directoryName.size() > 0) &&
      ((fileName.size() > 0) || (fileList.size() > 0))) {
    ostringstream errorMsg;
    errorMsg << "Must use exactly one of \"-d\", \"-f\", or \"-l\".";
    printErrorMsg(VB_ERROR, errorMsg.str(), __LINE__, __FUNCTION__, __FILE__);
    usage(1, argv[0]);
  ...
  if ((fileName.size() > 0) &&
      ((directoryName.size() > 0) || (fileList.size() > 0))) {
    ostringstream errorMsg;
    errorMsg << "Must use exactly one of \"-d\", \"-f\", or \"-l\".";
    printErrorMsg(VB_ERROR, errorMsg.str(), __LINE__, __FUNCTION__, __FILE__);
    usage(1, argv[0]);
  ...
  if ((fileList.size() > 0) &&
      ((directoryName.size() > 0) || (fileName.size() > 0))) {
    ostringstream errorMsg;
    errorMsg << "Must use exactly one of \"-d\", \"-f\", or \"-l\".";
    printErrorMsg(VB_ERROR, errorMsg.str(), __LINE__, __FUNCTION__, __FILE__);
    usage(1, argv[0]);
  ...
  if (directoryName.size() > 0) {
    ...
    if (errno != 0) {
      ostringstream errorMsg;
      errorMsg << "Opening directory [" << directoryName << "] failed due to ["
               << strerror(errno) << "].";
      printErrorMsgAndExit(VB_ERROR, errorMsg.str(), __LINE__, __FUNCTION__,
                           __FILE__, 1);
    }
    ...
    while ((dirInfo = readdir(dPtr)) != NULL) {
      ...
      if (utils::determineVBFileDims(fullFileName) == 4) {
        ...
        ostringstream errorMsg;
        errorMsg << "[" << fullFileName
                 << "] is not a 4D VoxBo data file or is unreadable.";
        printErrorMsgAndExit(VB_ERROR, errorMsg.str(), __LINE__, __FUNCTION__,
                             __FILE__, 1);
      }
    }
  ...
  for (unsigned int i = 0; i < inputFiles.size(); i++) {
    ...
    if (i == 0) {
      ...
      if (splitType == string("Extraction")) {
        ...
        if (outputFile.size() == 0) outputFile = S[3];
      } else {
        ostringstream errorMsg;
      ...
      // TGK: all the below are substitutions of GetHeader tes method where
      // getHeaderLine had been used
      if ((origVoxSizes[0] != splitTes[i].voxsize[0]) ||
          (origVoxSizes[1] != splitTes[i].voxsize[1]) ||
          (origVoxSizes[2] != splitTes[i].voxsize[2])) {
        ostringstream errorMsg;
        errorMsg << "The voxel sizes [" << origVoxSizes[0] << ", "
                 << origVoxSizes[1] << ", " << origVoxSizes[2]
                 << "] from the first input file [" << splitTes[0].filename
                 << "] do not match the voxel sizes [" << splitTes[i].voxsize[0]
                 << ", " << splitTes[i].voxsize[1] << ", "
                 << splitTes[i].voxsize[2] << "] from the current input file ["
                 << splitTes[i].filename << "].";
        printErrorMsgAndExit(VB_ERROR, errorMsg.str(), __LINE__, __FUNCTION__,
                             __FILE__, 1);
      }
      if ((origOrigin[0] != splitTes[i].origin[0]) ||
          (origOrigin[1] != splitTes[i].origin[1]) ||
          (origOrigin[2] != splitTes[i].origin[2])) {
        ostringstream errorMsg;
        errorMsg << "The origin coordinates [" << origOrigin[0] << ", "
                 << origOrigin[1] << ", " << origOrigin[2]
                 << "] from the first input file [" << splitTes[0].filename
                 << "] do not match the origin coordinates ["
                 << splitTes[i].origin[0] << ", " << splitTes[i].origin[1]
                 << ", " << splitTes[i].origin[2]
       ...
       *********************************************************************/
      if ((dateCreatedLine.length() > 0) &&
          (dateCreatedLine != splitTes[i].GetHeader("DateCreated:"))) {
        ostringstream errorMsg;
        errorMsg << "The creation date [" << dateCreatedLine
                 << "] from the first input file [" << splitTes[0].filename
                 << "] does not match the creation date ["
                 << splitTes[i].GetHeader("DateCreated:")
                 << "] from the current input file [" << splitTes[i].filename
                 << "].";
        printErrorMsgAndExit(VB_ERROR, errorMsg.str(), __LINE__, __FUNCTION__,
                             __FILE__, 1);
      }
      if ((rawFileLine.length() > 0) &&
          (rawFileLine != splitTes[i].GetHeader("RawFile:"))) {
        ostringstream errorMsg;
        errorMsg << "The raw file name [" << rawFileLine
                 << "] from the first input file [" << splitTes[0].filename
                 << "] does not match the raw file name ["
                 << splitTes[i].GetHeader("RawFile:")
                 << "] from the current input file [" << splitTes[i].filename
                 << "].";
        printErrorMsgAndExit(VB_ERROR, errorMsg.str(), __LINE__, __FUNCTION__,
                             __FILE__, 1);
      }
      if ((subjectNameLine.length() > 0) &&
          (subjectNameLine != splitTes[i].GetHeader("SubjectName:"))) {
        ostringstream errorMsg;
        errorMsg << "The subject name [" << subjectNameLine
                 << "] from the first input file [" << splitTes[0].filename
                 << "] does not match the subject name date ["
                 << splitTes[i].GetHeader("SubjectName:")
                 << "] from the current input file [" << splitTes[i].filename
                 << "].";
        printErrorMsgAndExit(VB_ERROR, errorMsg.str(), __LINE__, __FUNCTION__,
                             __FILE__, 1);
      }
      if ((subjectAgeLine.length() > 0) &&
          (subjectAgeLine != splitTes[i].GetHeader("SubjectAge:"))) {
        ostringstream errorMsg;
        errorMsg << "The subject age [" << subjectAgeLine
                 << "] from the first input file [" << splitTes[0].filename
                 << "] does not match the subject age ["
                 << splitTes[i].GetHeader("SubjectAge:")
                 << "] from the current input file [" << splitTes[i].filename
                 << "].";
        printErrorMsgAndExit(VB_ERROR, errorMsg.str(), __LINE__, __FUNCTION__,
                             __FILE__, 1);
      ...
      if (TRval > 0 && TRval != splitTes[i].voxsize[3]) {
        ostringstream errorMsg;
        errorMsg << "The TR [" << TRval << "] from the first input file ["
                 << splitTes[0].filename << "] does not match the TR ["
                 << splitTes[i].voxsize[3] << "] from the current input file ["
                 << splitTes[i].filename << "].";
        printErrorMsgAndExit(VB_ERROR, errorMsg.str(), __LINE__, __FUNCTION__,
                             __FILE__, 1);
      }
      if ((TELine.length() > 0) &&
          (TELine != splitTes[i].GetHeader("TE(msecs):"))) {
        ostringstream errorMsg;
        errorMsg << "The TE line [" << TELine << "] from the first input file ["
                 << splitTes[0].filename << "] does not match the TE line ["
                 << splitTes[i].GetHeader("TE(msecs):")
                 << "] from the current input file [" << splitTes[i].filename
                 << "].";
        printErrorMsgAndExit(VB_ERROR, errorMsg.str(), __LINE__, __FUNCTION__,
                             __FILE__, 1);
      }
      if ((orientationLine.length() > 0) &&
          (orientationLine != splitTes[i].GetHeader("Orientation:"))) {
        ostringstream errorMsg;
        errorMsg << "The orientation [" << orientationLine
                 << "] from the first input file [" << splitTes[0].filename
                 << "] does not match the orientation ["
                 << splitTes[i].GetHeader("Orientation:")
                 << "] from the current input file [" << splitTes[i].filename
                 << "].";
        printErrorMsgAndExit(VB_ERROR, errorMsg.str(), __LINE__, __FUNCTION__,
                             __FILE__, 1);
      }
      if ((coilNameLine.length() > 0) &&
          (coilNameLine != splitTes[i].GetHeader("CoilName:"))) {
        ostringstream errorMsg;
        errorMsg << "The coil name [" << coilNameLine
                 << "] from the first input file [" << splitTes[0].filename
                 << "] does not match the coil name ["
                 << splitTes[i].GetHeader("CoilName:")
                 << "] from the current input file [" << splitTes[i].filename
                 << "].";
        printErrorMsgAndExit(VB_ERROR, errorMsg.str(), __LINE__, __FUNCTION__,
                             __FILE__, 1);
      }
      if ((pulseSeqLine.length() > 0) &&
          (pulseSeqLine != splitTes[i].GetHeader("PulseSeq:"))) {
        ostringstream errorMsg;
        errorMsg << "The pulse sequence [" << pulseSeqLine
                 << "] from the first input file [" << splitTes[0].filename
                 << "] does not match the pulse sequence ["
                 << splitTes[i].GetHeader("PulseSeq:")
                 << "] from the current input file [" << splitTes[i].filename
                 << "].";
        printErrorMsgAndExit(VB_ERROR, errorMsg.str(), __LINE__, __FUNCTION__,
                             __FILE__, 1);
      }
      if ((fieldStrengthLine.length() > 0) &&
          (fieldStrengthLine != splitTes[i].GetHeader("FieldStrength:"))) {
        ostringstream errorMsg;
        errorMsg << "The field strength [" << fieldStrengthLine
                 << "] from the first input file [" << splitTes[0].filename
                 << "] does not match the field strength ["
                 << splitTes[i].GetHeader("FieldStrength:")
                 << "] from the current input file [" << splitTes[i].filename
                 << "].";
        printErrorMsgAndExit(VB_ERROR, errorMsg.str(), __LINE__, __FUNCTION__,
                             __FILE__, 1);
      }
      if ((taskTagLine.length() > 0) &&
          (taskTagLine != splitTes[i].GetHeader("TaskTag:"))) {
        ostringstream errorMsg;
        errorMsg << "The task tag [" << taskTagLine
                 << "] from the first input file [" << splitTes[0].filename
                 << "] does not match the task tag ["
                 << splitTes[i].GetHeader("TaskTag:")
                 << "] from the current input file [" << splitTes[i].filename
                 << "].";
        printErrorMsgAndExit(VB_ERROR, errorMsg.str(), __LINE__, __FUNCTION__,
                             __FILE__, 1);
      }
      if ((byteOrderLine.length() > 0) &&
          (byteOrderLine != splitTes[i].GetHeader("Byteorder:"))) {
        ostringstream errorMsg;
        errorMsg << "The byte order [" << byteOrderLine
                 << "] from the first input file [" << splitTes[0].filename
                 << "] does not match the byte order ["
                 << splitTes[i].GetHeader("Byteorder:")
                 << "] from the current input file [" << splitTes[i].filename
                 << "].";
        printErrorMsgAndExit(VB_ERROR, errorMsg.str(), __LINE__, __FUNCTION__,
                             __FILE__, 1);
      ...
      if (origDataType != splitTes[i].datatype) {
        ostringstream errorMsg;
        errorMsg << " The current data type [" << splitTes[i].datatype
                 << "] does not match the first data type [" << origDataType
                 << "].";
        printErrorMsgAndExit(VB_ERROR, errorMsg.str(), __LINE__, __FUNCTION__,
                             __FILE__, 1);
      }
      if (origDimT != (int)strtol(S[5])) {
        ostringstream errorMsg;
        errorMsg << " The current original dimT [" << (int)strtol(S[5])
                 << "] does not equal the first original dimT [" << origDimT
                 << "].";
        printErrorMsgAndExit(VB_ERROR, errorMsg.str(), __LINE__, __FUNCTION__,
                             __FILE__, 1);
      }
      if (dimX != splitTes[i].dimx) {
        ostringstream errorMsg;
        errorMsg << " The current dimX [" << splitTes[i].dimx
                 << "] does not equal the first dimX [" << dimX << "].";
        printErrorMsgAndExit(VB_ERROR, errorMsg.str(), __LINE__, __FUNCTION__,
                             __FILE__, 1);
      }
      if (dimY != splitTes[i].dimy) {
        ostringstream errorMsg;
        errorMsg << " The current dimY [" << splitTes[i].dimy
                 << "] does not equal the first dimY [" << dimY << "].";
        printErrorMsgAndExit(VB_ERROR, errorMsg.str(), __LINE__, __FUNCTION__,
                             __FILE__, 1);
      }
      if (dimZ != splitTes[i].dimz) {
        ostringstream errorMsg;
        errorMsg << " The current dimZ [" << splitTes[i].dimz
                 << "] does not equal the first dimZ [" << dimZ << "].";
        printErrorMsgAndExit(VB_ERROR, errorMsg.str(), __LINE__, __FUNCTION__,
                             __FILE__, 1);
      }
      if (splitType != S[1]) {
        ostringstream errorMsg;
        errorMsg << " The file [" << splitTes[i].filename
                 << "] has split type [" << S[1]
                 << "]. This does not match the split type of the first file ["
                 << splitTes[0].filename << "] which is [" << splitType << "].";
        printErrorMsgAndExit(VB_ERROR, errorMsg.str(), __LINE__, __FUNCTION__,
                             __FILE__, 1);
      }
    }
  ...
  for (unsigned int i = 0; i < inputFiles.size(); i++) {
    ...
    if (splitType == "Divided") {
       ...
       *********************************************************************/
      for (int rangeIndex = beginRange, splitDimTIndex = 0;
           rangeIndex <= endRange; rangeIndex++, splitDimTIndex++) {
        /*********************************************************************
        ...
        outputTes.SetValue(
            indexX, indexY, indexZ, rangeIndex,
            splitTes[i].GetValue(indexX, indexY, indexZ, splitDimTIndex));
      }
      CLOSE_SPATIAL_LOOPS
      if (moveParams) {
        /*********************************************************************
        ...
        memcpy(paramValues + (beginRange * MOVE_PARAMS), pVec.getData(),
               sizeof(double) * pVec.size());
      }
       ...
       *********************************************************************/
      for (int splitIndex = 0, startSeries = strtol(S[6]);
           splitIndex < splitTes[i].dimt;
           splitIndex++, startSeries += numBins) {
        /*********************************************************************
         * The appropriate element of tSeriesCheck[] is set to 1 to indicate  *
        ...
        outputTes.SetValue(
            indexX, indexY, indexZ, startSeries,
            splitTes[i].GetValue(indexX, indexY, indexZ, splitIndex));
      }
      CLOSE_SPATIAL_LOOPS
      if (moveParams) {
        VB_Vector pVec;
         ...
         *********************************************************************/
        for (size_t j = 0, k = (int)strtol(S[6]);
             j < (pVec.size() / MOVE_PARAMS); j++, k += numBins)
          memcpy(paramValues + (k * MOVE_PARAMS),
                 pVec.getData() + (j * MOVE_PARAMS),
                 sizeof(double) * MOVE_PARAMS);
      }
    } else {
      ostringstream errorMsg;
  ...
  for (int i = 0; i < origDimT; i++) {
    if (tSeriesCheck[i] == 0) {
      ostringstream errorMsg;
      errorMsg << " Did not find the time series number [" << i
               << "]. Invalid join.";
      printErrorMsgAndExit(VB_ERROR, errorMsg.str(), __LINE__, __FUNCTION__,
                           __FILE__, 1);
    }
  ...
  addHeaderLine(&outputTes, "Sfff", "VoxSizes(XYZ):", splitTes[0].voxsize[0],
                splitTes[0].voxsize[1], splitTes[0].voxsize[2]);
  addHeaderLine(&outputTes, "Siii", "Origin(XYZ):", splitTes[0].origin[0],
                splitTes[0].origin[1], splitTes[0].origin[2]);
  string timeStr;
  addHeaderLine(&outputTes, "SSS", "TesJoin:", timedate().c_str(),
                splitType.c_str());
  ...
  if (deleteFiles) {
    for (unsigned int i = 0; i < inputFiles.size(); i++) {
      ...
      if (unlink(inputFiles[i].c_str()) < 0) {
        ostringstream errorMsg;
        errorMsg << "Unable to delete file [" << inputFiles[i].c_str()
                 << "] due to: [" << strerror(errno) << "].";
        printErrorMsg(VB_WARNING, errorMsg.str());
      }
      if (moveParams) {
        string paramFile = inputFiles[i].substr(0, inputFiles[i].length() - 4) +
                           "_MoveParams.ref";
        errno = 0;
        if (unlink(paramFile.c_str()) < 0) {
          ostringstream errorMsg;
          errorMsg << "Unable to delete file [" << paramFile.c_str()
                   << "] due to: [" << strerror(errno) << "].";
          printErrorMsg(VB_WARNING, errorMsg.str());
        }
      }
    ...
    errno = 0;
    if ((directoryName.size() > 0) && (rmdir(directoryName.c_str()) == -1) &&
        (errno != ENOTEMPTY)) {
      ostringstream errorMsg;
      errorMsg << "Unable to delete directory [" << directoryName
               << "] due to: [" << strerror(errno) << "].";
      printErrorMsg(VB_WARNING, errorMsg.str());
    }
  }
...
void usage(const unsigned short exitValue, char *progName) {
  genusage(
      exitValue, progName, "- Tes file joining routine for VoxBo.",
      "-h -d[directory] -f[file name] -l[file list] -o[output file name] -m -r "
      "-v",
      ...
      "                          NOTE: Exactly one of \"-d\", \"-f\", or "
      "\"-l\"",
      "                          is required.",
      "-o <output file name>     Specify the output file name. Optional.",
      "                          Default output file name will be the original",
      "                          file name.",
      "-m                        Join the movement parameters files as well. "
      "Optional.",
      "-r                        If used, then remove the input files. "
      "Optional.",
 ...
 *********************************************************************/
bool verifyFile(const string &fileName) {
  if (vb_fileexists(fileName) && !access(fileName.c_str(), R_OK)) return true;
  return false;
}
...
void getVec(const string &tesFile, const int dimT, VB_Vector &pVec) {
  string paramFile =
      tesFile.substr(0, tesFile.length() - 4) + "_MoveParams.ref";
  if (!access(paramFile.c_str(), R_OK)) {
    ...
    if ((listoftypes.size() == 0) || (listoftypes[0].getDimensions() != 1)) {
      ostringstream errorMsg;
      errorMsg << "The movement parameters file [" << paramFile
               << "] is not a VoxBo vector file.";
      printErrorMsgAndExit(VB_ERROR, errorMsg.str(), __LINE__, __FUNCTION__,
                           __FILE__, 1);
    }
    ...
    if ((dimT * MOVE_PARAMS) != (int)(pVec.size())) {
      ostringstream errorMsg;
      errorMsg << "Tes file [" << tesFile
               << "] does not correspond to movement parameters file ["
               << paramFile << "].";
      printErrorMsgAndExit(VB_ERROR, errorMsg.str(), __LINE__, __FUNCTION__,
                           __FILE__, 1);
    }
...
void writeMoveParamsFile(const double *paramValues, const string &outputFile,
  ...
  string outputParamFile =
      outputFile.substr(0, outputFile.size() - 4) + "_MoveParams.ref";
  errno = 0;
  FILE *fVec = fopen(outputParamFile.c_str(), "w");
  if (!fVec) {
    ostringstream errorMsg;
    errorMsg << "Unable to open REF1 output file [" << outputParamFile
             << "] for write due to [" << strerror(errno) << "].";
    printErrorMsgAndExit(VB_ERROR, errorMsg.str(), __LINE__, __FUNCTION__,
                         __FILE__, 1);
  }
  string firstMoveParamsFile =
      firstSplitFile.substr(0, firstSplitFile.length() - 4) + "_MoveParams.ref";
  errno = 0;
  FILE *pFile = fopen(firstMoveParamsFile.c_str(), "r");
  if (!pFile) {
    ostringstream errorMsg;
    errorMsg << "Unable to open first split movement parameters file ["
             << firstMoveParamsFile << "] for reading due to ["
             << strerror(errno) << "].";
    printErrorMsgAndExit(VB_ERROR, errorMsg.str(), __LINE__, __FUNCTION__,
                         __FILE__, 1);
  }
...
string getLastTesSplitLine(const Tes &theTes) {
  ...
  if (tesSplitLine.length() == 0) {
    ostringstream errorMsg;
    errorMsg << "The file [" << theTes.filename
             << "] is not a split 4D data file.";
    printErrorMsgAndExit(VB_ERROR, errorMsg.str(), __LINE__, __FUNCTION__,
                         __FILE__, 1);
  }
