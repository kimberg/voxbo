  ...
  if ( this->currentSeries < (unsigned long) this->theTes.voxels) {
    ...
    *********************************************************************/
    if (this->theTes.mask[this->currentSeries]) {
      theTes.getXYZ(this->indexX,this->indexY,this->indexZ,this->currentSeries); //TGK change
      if (this->theTes.GetTimeSeries(this->indexX, this->indexY, this->indexZ))
        return 0;
      memcpy(tSeries, this->theTes.timeseries.getData(), this->dimT * sizeof(double));
    } 
    else
...
unsigned int TimeSeries::getDimX() const {
  return this->dimX;
} 

unsigned int TimeSeries::getDimY() const {
  return this->dimY;
} 

unsigned int TimeSeries::getDimZ() const {
  return this->dimZ;
} 

unsigned int TimeSeries::getDimT() const {
  return this->dimT;
} 

unsigned int TimeSeries::getIndexX() const {
  return this->indexX;
}

unsigned int TimeSeries::getIndexY() const {
  return this->indexY;
} 

unsigned int TimeSeries::getIndexZ() const {
  return this->indexZ;
} 
...
void TimeSeries::zeroOut(const unsigned int x, const unsigned int y, unsigned const int z) throw () {
...
const unsigned int yIndex, const unsigned int zIndex) throw () {
  this->getTimeSeries(tSeries, this->theTes.voxelposition(xIndex,
  yIndex, zIndex));
} 
...
void TimeSeries::zeroOut(const unsigned long index) throw() {
  ...
  *********************************************************************/
  if ( (index < (unsigned long) this->theTes.voxels) && (this->theTes.mask[index]) ) 
