...
void cr8SubSeries(int begin, int end, const Tes& inputTes, Tes& outputTes,
...
int main(int argc, char* argv[]) {
  ...
  processOpts(argc, argv, ":hi:o:re:d:b:f:v", "bZZbZiiZb", &printHelp, &tesFile,
              &outFileStem, &deleteInputFile, &extractionRange, &divSize,
   ...
   *********************************************************************/
    ...
    if (access(tesFile.c_str(), R_OK)) {
      ostringstream errorMsg;
      errorMsg << "The input 4D data file [" << tesFile
               << "] is not processes readable.";
    ...
    if (!validate4DFile(tesFile)) {
      ostringstream errorMsg;
      errorMsg << "The file [" << tesFile << "] is not a 4D VoxBo data file.";
      printErrorMsgAndExit(VB_ERROR, errorMsg.str(), __LINE__, __FUNCTION__,
                           __FILE__, 1);
    }  // if
  ...
  if ((extractionRange.size() == 0) && (divSize <= 0) && (numBins <= 0)) {
    ostringstream errorMsg;
    errorMsg
        << "Must select at least one way of generating a time series sub-set.";
    printErrorMsg(VB_ERROR, errorMsg.str(), __LINE__, __FUNCTION__, __FILE__);
    usage(1, argv[0]);
  }  // if
  ...
  if ((outputDir.size() > 0) && access(outputDir.c_str(), F_OK)) {
    ...
    if (!vb_direxists(tempDirName)) {
      ...
      if (mkdir(tempDirName.c_str(), 0776) == -1) {
        ostringstream errorMsg;
        errorMsg << "Error in creating directory [" << tempDirName
    ...
    if (access(tempDirName.c_str(), W_OK)) {
      ostringstream errorMsg;
      errorMsg << "The directory [" << tempDirName
               << "] is not writeable by this process.";
      printErrorMsgAndExit(VB_ERROR, errorMsg.str(), __LINE__, __FUNCTION__,
                           __FILE__, 1);
    }  // if
  ...
  if (access(xdirname(outFileStem).c_str(), W_OK)) {
    ostringstream errorMsg;
    errorMsg << "Effective user id [" << geteuid()
             << "] does not have write permission in the directory ["
  ...
  if (splitCount != joinCount) {
    ostringstream errorMsg;
    errorMsg << "The VoxBo 4D data file [" << inputTes.filename
             << "] has not been split [" << splitCount << "] and joined ["
             << joinCount << "] the same number of times.";
    printErrorMsgAndExit(VB_ERROR, errorMsg.str(), __LINE__, __FUNCTION__,
                         __FILE__, 1);
  }  // if
  ...
  if (extractionRange.size() > 0) {
    ...
    if (!validateString(pattern, extractionRange.c_str())) {
      ostringstream errorMsg;
      errorMsg << "Extraction range must be of the form positive integer, "
                  "dash, positive integer. Example: 23-45.";
      printErrorMsgAndExit(VB_ERROR, errorMsg.str(), __LINE__, __FUNCTION__,
                           __FILE__, 1);
    }  // if
    ...
    if (beginExt > endExt) {
       ...
       *********************************************************************/
      ostringstream errorMsg;
      errorMsg << "The start of the extraction [" << beginExt
               << "] must be <= to the end of the extraction [" << endExt
               << "].";
      printErrorMsgAndExit(VB_ERROR, errorMsg.str(), __LINE__, __FUNCTION__,
                           __FILE__, 1);
    ...
    if (endExt >= dimT) {
       ...
       *********************************************************************/
      ostringstream errorMsg;
      errorMsg << "The end of the extraction [" << endExt
               << "] must be < dimT [" << dimT << "].";
      printErrorMsgAndExit(VB_ERROR, errorMsg.str(), __LINE__, __FUNCTION__,
                           __FILE__, 1);
     ...
     *********************************************************************/
    addHeaderLine(outTes, "Sfff", "VoxSizes(XYZ):", inputTes.voxsize[0],
                  inputTes.voxsize[1], inputTes.voxsize[2]);
    addHeaderLine(outTes, "Siii", "Origin(XYZ):", inputTes.origin[0],
                  inputTes.origin[1], inputTes.origin[2]);
     ...
     *********************************************************************/
    addHeaderLine(outTes, "SSSSSi", "TesSplit:", "Extraction",
                  timedate().c_str(), tesFile.c_str(), extractionRange.c_str(),
                  dimT);
  ...
  if (divSize > 0) {
    ...
    if (divSize <= 0) {
      ostringstream errorMsg;
      errorMsg << "The division size [" << divSize << "] must be > 0.";
      printErrorMsgAndExit(VB_ERROR, errorMsg.str(), __LINE__, __FUNCTION__,
                           __FILE__, 1);
    }  // if
    ...
    for (int i = 0; (i + divSize) <= dimT; i += divSize, numFile++) {
      ...
      addHeaderLine(outTes, "SSSSSi", "TesSplit:", "Divided",
                    timedate().c_str(), tesFile.c_str(), numString, dimT);
  ...
  if (numBins > 0) {
    ...
    if (numBins > dimT) {
      ostringstream errorMsg;
      errorMsg << "numBins [" << numBins
               << "] must be less than the time dimension: [" << dimT << "]";
      printErrorMsgAndExit(VB_ERROR, errorMsg.str(), __LINE__, __FUNCTION__,
                           __FILE__, 1);
    }  // if
    ...
    for (int indexBin = 0; indexBin < numBins; indexBin++) {
      ...
      if (indexBin < leftOver) {
        outTesArr[indexBin].SetVolume(inputTes.dimx, inputTes.dimy,
                                      inputTes.dimz, numSeries + 1,
                                      inputTes.datatype);
      }  // if
      else {
        outTesArr[indexBin].SetVolume(inputTes.dimx, inputTes.dimy,
                                      inputTes.dimz, numSeries,
                                      inputTes.datatype);
      }  // else
       ...
       * outTesArr[indexBin].                                               *
       *********************************************************************/
      for (int indexOut = 0; indexOut < outTesArr[indexBin].dimt;
           indexOut++, j += numBins) {
        /*********************************************************************
        ...
        outTesArr[indexBin].SetValue(
            indexX, indexY, indexZ, indexOut,
            inputTes.GetValue(indexX, indexY, indexZ, j));
       ...
       *********************************************************************/
      addHeaderLine(outTesArr[indexBin], "Sfff",
                    "VoxSizes(XYZ):", inputTes.voxsize[0], inputTes.voxsize[1],
                    inputTes.voxsize[2]);
      addHeaderLine(outTesArr[indexBin], "Siii",
                    "Origin(XYZ):", inputTes.origin[0], inputTes.origin[1],
                    inputTes.origin[2]);
       ...
       * NOTE: Each of the above fields will be delimited by tabs.          *
       *********************************************************************/
      addHeaderLine(outTesArr[indexBin], "SSSSiii", "TesSplit:", "Binned",
                    timedate().c_str(), tesFile.c_str(), numBins, dimT,
                    startSeries);
      ...
      if (outTesArr[indexBin].WriteFile()) {
        ostringstream errorMsg;
        errorMsg << "Unable to write Tes file [" << outTesArr[indexBin].filename
                 << "].";
        printErrorMsgAndExit(VB_ERROR, errorMsg.str(), __LINE__, __FUNCTION__,
                             __FILE__, 1);
      }  // if
  ...
  if (deleteInputFile) {
    ...
    if (unlink(tesFile.c_str()) < 0) {
       ...
       *********************************************************************/
      ostringstream errorMsg;
      errorMsg << "Error in removing file [" << tesFile
               << "] due to: " << strerror(errno);
      printErrorMsg(VB_ERROR, errorMsg.str(), __LINE__, __FUNCTION__, __FILE__);
...
void usage(const unsigned short exitValue, char* progName) {
  printf("\nVoxBo tesplit (v%s)\n", vbversion.c_str());
  genusage(
      exitValue, progName, "- 4D data file splitting routine for VoxBo.",
      ...
      "-r                        If used, then remove the input file. "
      "Optional.",
      ...
      "                          output file may have fewer time series than "
      "divisor.",
      "-b <bins>                 Each \"bin\" gets a time series until the "
      "time",
      "                          series are exhausted. Using \"-b 2\" results",
      "                          in an even/odd split, resulting in 2 output "
      "files.",
      "                          NOTE: At least one of -e, -d, and -b is "
      "required.",
      ...
      "                          it will be created and it will be a "
      "sub-directory",
      "                          of the directory containing the input 4D data "
      "file.",
      "                          NOTE: This option overrides -o.",
      "-v                        Global VoxBo version number. Optional.",
      "                          NOTE: Previously split files are not allowed "
      "to",
...
void cr8SubSeries(int begin, int end, const Tes& inputTes, Tes& outputTes,
  ...
  if (begin < 0) {
    ostringstream errorMsg;
    errorMsg << "The begin point [" << begin << "] must be non-negative.";
    printErrorMsgAndExit(VB_ERROR, errorMsg.str(), __LINE__, __FUNCTION__,
                         __FILE__, 1);
  }  // if
  ...
  if (end >= inputTes.dimt) {
    ostringstream errorMsg;
    errorMsg << "The end point [" << end
             << "] must be less than the time dimension: [" << inputTes.dimt
             << "].";
    printErrorMsgAndExit(VB_ERROR, errorMsg.str(), __LINE__, __FUNCTION__,
                         __FILE__, 1);
  }  // if
   ...
   * data type.                                                         *
   *********************************************************************/
  outputTes.SetVolume(inputTes.dimx, inputTes.dimy, inputTes.dimz, newDimT,
                      inputTes.datatype);
