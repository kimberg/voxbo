...
}  // void genusage(const unsigned short exitValue, char *progName, const char
   // *desc, const char *basicInfo, ...)
    ...
    if (optInfoArr[i].optPresent) {
      ...
      switch (optInfoArr[i].typeArg) {
        ...
        case 'c':
           ...
           * appropriate pointer to one of the char pointers from the * variable
           *list of parameters passed to this function and it gets    * stored
           *in c_char. Then c_char is assigned the value from           *
        ...
        case 'u':
          ...
          (*u_unsigned_short) =
              (unsigned short)strtol(optInfoArr[i].optArg.c_str());
          break;
        ...
        case 'i':
           ...
           * appropriate pointer to one of the int pointers from the * variable
           *list of parameters passed to this function and it gets    * stored
           *in i_int. Then i_int is assigned the value from             *
           * optInfoArr[i].optArg[0]. *
           *********************************************************************/
          i_int = va_arg(ap, int *);
        ...
        case 'I':
          ...
          (*I_unsigned_int) =
              (unsigned int)strtol(optInfoArr[i].optArg.c_str());
          break;
        ...
        case 'l':
           ...
           * appropriate pointer to one of the long pointers from the * variable
           *list of parameters passed to this function and it gets    * stored
           *in l_long. Then l_long is assigned the value from           *
           * optInfoArr[i].optArg[0]. *
           *********************************************************************/
        ...
        case 'L':
          ...
          (*L_unsigned_long) =
              (unsigned long)strtol(optInfoArr[i].optArg.c_str());
          break;
        ...
        case 'd':
          ...
          /*********************************************************************
           * This case block is for double. va_arg(ap, double *) will return
           *the* appropriate pointer to one of the double pointers from the *
           * variable list of parameters passed to this function and it gets *
        ...
        default:
          ostringstream errorMsg;
          errorMsg << "FUNCTION [" << __FUNCTION__
                   << "]. Unknown primitive type for processOpts: ["
                   << optInfoArr[i].typeArg << "] in option number [" << i
                   << "]. Option name: [" << optInfoArr[i].optName
                   << "] Exiting.";
...
int getTempFileName(char *tempFile, const size_t length) {
  ...
  if (strcmp(last6, "XXXXXX") != 0) {
     ...
     *********************************************************************/
      ...
      errorMsg << "Line Number [" << __LINE__ << "] File: [" << __FILE__
               ...
               << tempFile
               << "] for temporary file name. insufficient space to append "
                  "\"XXXXXX\".";
      printErrorMsg(VB_ERROR, errorMsg.str());
      return -1;
    }  // else
  ...
  if (fd == -1) {
    ...
    errorMsg << "Line Number [" << __LINE__ << "] File: [" << __FILE__
             ...
             << "] Unable to create temporary file name from tempFile ["
  ...
  if (unlink(tempFile) == -1) {
    ...
    errorMsg << "Line Number [" << __LINE__ << "] File: [" << __FILE__
             << "] Function: [" << __FUNCTION__
             << "] Unable to delete temporary file from tempFile [" << tempFile
