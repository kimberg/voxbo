...
void printHelp() {
  ...
  printf(
      "  Convert a movement parameter ref file to a new file with 7 elements "
      "per line\n\n");
  printf("usage:\n");
  printf("  sortmvpm <input mvpm filename> <output filename>\n");
  printf(
      "        <input mvpm filename>:  original movement parameter ref file\n");
  printf(
      "            <output filename>:  new file that has 7 elements per "
      "line\n");
}
...
int chkInFile(const char *fileName) {
  VB_Vector testVec;
  if (testVec.ReadFile((string)fileName)) return 1;

  int vecSize = testVec.getLength();
  if (vecSize % 7 == 0) return 0;
  return 2;
}
