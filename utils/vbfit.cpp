...
void checkLength2(size_t newLen, size_t len1, const char *newStr,
                  const char *str1);

/* Functions for interactive mode, commented out now because it is not being
 * used */
// void interactive();
// string readFitFile();
...
int main(int argc, char *argv[]) {
  ...
  // "-r" plus "-o" option: when both reference function and output filename are
  // given
  if (argc >= 8 && dancmp(argv[3], "-r") && dancmp(argv[argc - 2], "-o")) {
    int refIndex = 4;
    for (int m = refIndex; m < argc - 2; m++) checkInFile(argv[m]);
    ...
    fitOneOverF(psVec, refFunc, TRin, sigma, var3min, var1, var2, var3,
                outputFile);
  }

  // only "-r" option: when reference function is defined in arguments
  else if (argc >= 5 && dancmp(argv[2], "-r") &&
           !dancmp(argv[argc - 2], "-o")) {
    int refIndex = 4;
    for (int m = refIndex; m < argc; m++) checkInFile(argv[m]);
  ...
  // only "-o" option: when output file is defined in arguments
    ...
    int psIndex = 3;
    for (int m = psIndex; m < argc - 2; m++) checkInFile(argv[m]);
  ...
  /* generic case: when no "-i", "-r" or "-o" found, always treat second
   * argument as TR, others after as power spectrum filename */
  else if (argc >= 3 && !dancmp(argv[2], "-r") &&
           !dancmp(argv[argc - 2], "-o")) {
    int psIndex = 3;
    for (int m = psIndex; m < argc; m++) checkInFile(argv[m]);
...
/* This function checks whether totalReps is multiple of inout vector length or
 * not. */
void checkLength(double vecLength, double totalReps, const char *filename) {
  if ((int)totalReps % (int)vecLength) {
    printf(
        "totalReps (%d) is not a multiple of number of elements in PS file "
        "(%d):\n %s\n",
...
/* This function compares the first argument (newLen) with the second argument
 * (len1). If they are not equal, , print out error message and terminate the
 * whole process. The 3rd argument is filename whose # of element is newLen.
...
void printHelp() {
  ...
  printf("usage:\n");
  printf(
      "  vbfit <TR> <totalReps> [-r <refFunc>] <ps#1> [<ps#2> ...] [-o "
      "<output>]\n\n");

  printf("        <TR>:  time of repetition (unit: ms)\n");
  printf(
      " <totalReps>:  total # of image based on which power spectrum will be "
      "drawn\n");
  printf("   <refFunc>:  reference function file (optional)\n");
  printf(
      "      <ps#1>:  power spectrum file #1 (ps#2, ps#3 ... are optional)\n");
  printf(
      "    <output>:  filename where fitting parameters will be saved\n\
...
//   printf("Please type in the sigma values for the points fitted (default is
...
//   printf("Please type in the output filename (default is
//   fittingParams.ref):\n"); outputFile = readOutputFile(outputDefault);
//   curvefit(xFilename.data(), yFilename.data(), inputSigma, var3min,
//   var1_init,
...
//       inputString);
//     else if (fileStatus == 1) {
//       printf("%s exists. Do you want to overwrite this file? <y/n> ",
//       inputString); ovwString = getInputString().data();
...
//     printf("Please type in another filename (default is
//     fittingParams.ref):\n"); inputString = getInputString().data(); if
//     (inputString[0] == '\0')
//       inputString = outputDefault;
