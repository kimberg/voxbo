...
VecView::VecView(const char *inputFile, int inputMode, QWidget *parent,
                 const char *name)
...
VecView::VecView(VB_Vector *inputVec, bool mvpmFlag, QWidget *parent,
                 const char *name)
    : Q3MainWindow(parent, name) {
  init();
...
VecView::~VecView() {
  mainScreen->clear();
  if (colorList.size()) colorList.clear();
}
...
void VecView::vecShow(VB_Vector *inputVec) {
  ...
  setCommon();
  statusBar()->message(
      QString("Ref file length: %1").arg(inputVec->getLength()));
}
...
void VecView::matShow() {
  ...
  if (vecNum > 1)
    statusBar()->message(
        QString("Column #1 (G matrix dimension: %1 rows, %2 columns)")
            .arg(vecLen)
            .arg(vecNum));
  else
    statusBar()->message(
        QString("Column #1 (G matrix dimension: %1 rows, 1 column)")
            .arg(vecLen));
}
...
void VecView::setVecBox() {
  ...
  QObject::connect(vecSlider, SIGNAL(valueChanged(int)), this,
                   SLOT(changeVec(int)));

  showAll = new QCheckBox("Show All", vecBox);
  QObject::connect(showAll, SIGNAL(toggled(bool)), this,
                   SLOT(toggleShow(bool)));
  autoScale = new QCheckBox("Y Auto Scale", vecBox);
  autoScale->setChecked(true);
  QObject::connect(autoScale, SIGNAL(toggled(bool)), this,
                   SLOT(toggleScale(bool)));
}
  ...
  if (showStat) {
    ...
    for (int i = 0; i < (int)vecList.size(); i++) {
      ...
      if (plotStat == 2 && i != (int)vecList.size() - 1)
        statusTxt += "Column #" + QString::number(i + 1) + ": " +
                     QString(colorList(j)) + "; ";
      else if (plotStat == 2)
        statusTxt +=
            "Column #" + QString::number(i + 1) + ": " + QString(colorList(j));
    ...
    if (plotStat == 2 && vecNum > 1)
      statusBar()->message(
          QString("Column #%1 (G matrix dimension: %2 rows, %3 columns)")
              .arg(vecIndex + 1)
              .arg(vecLen)
              .arg(vecNum));
    else if (plotStat == 2)
      statusBar()->message(
          QString("Column #%1 (G matrix dimension: %2 rows, 1 column)")
              .arg(vecIndex + 1)
              .arg(vecLen));
    else
      statusBar()->message(
          QString("%1 length: %2").arg(refFiles(vecIndex)).arg(vecLen));
...
void VecView::mvpmShow(VB_Vector *inputVec) {
  ...
  QRadioButton *XYZ =
      new QRadioButton("X (green), Y (red), Z (yellow)", mpGroup);
  ...
  statusBar()->message(
      QString("Movement parameter component length: %1").arg(unitLength));
}
...
void VecView::setCommon() {
  ...
  /* Add "magnification" and "position" slide bars and a "center" button in the
   * middle */
  ...
  QObject::connect(magSlider, SIGNAL(valueChanged(int)), mainScreen,
                   SLOT(setXMag(int)));
  QObject::connect(mainScreen, SIGNAL(xMagChanged(int)), magSlider,
                   SLOT(setValue(int)));
...
/* This function reads the input matrix file and puts each column into vecList
 * array */
...
void VecView::togglePS(bool psStat) {
  ...
  if (psChecked && plotStat == 0) mainScreen->addVector(psList[0]);
...
void VecView::keyReleaseEvent(QKeyEvent *e) {
  if (e->key() == Qt::Key_Control) ctrlPressed = false;
}
...
int checkInFile(char *fileName, bool mvpmFlag) {
  ...
  if (!mvpmFlag) {
    if (testVec.ReadFile((string)fileName) == 0)
      return 1;
    else if (testMat.ReadHeader((string)fileName) == 0 && testMat.m &&
             testMat.n)
      return 2;
    else
      return -1;
...
void vecview_help() { cout << boost::format(myhelp) % vbversion; }

void vecview_version() { printf("VoxBo vecview (v%s)\n", vbversion.c_str()); }
