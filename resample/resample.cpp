...
int main(int argc, char *argv[]) {
  ...
  if (files.size() == 1) files.Add("r" + files[0]);
  if (files.size() != 2) {
  ...
  if (q_xxflag) rr.SetXX(x1, xstep, nx);
  if (q_yyflag) rr.SetYY(y1, ystep, ny);
  if (q_zzflag) rr.SetZZ(z1, zstep, nz);
  Cube newcube;
  if (ts.data) {
    Tes newtes;
    for (int i = 0; i < ts.dimt; i++) {
      ...
      if (i == 0) {
        newtes.SetVolume(newcube.dimx, newcube.dimy, newcube.dimz, ts.dimt,
                         newcube.datatype);
...
void resample_help() {
  ...
  printf(
      "   -rw <ref>      aligns to ref on all dims, using corner position, "
      "same voxel sizes\n");
