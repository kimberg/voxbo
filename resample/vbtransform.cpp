...
int main(int argc, char *argv[]) {
  ...
  VBTransform *r =
      new VBTransform(argc, argv);  // init resample object, parse args
  if (!r) {
  ...
  if (!err) err = r->VBTransformFile();  // do it
  delete r;                              // clean up
  if (err)
...
int VBTransform::Transform3D() {
  stringstream tmps;

  if (VBTransformCube()) return 101;
...
int VBTransform::NNResampleCube() {
  int i, j, k;
  if (!mycube) return 5;
  newcube = new Cube();
...
int VBTransform::SincVBTransformCube() {
  int i, j, k;
