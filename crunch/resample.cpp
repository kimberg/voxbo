...
int main(int argc, char *argv[]) {
  ...
  if (!err) err = r->ResampleFile();  // do it
  delete r;                           // clean up
  if (err)
...
int Resample::SetAlign(const string mode, const string ref) {
  Cube refcube;
  if (refcube.ReadFile(refname)) return 100;
  if (mode == "-aa" || mode == "-ax") {
    if (ref == "origin")
      x1 = (float)mycube->origin[0] -
           (((float)refcube.origin[0] * refcube.voxsize[0]) /
            mycube->voxsize[0]);
  }
  if (mode == "-aa" || mode == "-ay") {
    if (ref == "origin")
      y1 = (float)mycube->origin[1] -
           (((float)refcube.origin[1] * refcube.voxsize[1]) /
            mycube->voxsize[1]);
  }
  if (mode == "-aa" || mode == "-az") {
    if (ref == "origin")
      z1 = (float)mycube->origin[2] -
           (((float)refcube.origin[2] * refcube.voxsize[2]) /
            mycube->voxsize[2]);
  }
  return 0;
}

int Resample::UseCorner(const string &refname) {
  Cube refcube;
  stringstream tmps;
  ...
  if (ourline.size() != 3) {
    tmps.str("");
    tmps << "resample: input image file doesn't have an absolute corner "
            "position tag";
  ...
  if (refline.size() != 3) {
    tmps.str("");
    tmps << "resample: reference image file doesn't have an absolute corner "
            "position tag";
    printErrorMsg(VB_ERROR, tmps.str());
    return 101;
  }
...
int Resample::UseZ(const string &refname, double zsize) {
  CrunchCube *refcube;
...
int Resample::UseDims(const string &refname) {
  CrunchCube *refcube;
  refcube = new CrunchCube;
  refcube->ReadFile(refname);
  if (!refcube->data_valid) {
...
int Resample::UseTLHC(const string &refname) {
  CrunchCube *refcube;
...
int Resample::Resample3D() {
  stringstream tmps;

  if (ResampleCube()) return 101;
        ...
        switch (mycube->datatype) {
          case vb_byte:
            resample_sinc(1, (unsigned char *)mycube->data, out, c1, c2, c3,
                          mycube->dimx, mycube->dimy, mycube->dimz, 5, 0.0,
                          1.0);
            break;
          case vb_short:
            ...
            break;
          case vb_long:
            resample_sinc(1, (int *)mycube->data, out, c1, c2, c3, mycube->dimx,
...
void resample_help() {
  ...
  printf(
      "   -ra <ref>      aligns to ref on all dims, using corner position\n");
  printf("   -d  <x y z>    desired dimensions\n");
  printf(
      "   -nn            use nearest neighbor interpolation (default is "
      "sinc)\n");
  printf(
      "   -xx <a b c>    a=start voxel (from 0); b=interval in voxels; c=# of "
      "voxels\n");
  ...
  printf(
      "  invert the order of the slices: resample in.cub out.cub -zz 20 -1 "
      "21\n");
  printf("  swap left-right: resample in.cub out.cub -xx 63 -1 64\n");
  printf(
      "  interpolate in-plane: resample in.cub out.cub -xx 0 .25 256 -yy 0 .25 "
      "256\n");
