...
int GLMConfig::ParseFile(string filename) {
  ifstream infile;
  char buf[BUFLEN];
  tokenlist args;
  stringstream tmps;
  ...
  while (infile.getline(buf, BUFLEN, '\n')) {
    ...
    if (args.size()) {
      ...
      if (args[0] == "name" || args[0] == "glm") {
        ...
        if (args.size() > 0) gp->middles = args[1];
        ...
        if (args[1] == "yes" || args[1] == "1")
          ...
          tmps.str("");
          tmps << "vbmakeglm: unrecognized value for audit flag (should be "
                  "yes/no)";
          printErrorMsg(VB_ERROR, tmps.str());
        }
        ...
        if (args[1] == "yes" || args[1] == "1") {
          ...
          tmps.str("");
          tmps << "vbmakeglm: unrecognized value for driftcorrect flag (should "
                  "be yes/no)";
          printErrorMsg(VB_ERROR, tmps.str());
        }
      } else if (args[0] == "email") {
        gp->emailflag = 1;
        if (args.size() == 2) gp->email = args[1];
            ...
            if (!gp->CreateGLMDir()) {
              if (f_setuponly)
                ...
                if (gp->seq.Submit(vbp))
                  ...
                  cout << "[I] vbmakeglm: your glm sequence was submitted "
                          "successfully\n";
              }
            }
          }
        ...
        if (args.size() == 2) ParseFile(args[1]);
      } else {
        tmps.str("");
...
void vbmakeglm_help() {
  ...
  printf("notes:\n");
  printf(
      "  If --run is specified without =n, the default is the total number "
      "of\n");
...
void vbmakeglm_sample(FILE *fp) {
  fprintf(fp,
          "###############################################################\n");
  fprintf(fp, "# Sample .glm file for VoxBo vbmakeglm.\n");
  fprintf(fp,
          "# At the top, put stuff that's common to all GLMS in this file.\n");
  fprintf(fp,
          "###############################################################\n");
  fprintf(fp, "\n");
  ...
  fprintf(fp, "highs 1\n\n");
  fprintf(fp,
          "# which random frequencies fo you want to filter out? (separate by "
          "spaces)\n");
  fprintf(fp, "# middles 15 16 21\n\n");
  fprintf(fp,
          "# how many total data points per voxel (typically time points)?\n");
  fprintf(fp, "orderg 160\n\n");
  fprintf(
      fp,
      "# how many pieces do you want to break the matrix operations into?\n");
  ...
  fprintf(
      fp,
      "noisemodel /usr/local/VoxBo/elements/noisemodels/smooth_params.ref\n\n");
  fprintf(fp,
          "# specify the location of your G matrix, or just \"makerandfxg\" "
          "for makerandfxg\n");
  fprintf(fp, "gmatrix /data/Models/mytask.G\n");
  fprintf(fp, "# makerandfxg\n\n");
  fprintf(fp,
          "# the location of a reference function to copy into the GLM "
          "directory\n");
  fprintf(fp, "refname /data/Models/motor.ref\n\n");
  fprintf(fp,
          "# priority (1 for overnight, 2 for low, 3 for normal, 4 and 5 for "
          "various emergencies)\n");
  fprintf(fp, "pri 3\n\n");
  fprintf(fp, "# do some summary statistics on your GLM?\n");
  fprintf(fp, "audit yes\n\n");
  fprintf(fp,
          "# mean normalize?  usually say yes only if you have multiple runs "
          "of BOLD data\n");
