...
CrunchCube *dan_write_sn(CrunchCube *VOL, Matrix &affine, Matrix &dims,
                         ...
                         RowVector &Vox, int Hold, int affdefault) {
  Matrix tx, ty, tz;
  ...
  for (i = 0; i < x.length(); i++) x(i) = (x(i) / dims(2, 0)) + dims(3, 0);

  y = dan_span(bb(0, 1), bb(1, 1), Vox(1));
  for (i = 0; i < y.length(); i++) y(i) = (y(i) / dims(2, 1)) + dims(3, 1);

  z = dan_span(bb(0, 2), bb(1, 2), Vox(2));
  for (i = 0; i < z.length(); i++) z(i) = (z(i) / dims(2, 2)) + dims(3, 2);
  ...
  for (j = 0; j < z.length(); j++) {
    ...
    for (k = 0; k < X2.rows(); k++) {
      for (l = 0; l < X2.cols(); l++) {
        if ((X2(k, l) >= 1 - TINY) && (X2(k, l) <= (VOL->dimx + TINY)) &&
            (Y2(k, l) >= 1 - TINY) && (Y2(k, l) <= (VOL->dimy + TINY)) &&
            (Z2(k, l) >= 1 - TINY) && (Z2(k, l) <= (VOL->dimz + TINY)))
          Mask(k, l) = 1;
        else
          Mask(k, l) = 0;
      }
    ...
    for (k = 0; k < d.rows(); k++)
      for (l = 0; l < d.cols(); l++) d(k, l) *= Mask(k, l);
    dv = vectorize(d);
