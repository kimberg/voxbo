...
int get_datasize(int type) {
  ...
  else if (type == vb_short)
    return (sizeof(short));
  else if (type == vb_long)
    return (sizeof(int));
  else if (type == vb_float)
    return (sizeof(float));
  else if (type == vb_double)
...
void print(const RowVector &vec, int size) {
  print("Anonymous rowvector", vec, size);
}

void print(const char *s, const Matrix &mat, int size) {
  int i, j;
...
void print(const char *s, const RowVector &vec, int size) {
  int i, j;

  j = 0;
  if (s != NULL) cerr << endl << s << " (" << vec.length() << "):" << endl;
...
void print(const char *s, const ColumnVector &vec, int size) {
  int i, j;
