...
int Perfmask::Go() {
  if (args.size() < 3) {
    printf(
        "*** perfmask: requires segmentation, reference epi, and output "
        "filename\n");
    return 110;
  }
...
void Perfmask::SelectRegions(CrunchCube *rcube) {
  ...
  for (i = 0; i < rcube->dimx; i++) {
    for (j = 0; j < rcube->dimy; j++) {
      for (k = 0; k < rcube->dimz; k++) {
        ...
        if (val != 0.0) {
          Region rg;
          growregion(rg, *rcube, i, j,
                     k);  // clears the voxels as a side effect!
  ...
  for (i = 0; i < (int)regionlist.size(); i++) {
    ...
    for (j = 0; j < (int)regionlist[i].vox.size(); j++) {
      rcube->SetValue(regionlist[i].vox[j].x, regionlist[i].vox[j].y,
                      regionlist[i].vox[j].z, 1.0);
    }
  }
}
  ...
  for (i = x - 1; i < x + 2; i++) {
    ...
    for (j = y - 1; j < y + 2; j++) {
      ...
      for (k = z - 1; k < z + 2; k++) {
        ...
        if (cb.GetValue(i, j, k)) growregion(reg, cb, i, j, k);
      }
    }
  }
}

int Perfmask::UseDims(const string &refname) {
  CrunchCube *refcube;
        ...
        switch (incube->datatype) {
          case vb_byte:
            resample_sinc(1, (unsigned char *)incube->data, out, c1, c2, c3,
                          incube->dimx, incube->dimy, incube->dimz, 5, 0.0,
                          1.0);
            break;
          case vb_short:
            ...
            break;
          case vb_long:
            resample_sinc(1, (int *)incube->data, out, c1, c2, c3, incube->dimx,
                          incube->dimy, incube->dimz, 5, 0.0, 1.0);
            break;
          case vb_float:
...
void perfmask_help() {
  printf("\nVoxBo perfmask (v%s)\n", vbversion.c_str());
  printf(
      "summary: create a mask from a segmentation in the space of an epi "
      "image\n");
