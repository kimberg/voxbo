...
int VBQA::Go(tokenlist &args) {
  ...
  outputname = xstripwhitespace(outputname,
                                "_");  // strip leading and trailing underscores

  // if we're deleting, just delete
  if (deleteflag) {
    for (i = 0; i < (int)deletelist.size(); i++) {
      sprintf(tmp, "rm -r %s/.voxbo/reports/%s", vbp.homedir.c_str(),
              deletelist[i].c_str());
      system(tmp);
    }
    vb_buildindex();
...
void VBQA::ShowNormSlices(vector<string> dirlist) {
  QWidget w;
  int j, interval;
  glob_t gb;
  char fname[512];
  ...
  for (size_t i = 0; i < vg.size();
       i++) {  // should only be one, but what the heck
    cube.ReadFile(vg[i]);
  ...
  // FIXME should also show some slices from the template, to see how things
  // went
}

void VBQA::PlotMoveParams(vector<string> dirlist) {
  ...
  for (i = 0; i < dirlist.size(); i++) {
    ...
    // found no data?
    if (!allmoveparams.getLength()) continue;
    ...
    string subtitle =
        (string) "movement parameters from directories below <b>" +
        (string)dirlist[i] + (string) "</b>";
    ...
    if (multiples) {
      ...
      outfile << "Note that " << multiples << " of your images "
              << (multiples == 1 ? "was" : "were")
              << " corrected more than once." << endl;
      outfile << "</p>" << endl;
    }
...
void VBQA::GenerateMovementWarnings(vector<string> dirlist) {}

void VBQA::PlotGlobalSignal(vector<string> dirlist) {
  VB_Vector allgs;
  size_t i, j;
  char fname[STRINGLEN];
  ...
  for (i = 0; i < dirlist.size(); i++) {
    ...
    string subtitle = (string) "global signal from files a level below <b>" +
                      (string)dirlist[i] + (string) "</b>";
    boxit(outfile, "Global Signal", subtitle);

    outfile << "<pre>from below directory: " << dirlist[i] << "</pre><br>"
            << endl;
    outfile << "<p>" << endl;
    outfile << "Here is the global signal from all your processed 4D files, "
               "concatenated"
            << endl;
    ...
    for (j = 0; j < (int)allgs.getLength(); j++) {
      if (allgs[j] > high || allgs[j] < low) spikes.push_back(j);
      if (fabs(allgs[j]) < .001) lowvals.push_back(j);
    }
    if (spikes.size()) {
      outfile << "<p>" << endl;
      outfile << "<font size=+1 color=red><b>Warning</b></font>" << endl;
      outfile << "We found " << spikes.size()
              << " spikes in your data, using a criterion of 3.5sd," << endl;
      outfile << "at the following time points (counting from 0): " << endl;
      outfile << spikes[0];
      for (j = 1; j < (int)spikes.size(); j++) outfile << ", " << spikes[j];
      outfile << "\n</p>" << endl;
    }
    if (lowvals.size()) {
      outfile << "<p>" << endl;
      outfile << "<font size=+1 color=red><b>Warning</b></font>" << endl;
      outfile << "We found " << lowvals.size()
              << " values in your data below an absolute value of .001,"
              << endl;
      outfile << "at the following time points (counting from 0): " << endl;
      for (j = 0; j < (int)lowvals.size(); j++) outfile << lowvals[j] << " ";
      outfile << "It might not be an issue, but you might want to make sure "
                 "data didn't get zeroed out somehow."
              << endl;
      outfile << "</p>" << endl;
    }
...
void VBQA::CheckGLMInfo(vector<string> dirlist) {
  size_t i, j;
  ifstream infofile;
  struct stat st;
  ...
  for (i = 0; i < (int)dirlist.size(); i++) {
    ...
    for (j = 0; j < vg.size(); j++) {
      ...
      if (stat(prmname.c_str(), &st)) continue;
      infofile.open(vg[j], ios::in);
      if (infofile) {
        subtitle = (string) "info file from directory <b>" +
                   (string)gb.gl_pathv[j] + (string) "</b>";
        ...
        if (collinearitywarnings)
          outfile << "<font size=+1 color=red><b>Warning: "
                  << collinearitywarnings
                  << " of your variables had Rmul values above 0.5</b></font>"
                  << endl;
      }
    }
  }
...
void VBQA::BuildDirList(string dir, int level) {
  // first add it
...
void VBQA::ExamineTesFiles(vector<string> dirlist) {
  ...
  for (i = 0; i < dirlist.size(); i++) vg.append(dirlist[i] + "/*.tes");
  ...
  for (i = 0; i < vg.size(); i++) {
    ...
    sprintf(tmp, "voxsize(%.4f,%.4f,%.4f)", mytes.voxsize[0], mytes.voxsize[1],
            mytes.voxsize[2]);
    mysteps.push_back(tmp);
    ...
    for (j = 0; j < (int)mytes.header.size(); j++) {
      ...
      if (args[0] == "PulseSeq:") {
        ...
        mysteps.push_back(tmp);
      } else if (args[0] == "Realigned" && args[1] == "by" &&
                 args[2] == "realign:")
        ...
        mysteps.push_back(tmp);
      } else if (args[0] == "Normalized" && args[1] == "by" &&
                 args[2] == "norm:")
        mysteps.push_back("norm");
      else if (args[0] == "SpatialSmooth:") {
        sprintf(tmp, "smooth(%s,%s,%s)", args[2].c_str(), args[3].c_str(),
                args[4].c_str());
        mysteps.push_back(tmp);
      }
    }
  ...
  outfile << "<p>" << endl;
  outfile << "The TES files listed below have been grouped according to various"
          << endl;
  outfile << "characteristics, including their dimensions and some of the "
             "processing steps"
          << endl;
  outfile << "applied to them.  Files that have been grouped together" << endl;
  outfile << "are not guaranteed to have been processed identically, but files "
             "grouped separately"
          << endl;
  outfile << "almost certainly differ in some way." << endl;
  outfile
      << "So you might be concerned if you see any oddballs, files that should"
      << endl;
  outfile
      << "have been processed just like the rest but for some reason weren't."
      << endl;
  outfile
      << "Or if you half your subjects were processed one way, half another."
      << endl;
  outfile << "Note that this program only knows about certain things, mostly "
             "the standard"
          << endl;
  outfile << "set of VoxBo preprocessing steps.  So you still need to be "
             "careful, even."
          << endl;
  outfile << "if all your TES files end up in the expected number of groups."
          << endl;
  outfile << "</p>" << endl;
  ...
  for (i = 0; i < pplist.size(); i++) {
    ...
    outfile << "<tr><td colspan=2 bgcolor=#D0D0FF><center><b>Group " << i + 1
            << "</b></center></td></tr>" << endl;
    outfile << "<tr><td "
               "bgcolor=#D0D0D0><center><b>Characteristics</b></center></td>"
            << endl;
    outfile << "<td bgcolor=#D0D0D0><center><b>Files</b></center></td></tr>"
            << endl;
...
void vbqa_help() {
  ...
  printf("summary:\n");
  printf(
      "  vbqa produces voxbo quality assurance reports in html format.  "
      "Point\n");
  printf(
      "  your web browser at HOME/.voxbo/reports/index.html to review your "
      "reports.\n");
  printf("usage:\n");
