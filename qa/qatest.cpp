...
int VBQA::Go(string data_dir, string output_dir) {
  datadir = data_dir;
...
void VBQA::GenerateMovementWarnings() {}

void VBQA::PlotGlobalSignal() {}

void VBQA::CopyGLMInfo() {}

void VBQA::RenderSlice(QWidget &w, Cube &c, int slice) {
  ...
  for (i = 0; i < cube.dimx; i++) {
    for (j = 0; j < cube.dimy; j++) {
      ...
      val = scaledvalue(minvalue, maxvalue, q_brightness, q_contrast,
                        cube.GetValue(i, j, q_slice));
      rr = gg = bb = val;
