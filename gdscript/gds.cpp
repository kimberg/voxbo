...
void scriptReader::makeAllG() {
  for (int i = 0; i < (int)sessionList.size(); i++) sessionList[i].writeG();
}
...
bool scriptReader::parseFile(string filename, int inLineNum) {
  ...
  while (infile.getline(buf, BUFLEN, '\n')) {
    lineNum++;
    args.ParseLine(buf);  // ParseLine() already takes care of comment lines
                          // ("#" or ";" lines)
    ...
    if (args[0] == "include") {
      if (args.size() == 2) {
        ...
        printf("GDS SYNTAX ERROR in %s [line %d]:\n", filename.c_str(),
               lineNum);
        printf(
            "--- include needs one and only one argument: <another gds "
            "filename>\n");
    ...
    if (args[0] == "gsession") {
      if (mySession.newCovFlag || mySession.modCovFlag ||
          mySession.modPlusFlag || mySession.effFlag) {
        mySession.gsessionFlag = true;
        printf("GDS SYNTAX ERROR in %s [line %d]:\n", filename.c_str(),
               lineNum);
        printf(
            "--- gsession is NOT allowed in the middle of "
            "newcov/modcov/modcov+/chkeff section\n");
        mySession.validity = false;
        continue;
      }
      ...
      if (gcounter) {
        ...
        if (mySession.validity && !validateOnly && !mySession.covList.size()) {
          printf(
              "GDS ERROR in %s [line %d]: %s session doesn't include any "
              "covariates\n",
    ...
    // openG opens an existing G matrix file
      ...
      if (mySession.newCovFlag || mySession.modCovFlag ||
        ...
        printf("GDS SYNTAX ERROR in %s [line %d]:\n", filename.c_str(),
               lineNum);
        printf(
            "--- openG is NOT allowed in the middle of "
            "newcov/modcov/modcov+/chkeff section\n");
        mySession.validity = false;
        continue;
      }
      if (args.size() != 2) {
        printf("GDS SYNTAX ERROR in %s [line %d]:\n", filename.c_str(),
               lineNum);
        printf(
            "--- openG needs one and only one argument: <G matrix filename>\n");
        mySession.validity = false;
        continue;
      }
      if (mySession.chkG(filename.c_str(), args(1))) {
        ...
        printf(
            "    It may also clear previous scan definition and overwrite "
            "TR/length/sampling values.\n");
    ...
    // Read tes files
      ...
      if (mySession.newCovFlag || mySession.modCovFlag ||
        ...
        printf("GDS SYNTAX ERROR in %s [line %d]:\n", filename.c_str(),
               lineNum);
        printf(
            "--- scan is NOT allowed in the middle of "
            "newcov/modcov/modcov+/chkeff section\n");
        mySession.validity = false;
        continue;
      }
    ...
    // Read TR
      ...
      if (mySession.newCovFlag || mySession.modCovFlag ||
        ...
        printf("GDS SYNTAX ERROR in %s [line %d]:\n", filename.c_str(),
               lineNum);
        printf(
            "--- %s is NOT allowed in the middle of "
            "newcov/modcov/modcov+/chkeff section\n",
            args(0));
        mySession.validity = false;
        continue;
      ...
      if (mySession.doneCommon) {
        printf("GDS WARNING in %s [line %d]:\n", filename.c_str(), lineNum);
        printf(
            "--- TR is being reset, covariates defined before this line will "
            "be cleared\n");
        mySession.resetTR();
      ...
      if (args.size() == 2) {
        ...
        printf("GDS SYNTAX ERROR in %s [line %d]:\n", filename.c_str(),
               lineNum);
        printf(
            "--- TR needs one and only one argument: <TR value (unit: ms)>\n");
        mySession.validity = false;
      }
    }
    // Read upsampling rate
      ...
      if (mySession.doneCommon) {
        printf("GDS WARNING in %s [line %d]:\n", filename.c_str(), lineNum);
        printf(
            "--- sampling is being reset, covariates defined before this line "
            "will be cleared\n");
        mySession.resetSampling();
      } else if (mySession.samplingFlag) {
        printf("GDS WARNING in %s [line %d]:\n", filename.c_str(), lineNum);
        printf(
            "--- sampling already exists, previous definition will be "
            "overwritten\n");
      }
      if (args.size() == 2) {
        if (strtol(args[1]) > 0) {
        ...
        printf("GDS SYNTAX ERROR in %s [line %d]:\n", filename.c_str(),
               lineNum);
        printf(
            "--- sampling needs one and only one argument: <upsampling rate "
            "(unit: ms)>\n");
        mySession.validity = false;
      }
    }
    // Define total number of time points (images)
      ...
      if (mySession.doneCommon) {
        printf("GDS WARNING in %s [line %d]:\n", filename.c_str(), lineNum);
        printf(
            "--- length is being reset, covariate(s), scan file(s) and "
            "condition function\n");
        ...
        printf("GDS syntax WARNING in %s [line %d]:\n", filename.c_str(),
               lineNum);
        printf(
            "--- length line already exists, previous definition will be "
            "overwritten\n");
      }
      if (args.size() == 2) {
        if (strtol(args[1]))
    ...
    // Define condition function
      ...
      if (args.size() != 2) {
        printf("GDS SYNTAX ERROR in %s [line %d]:\n", filename.c_str(),
               lineNum);
        printf(
            "--- condition needs one and only one argument: <condition "
            "function filename>\n");
        mySession.validity = false;
        continue;
      }
      if (mySession.chkinfile(args(1))) {
        if (mySession.condfxn.length()) {
          printf("GDS WARNING in %s [line %d]:\n", filename.c_str(), lineNum);
          printf(
              "--- condition line already exists, previous definition will be "
              "overwritten\n");
        }
    ...
    /* Define condition label file, it needs one and only one argument as
       filename */
      ...
      if (mySession.userCondKey.size()) {
        ...
        printf("GDS SYNTAX ERROR in %s [line %d]:\n", filename.c_str(),
               lineNum);
        printf(
            "--- condition-label-file needs one and only one argumentas: "
    ...
    /* modify a certain condition label's name */
      ...
      if (args.size() != 3) {
        ...
        printf(
            "    <original condition label name> <new condition label name>\n");
        mySession.validity = false;
      } else {
        if (!mySession.doneCondition) mySession.chkCondition();
    ...
    /* save condition labels as a txt file */
      ...
      if (args.size() != 2) {
        printf("GDS SYNTAX ERROR in %s [line %d]:\n", filename.c_str(),
               lineNum);
        printf(
            "--- save-condition-label needs one and only one argument: <txt "
            "filename>\n");
        mySession.validity = false;
      } else {
        if (!mySession.doneCondition) mySession.chkCondition();
    ...
    /* save a certain covariate as a ref file */
      ...
      if (mySession.newCovFlag || mySession.modCovFlag ||
        ...
        printf("GDS SYNTAX ERROR in %s [line %d]:\n", filename.c_str(),
               lineNum);
        printf(
            "--- save-cov needs two and only two arguments: <covariate name> "
            "<ref filename>\n");
        mySession.validity = false;
      } else if (!validateOnly) {
     ...
     ****************************************************/
      ...
      if (!mySession.doneCommon) mySession.chkCommon();
      mySession.chkEnd();
      ...
      // Add single covariate
        ...
        if (args.size() != 3) {
          printf("GDS SYNTAX ERROR in %s [line %d]:\n", filename.c_str(),
                 lineNum);
          printf(
              "--- newcov single takes one and only one argument: <ref "
              "filename>\n");
          mySession.validity = false;
      ...
      // Old tag for adding single covariate, (obsolescent, kept for
      // compatibility)
      else if (args[1] == "add-new") {
        printf("GDS SYNTAX WARNING in %s [line %d]:\n", filename.c_str(),
               lineNum);
        printf(
            "--- add-new is an obsolescent tag, recommended format to add "
            "single covariate:\n");
        ...
        if (args.size() == 2)
          ...
          printf("GDS SYNTAX ERROR in %s [line %d]:\n", filename.c_str(),
                 lineNum);
          printf(
              "--- add-new takes one and only one argument: <ref filename>\n");
          mySession.validity = false;
        }
      }
      // Add intercept
        ...
        // Default name
        if (args.size() == 2) mySession.newVar.varname = "Intercept";
      ...
      // Obsolescent tag for intercept, kept for compatibility
        ...
        printf("GDS SYNTAX WARNING in %s [line %d]:\n", filename.c_str(),
               lineNum);
        printf(
            "--- add-intercept is an obsolescent tag, use intercept instead\n");
        mySession.newCovFlag = true;
        mySession.interceptFlag = true;
        // Default type is 'N'
        mySession.newVar.type = 'K';
        ...
        // Use the argument as name
          ...
          printf("GDS SYNTAX ERROR in %s [line %d]:\n", filename.c_str(),
                 lineNum);
          printf(
              "--- add-intercept takes an optional argument (covariate "
              "name)\n");
          mySession.validity = false;
        }
      }
      ...
      // Obsolescent tag to add diagonal set
        ...
        printf("GDS SYNTAX WARNING in %s [line %d]:\n", filename.c_str(),
               lineNum);
        printf(
            "--- add-diagonal is an obsolescent tag, use diagonal instead\n");
        mySession.chkEnd();
        mySession.newCovFlag = true;
        mySession.diagonalFlag = true;
        // Default type is 'I'
      ...
      // Obsolescent tag to add contrasts, kept for compatibility
        ...
        printf("GDS SYNTAX WARNING in %s [line %d]:\n", filename.c_str(),
               lineNum);
        printf(
            "--- add-contrast is an obsolescent tag, use diagonal instead\n");
        mySession.chkEnd();
        mySession.newCovFlag = true;
        mySession.contrastFlag = true;
        // Default type is 'I'
      ...
      // Add var length trial effects
        ...
        if (args.size() != 3) {
          printf("GDS SYNTAX ERROR in %s [line %d]:\n", filename.c_str(),
                 lineNum);
          printf(
              "--- newcov var-trialfx needs one and only one argument: <trial "
              "file path>\n");
          mySession.validity = false;
        } else if (!mySession.chkinfile(args(2))) {
          printf("GDS SYNTAX ERROR in %s [line %d]:\n", filename.c_str(),
      ...
      // Add scan effect
        ...
        if (args.size() > 2) {
          ...
          printf("GDS SYNTAX ERROR in %s [line %d]:\n", filename.c_str(),
                 lineNum);
          printf(
              "--- only one tes file available, scan-effect line is illegal\n");
          mySession.validity = false;
        }
      }
      // Add global signals
      ...
      // Add movement parameters
        ...
        if (mySession.fakeTes) {
          printf("GDS SYNTAX ERROR in %s [line %d]:\n", filename.c_str(),
                 lineNum);
          printf(
              "--- fake tes files found, movement parameter file not exist\n");
          mySession.validity = false;
        }
        if (args.size() > 2) {
          printf("GDS SYNTAX ERROR in %s [line %d]:\n", filename.c_str(),
          ...
          printf("GDS SYNTAX ERROR in %s [line %d]:\n", filename.c_str(),
                 lineNum);
          printf(
              "--- tes file(s) must be defined for movement parameter "
              "design\n");
          mySession.validity = false;
        }
      }
      // Spike covariate(s) is a new functionality
        ...
        if (args.size() == 3)
          ...
          printf("GDS SYNTAX ERROR in %s [line %d]:\n", filename.c_str(),
                 lineNum);
          printf(
              "--- spike only takes an optional argument: absolute spike "
              "position\n");
          mySession.validity = false;
        }
      }
      ...
      // Copy a certain covariate
        ...
        if (args.size() == 3) {
          ...
          printf("GDS SYNTAX ERROR in %s [line %d]:\n", filename.c_str(),
                 lineNum);
          printf(
              "--- newcov cp requires exactly one argument: covariate name\n");
          mySession.validity = false;
        }
      }
    ...
    // type: interest / NoInterest / KeepNoInterest
      ...
      if (args.size() == 2 && foo)
        ...
        printf("GDS SYNTAX ERROR in %s [line %d]:\n", filename.c_str(),
               lineNum);
        printf(
            "--- type needs one and only one argument: <covariate type "
            "(I/N/K)>\n");
        mySession.validity = false;
      }
    }
    // "varfile" is obsolescent, kept for compatibility
      ...
      printf("GDS SYNTAX WARNING in %s [line %d]:\n", filename.c_str(),
             lineNum);
      printf(
          "--- varfile is an obsolescent tag, recommended format to add single "
          "covariate is:\n");
      printf("\tnewcov single <ref file name>\n");
    ...
    // (Similar to the name check for cov-name. This step is not done yet.)
      ...
      if (!mySession.newCovFlag && !mySession.modCovFlag &&
        ...
        printf("GDS syntax WARNING in %s [line %d]:\n", filename.c_str(),
               lineNum);
        printf(
            "--- Redundant group line: group already defined in this "
            "section\n");
      } else if (args.size() == 2)
        ...
        printf("GDS SYNTAX ERROR in %s [line %d]:\n", filename.c_str(),
               lineNum);
        printf(
            "--- group needs one and only one argument: <covariate group "
            "name>\n");
        mySession.validity = false;
      }
    }
    ...
    // "option" is simulating "modify" menu
      ...
      // "convolve" option is special because it takes at least two arguments
        ...
        if (args.size() != 3) {
          printf("GDS SYNTAX ERROR in %s [line %d]:\n", filename.c_str(),
                 lineNum);
          printf(
              "--- option time-shift needs one and only one argument: <time "
              "shift value (unit: ms)>\n");
          mySession.validity = false;
        } else {
      ...
      // it is limited in "newcov cp" session
        ...
        if (!mySession.cpFlag) {
          printf("GDS SYNTAX ERROR in %s [line %d]:\n", filename.c_str(),
                 lineNum);
          printf(
              "--- derivative option is ONLY allowed in \"newcov cp\" "
              "section\n");
          mySession.validity = false;
          ...
          printf("GDS SYNTAX ERROR in %s [line %d]:\n", filename.c_str(),
                 lineNum);
          printf(
              "--- derivative option is exclusive with itself or exponential "
              "option\n");
          ...
          printf("GDS SYNTAX ERROR in %s [line %d]:\n", filename.c_str(),
                 lineNum);
          printf(
              "--- option derivative needs one and only one argument: <# of "
              "derivative>\n");
          mySession.validity = false;
        } else {
          ...
          if (tmpInt <= 0) {
            printf("GDS SYNTAX ERROR in %s [line %d]:\n", filename.c_str(),
                   lineNum);
            printf(
                "--- invalid derivative number (must be a positive integer): "
                "%s\n",
                args[2].c_str());
            mySession.validity = false;
          } else if ((mySession.totalReps * mySession.TR /
                      mySession.tmpResolve) %
                     2) {
            printf("GDS SYNTAX ERROR in %s [line %d]:\n", filename.c_str(),
                   lineNum);
            printf(
                "--- derivative option not allowed for covariate with odd "
                "length\n");
            mySession.validity = false;
          } else {
      ...
      // it is limited in "newcov cp" session
        ...
        if (!mySession.cpFlag) {
          printf("GDS SYNTAX ERROR in %s [line %d]:\n", filename.c_str(),
                 lineNum);
          printf(
              "--- exponential option is ONLY allowed in \"newcov cp\" "
              "section\n");
          mySession.validity = false;
        } else if (mySession.derivFlag || mySession.expnFlag) {
          printf("GDS SYNTAX ERROR in %s [line %d]:\n", filename.c_str(),
                 lineNum);
          printf(
              "--- exponential option is exclusive with itself or "
              "derivative:\n");
          printf("    Use another modcov section for these actions instead\n");
          mySession.validity = false;
        } else if (args.size() != 3) {
          printf("GDS SYNTAX ERROR in %s [line %d]:\n", filename.c_str(),
                 lineNum);
          printf(
              "--- option exponential needs one and only one argument: <# of "
              "exponential>\n");
          mySession.validity = false;
        } else {
      ...
      // "eigen-set" is eigenvector set option, it removes the original
      // covariate and adds three new covariates; it is limited in modcov+
      // section
        ...
        if (!mySession.modPlusFlag) {
          printf("GDS SYNTAX ERROR in %s [line %d]:\n", filename.c_str(),
                 lineNum);
          printf(
              "--- eigen-vector option is ONLY allowed in \"modcov+\" "
              "section\n");
          mySession.validity = false;
          ...
          printf("GDS SYNTAX ERROR in %s [line %d]:\n", filename.c_str(),
                 lineNum);
          printf(
              "--- eigen-vector option is exclusive with itself or the "
              "following options:\n");
          printf("    fir fourier\n");
          printf("    Use another modcov+ section for these actions instead\n");
          mySession.validity = false;
      ...
      // "fir" is finite impulse response option, it modifies original covariate
      // and adds a few new ones; it is limited in modcov+ section
      else if (args[1] == "fir") {
        if (!mySession.modPlusFlag) {
          printf("GDS SYNTAX ERROR in %s [line %d]:\n", filename.c_str(),
                 lineNum);
          ...
          printf("GDS SYNTAX ERROR in %s [line %d]:\n", filename.c_str(),
                 lineNum);
          printf(
              "--- fir option is exclusive with itself or the following "
              "options:\n");
      ...
      // adds a few new ones; it is limited in modcov+ section
        ...
        if (!mySession.modPlusFlag) {
          printf("GDS SYNTAX ERROR in %s [line %d]:\n", filename.c_str(),
                 lineNum);
          printf(
              "--- fourier-set option is ONLY allowed in \"modcov+\" "
              "section\n");
          mySession.validity = false;
        } else if (mySession.esFlag || mySession.firFlag || mySession.fsFlag) {
          printf("GDS SYNTAX ERROR in %s [line %d]:\n", filename.c_str(),
                 lineNum);
          printf(
              "--- fourier-set option is exclusive with itself or the "
              "following options:\n");
          printf("    eigen-vector fourier\n");
          printf("    Use another modcov+ section for these actions instead\n");
          mySession.validity = false;
          ...
          printf(
              "    Use fs-peroid, fs-harmonics, fs-zero-freq and fs-alter-cov "
      ...
      // "multiply" is a regular option that accepts one argument as covariate
      // name
        ...
        if (args.size() != 3) {
          printf("GDS SYNTAX ERROR in %s [line %d]:\n", filename.c_str(),
                 lineNum);
          printf(
              "--- option multiply needs one and only one argument: <covariate "
              "name>\n");
          mySession.validity = false;
        } else if (!mySession.validateOnly) {
      ...
      // "orthog" does orthogonalization, it is a regular option that doesn't
      // need any argument
        ...
        if (args.size() != 2) {
          ...
          printf(
              "--- please use orth-type or orth-name tag to set its "
              "parameter.\n");
          mySession.validity = false;
        } else {
      ...
      // For regular options that only modify covariate and don't need any
      // arguments They are: mean-center, mean-center-non-zero, unit-variance,
      // unit-excursion, convert-delta
      else if (args.size() == 2) {
    ...
    // 1st tag for orthogalization parameter: covariate type (exclusive with
    // orth-name)
      ...
      if (!mySession.orthFlag) {
        ...
        if (args.size() != 2) {
          printf("GDS SYNTAX ERROR in %s [line %d]:\n", filename.c_str(),
                 lineNum);
          printf(
              "--- orth-type takes one and only one argument: <covariate "
              "type>\n");
          mySession.validity = false;
    ...
    // 2nd tag for orthogalization parameter: covariate name(s) (exclusive with
    // orth-type)
      ...
      if (!mySession.orthFlag) {
        ...
        if (args.size() < 2) {
          printf("GDS SYNTAX ERROR in %s [line %d]:\n", filename.c_str(),
                 lineNum);
          printf(
              "--- orth-name needs at least one argument: <covariate name>\n");
          mySession.validity = false;
        } else if (!mySession.validateOnly) {
          int orthIndex;
    ...
    // Fourier set parameter: period to model (integer)
      ...
      if (!mySession.fsFlag) {
        ...
        printf("GDS SYNTAX ERROR in %s [line %d]:\n", filename.c_str(),
               lineNum);
        printf(
            "--- fs-period takes one and only one argument: <period value "
            "(unit: second)>\n");
        mySession.validity = false;
      } else {
        if (!mySession.doneCommon) mySession.chkCommon();
    ...
    // Fourier set parameter: number of harmonics (integer)
      ...
      if (!mySession.fsFlag) {
        ...
        printf("GDS SYNTAX ERROR in %s [line %d]:\n", filename.c_str(),
               lineNum);
        printf(
            "--- fs-harmonics takes one and only one argument: <# of "
            "harmonics>\n");
        mySession.validity = false;
    ...
    // "matrix-row" is specially designed for adding contrasts
      ...
      if (!mySession.contrastFlag) {
        ...
        mySession.validity = false;
      } else if (mySession.condKey.size() &&
                 args.size() != mySession.condKey.size() + 1) {
        printf("GDS SYNTAX ERROR in %s [line %d]:\n", filename.c_str(),
               lineNum);
        printf(
            "--- the number of arguments for matrix-row doesn't match the "
    ...
    // "center-norm" is another special tag for "add-contrast"
      ...
      if (!mySession.contrastFlag && !mySession.diagonalFlag) {
        printf("GDS SYNTAX ERROR in %s [line %d]:\n", filename.c_str(),
               lineNum);
        printf(
            "--- center-norm is valid only for contrast or diagonal section\n");
        mySession.validity = false;
      } else if (mySession.contrastFlag && args.size() == 2 &&
                 (args[1] == "yes" || args[1] == "Yes" || args[1] == "YES" ||
                  args[1] == "y" || args[1] == "Y" || args[1] == "1"))
    ...
    // "scan-length" is a keyword for scan-effect when no tes files defined
      ...
      if (!mySession.scanFlag) {
        ...
        for (size_t k = 1; k < args.size(); k++) mySession.scanLen.Add(args[k]);
    }
    // "absolute" is specially designed to add spike covariates at obsolute
    // position
      ...
      if (!mySession.spikeFlag) {
        ...
        printf("GDS SYNTAX ERROR in %s [line %d]:\n", filename.c_str(),
               lineNum);
        printf(
            "--- absolute requiress one and only one argument: spike "
            "position\n");
        mySession.validity = false;
      }
    }
    // "relative" defines spike position relative to a real/fake tes file
      ...
      if (!mySession.spikeFlag) {
        ...
        printf("GDS SYNTAX ERROR in %s [line %d]:\n", filename.c_str(),
               lineNum);
        printf(
            "--- relative requires two and only two arguments: <tes file "
            "name/index> and <spike position>\n");
        mySession.validity = false;
      } else {
    ...
    // "modcov" modifies a certain covariate
      ...
      if (args.size() != 2) {
        printf("GDS SYNTAX ERROR in %s [line %d]:\n", filename.c_str(),
               lineNum);
        printf(
            "--- modcov needs one and only one arguments: <covariate name>\n");
        mySession.validity = false;
      } else if (!mySession.validateOnly) {
    ...
    // delete a certain covariate
      ...
      if (mySession.newCovFlag || mySession.modCovFlag ||
        ...
        printf("GDS SYNTAX ERROR in %s [line %d]:\n", filename.c_str(),
               lineNum);
        printf(
            "--- del-cov line is NOT allowed in the middle of "
            "newcov/modcov/modcov+/chkeff section\n");
        mySession.validity = false;
      } else if (args.size() < 2) {
        printf("GDS SYNTAX ERROR in %s [line %d]:\n", filename.c_str(),
        ...
        if (mySession.validity) mySession.delCov();
      }
    }
    // delete all covariates
      ...
      if (mySession.newCovFlag || mySession.modCovFlag ||
        ...
        printf("GDS SYNTAX ERROR in %s [line %d]:\n", filename.c_str(),
               lineNum);
        printf(
            "--- del-all-cov line is NOT allowed in the middle of "
            "newcov/modcov/modcov+/chkeff section\n");
        mySession.validity = false;
      } else if (args.size() > 1) {
        printf("GDS SYNTAX ERROR in %s [line %d]:\n", filename.c_str(),
    ...
    // Check efficiency
      ...
      if (mySession.newCovFlag || mySession.modCovFlag ||
        ...
        printf("GDS SYNTAX ERROR in %s [line %d]:\n", filename.c_str(),
               lineNum);
        printf(
            "--- chkeff line is NOT allowed in the middle of "
            "newcov/modcov/modcov+/chkeff section\n");
        mySession.validity = false;
      } else if (args.size() != 2) {
        mySession.effFlag = true;
        printf("GDS SYNTAX ERROR in %s [line %d]:\n", filename.c_str(),
               lineNum);
        printf(
            "--- chkeff needs one and only one argument: <covariate name>\n");
        mySession.validity = false;
      } else if (!validateOnly) {
        mySession.effFlag = true;
        int selection = mySession.getCovID(args[1]);
    ...
    // downsample option in efficiency check
      ...
      if (!mySession.effFlag) {
        ...
        printf("GDS SYNTAX ERROR in %s [line %d]:\n", filename.c_str(),
               lineNum);
        printf(
            "--- downsample needs one and only one argument: <downsample "
            "option>\n");
        mySession.validity = false;
    ...
    // filter name for efficiency check
      ...
      if (!mySession.effFlag) {
        ...
        printf("GDS SYNTAX ERROR in %s [line %d]:\n", filename.c_str(),
               lineNum);
        printf(
            "--- filter needs one and only one argument: <filter filename>\n");
        mySession.validity = false;
      } else if (!mySession.chkinfile(args(1))) {
        printf("GDS SYNTAX ERROR in %s [line %d]:\n", filename.c_str(),
               lineNum);
    ...
    // type for efficiency check
      ...
      if (!mySession.effFlag) {
        ...
        printf("GDS SYNTAX ERROR in %s [line %d]:\n", filename.c_str(),
               lineNum);
        printf(
            "--- eff-type needs one and only one argument: <covariate type>\n");
        mySession.validity = false;
      ...
      // Make sure this type is same as the base covariate's type
               ...
               !mySession.typeMatch(mySession.chkEffType(args[1]),
                                    mySession.effBaseIndex)) {
        printf("GDS SYNTAX ERROR in %s [line %d]:\n", filename.c_str(),
               lineNum);
    ...
    // cutoff value for efficiency check
      ...
      if (!mySession.effFlag) {
        ...
        printf("GDS SYNTAX ERROR in %s [line %d]:\n", filename.c_str(),
               lineNum);
        printf(
            "--- eff-cutoff needs one and only one argument: <cutoff value>\n");
        mySession.validity = false;
...
gSession::gSession() { init(); }

/* Destructor */
gSession::~gSession() { covList.clear(); }
...
void gSession::init() {
  ...
  fsPeriod = fsHarmonics =
      -1;              // Fourier set default: period and number of harmonics
...
void gSession::reset() {
  newVar.init();
  if (newVar.varvec.getLength()) newVar.varvec.setAll(0);
  if (covName.size()) covName.clear();
  if (option.size()) option.clear();
  if (convolOpt.size()) convolOpt.clear();
  if (singleFile.length()) singleFile = "";
  if (trialFile.length()) trialFile = "";
  if (txtFile.length()) txtFile = "";
  if (scanLen.size()) scanLen.clear();
  if (spike.size()) spike.clear();
  if (newList.size()) newList.clear();
  if (orthID.size()) orthID.clear();
  if (delID.size()) delID.clear();
...
void gSession::resetTR() {
  ...
  reset();
  // reset tes files
  if (teslist.size()) teslist.clear();
  if (lenList.size()) lenList.clear();
  if (tesReal.size()) tesReal.clear();
  fakeTes = false;
...
void gSession::resetLen() {
  ...
  if (userCondKey.size()) userCondKey.clear();
  // reset previous covariates
  if (covList.size()) covList.clear();
}
...
int gSession::chkSessionName(const char *inputStr) {
  ...
  if (!vb_direxists(parent)) return 0;
  // Return 1 if parent dir is not writeable
  if (access(parent.c_str(), W_OK)) return 1;

  int len = foo.length();
  string rootStr = xrootname(foo);
  // Return 2 if .G exists and is a directory
  if (foo.substr(len - 2, 2) == ".G" && vb_direxists(foo)) return 2;
...
int gSession::chkdirname(const char *indirname) {
  if (!access(indirname, W_OK)) return 0;
  return 1;
}
...
int gSession::chkinfile(const char *inputname) {
  if (access(inputname, R_OK)) return 0;
  return 1;
}

/* Check a variable's type, returns 1 if it's I/N/K, return 0 otherwise. */
char gSession::chkType(string inputType) {
  if (inputType == "I" || inputType == "i" || inputType == "Interest" ||
      inputType == "interest")
    return 'I';
  if (inputType == "N" || inputType == "n" || inputType == "NoInterest" ||
      inputType == "Nointerest" || inputType == "noInterest" ||
      inputType == "nointerest")
    return 'N';
  if (inputType == "K" || inputType == "k" || inputType == "KeepNoInterest" ||
      inputType == "keepNoInterest" || inputType == "keepNointerest" ||
      inputType == "keepNoInterest" || inputType == "keepnointerest")
    return 'K';
  //   if (inputType == "D" || inputType == "d" || inputType == "Dependent" ||
  //   inputType == "dependent")
  //     return 'D';
  return 0;
}
...
int gSession::chkCovName(string inputStr) {
  ...
  for (size_t i = 0; i < covName.size(); i++) {
    if (inputStr == covName[i]) return -2;
  }
...
void gSession::end() {
  ...
  if (orthFlag && !orthTypeFlag && !orthNameFlag) {
    printf("GDS SYNTAX ERROR in %s [line %d]:\n", inputFilename.c_str(),
           lineNum);
    printf(
        "--- invalid orthog option: no orth-type or orth-name lines found\n");
    validity = false;
  }

  if (singleFlag) {
    chkSingle();
    if (validity && !validateOnly) addSingle();
    reset();
    singleFlag = false;
  } else if (interceptFlag) {
    if (validity && !validateOnly) addIntercept();
    reset();
    interceptFlag = false;
  } else if (trialFlag) {
    if (validity && !validateOnly) addTrialfx();
    reset();
    trialFlag = false;
  } else if (diagonalFlag) {
    if (validity && !validateOnly) addDiagonal();
    reset();
    newDS.init();
    diagonalFlag = false;
    ...
    if (validity && !validateOnly) addContrast();
    reset();
    newContrast.init();
    contrastFlag = false;
  } else if (varTrialFlag) {
    chkTrialFile();
    if (validity && !validateOnly) addVarTrialfx();
    reset();
    varTrialFlag = false;
  } else if (scanFlag) {
    if (validity && !teslist.size()) chkScanLen();
    if (validity && !validateOnly) addScanfx();
    reset();
    scanFlag = false;
  } else if (gsFlag) {
    if (validity) chkGS();
    if (validity && !validateOnly) addGS();
    reset();
    gsFlag = false;
  } else if (mpFlag) {
    if (validity) chkMP();
    if (validity && !validateOnly) addMP();
    reset();
    mpFlag = false;
  } else if (spikeFlag) {
    if (validity && !validateOnly) addSpike();
    reset();
    spikeFlag = false;
  } else if (txtFlag) {
    if (validity) chkTxt();
    if (validity && !validateOnly) addTxt();
    reset();
    txtFlag = false;
  } else if (cpFlag) {
    if (validity && !validateOnly) cpCov();
    reset();
    cpFlag = false;
  } else if (modCovFlag) {
    if (validity && !validateOnly) modCov();
    reset();
  } else if (modPlusFlag) {
    chkModPlus();
    if (validity && !validateOnly) modPlus();
    reset();
  } else if (effFlag) {
    chkEffVar();
    if (validity && !validateOnly) exeEff();
    resetEff();
  }
...
void gSession::chkCommon() {
  ...
  if (TR && tesTR && TR != tesTR) {
    printf("gds syntax error in %s [gSession starting from line %d]:\n",
           inputFilename.c_str(), startLine);
    printf(
        "--- TR in tes file's header (%d) doesn't match the value on line %d "
        "(%d)\n",
        tesTR, lineNum, TR);
    validity = false;
    return;
  ...
  if (!totalReps && !tesReps) {
    printf("gds syntax error in %s [gSession starting from line %d]:\n",
           inputFilename.c_str(), startLine);
    printf(
        "--- length or scan lines required to define the total number of "
        "images\n");
    validity = false;
    return;
  }
  ...
  if (totalReps && tesReps && totalReps != tesReps) {
    printf("gds syntax error in %s [gSession starting from line %d]:\n",
           inputFilename.c_str(), startLine);
    printf(
        "--- total length in tes files (%d) doesn't match the value defined in "
        "length line(%d)\n",
        tesReps, totalReps);
    validity = false;
    return;
  ...
  if (gDirFlag) gFilename = dirname + "/" + xfilename(dirname);
}
...
void gSession::addSingle() {
  ...
  newCovMod();
  // set up varname
  if (covName.size()) newVar.varname = covName[0];
...
void gSession::chkSingle() {
  ...
  if (newStat) {
    ...
    printf("GDS SYNTAX ERROR in %s single section [ended at line %d]:\n",
           inputFilename.c_str(), lineNum);
    printf(
        "--- number of elements (%d) in %s is NOT multiple of total length "
        "(%d)\n",
    ...
    printf("GDS SYNTAX ERROR in %s single section [ended at line %d]:\n",
           inputFilename.c_str(), lineNum);
    printf(
        "--- reference function (%s) with a length of %d can NOT be upsampled at the \
...
void gSession::newCovMod() {
  for (size_t i = 0; i < option.size(); i++) singleOpt(newVar, option[i]);
}
...
void gSession::modCov() {
  ...
  if (covName.size()) covList[singleID].varname = covName[0];
  if (newVar.group.length()) covList[singleID].group = newVar.group;
  if (newVar.type != 'u') covList[singleID].type = newVar.type;
}

/* chkModPlus() makes sure eigen vector, fir or Fourier set options are all
 * valid */
void gSession::chkModPlus() {
  ...
  if (esFlag && magFactor != floor(magFactor)) {
    printf("GDS SYNTAX ERROR in %s [line %d]:\n", inputFilename.c_str(),
           lineNum);
    printf(
        "--- eigen-vector option requires 2000 to be multiple of sampling "
        "rate\n");
    validity = false;
  }
...
void gSession::eigenSet() {
  ...
  for (int i = 0; i < esIndex; i++) singleOpt(newVar, option[i]);
  insertES();
}

/* prepNew() is written specially for modcov+ tag. It copies selected covariate
 * to newVar. If no type/group defined, the original ones will be copied */
void gSession::prepNew() {
  if (!newVar.group.length()) newVar.group = covList[singleID].group;

  if (newVar.type == 'u') newVar.type = covList[singleID].type;

  newVar.varname = covList[singleID].varname;
  newVar.varvec = covList[singleID].varvec;
}

/* insertES() inserts three new eigen vector set covariates, then execute the
 * rest of options after eigen-vector, rename covariates */
void gSession::insertES() {
  ...
  for (size_t i = 0; i < 3; i++) {
    ...
    for (int j = 0; j < vecLength; j++) (*eigenVec)[j] = (*tmpVec)[j];
    ...
    if (i < covName.size()) newVar.varname = covName[i];
...
/* firSet() modifies original covariate and adds a few new ones based on number
 * of fir */
...
/* insertFIR() create FIR set covariates, execute the options after fir, insert
 * new covariates right after the original covariate */
void gSession::insertFIR() {
  ...
  for (size_t j = 0; j <= (size_t)firNum; j++) {
    ...
    if (j > 0) orgVec.phaseShift(timeShift, newVar.varvec);
    // default FIR name
    ...
    if (j < covName.size()) newVar.varname = covName[j];
...
void gSession::fourierSet() {
  ...
  for (int i = 0; i < fsIndex; i++) singleOpt(newVar, option[i]);
  insertFS();
}
...
void gSession::insertFS() {
  ...
  int orderG = totalReps * TR / tmpResolve;
  double pix2 = 2.0 * PI;
  // newIndex tracks the position where the next new covariate should be
  // inserted
...
VB_Vector *gSession::fs_getFFT(VB_Vector *currentVec, VB_Vector *fourierVec) {
  if (fsDeltaCov) calcDelta(currentVec);
 ...
 * Options allowed are: mean-center, mean-center-non-zero, unit-variance,
 * unit-excursion, convert-delta, convolve, ts (time shift), multi (multiply),
 * orth (orthogonalize) */
...
void gSession::mcNonZero(VB_Vector &inputVec) {
  ...
  for (int i = 0; i < length; i++) {
    ...
    if (element) inputVec.setElement(i, element - nonZeroMean);
  }
}
...
/* chkOrthType() is similar to chkType(), but the input type could be
 * "ALL/All/all/A/a" */
char gSession::chkOrthType(string inputType) {
  if (inputType == "ALL" || inputType == "All" || inputType == "all" ||
      inputType == "A" || inputType == "a")
    return 'A';
...
/* getOrhtID() collects orthogonalization covariate index and put in orthID
 * array */
void gSession::getOrthID(char inputType) {
  ...
  if (inputType == 'A') {
    for (int i = 0; i < (int)covList.size(); i++) {
      if (i != singleID) orthID.push_back(i);
    }
    return;
  }

  for (int i = 0; i < (int)covList.size(); i++) {
    if (i != singleID && covList[i].type == inputType) orthID.push_back(i);
  }
}

/* Orthogonalize input vb_vector */
void gSession::orthogonalize(VB_Vector &inputVec) {
  if (!orthID.size()) return;
  ...
  // DYK subG.MakeInCore();
  for (int i = 0; i < colNum; i++) subG.SetColumn(i, covList[orthID[i]].varvec);
  ...
  if (orthVec.size() < 1) {
    printf("GDS SYNTAX ERROR in %s [line %d]:\n", inputFilename.c_str(),
           lineNum);
    printf(
        "--- invalid orthogonalization: covariates are linearly dependent!\n");
    validity = false;
    return;
  }
  inputVec -= orthVec;
...
int gSession::chkOption(const char *input) {
  if (!strcmp(input, "mean-center")) return 1;
  if (!strcmp(input, "mean-center-non-zero")) return 1;
  if (!strcmp(input, "unit-variance")) return 1;
  if (!strcmp(input, "unit-excursion")) return 1;
  if (!strcmp(input, "convert-delta")) return 1;
...
int gSession::chkConvol(tokenlist convOpt) {
  if (convOpt.size() < 2 || convOpt.size() > 3) {
    printf("GDS SYNTAX ERROR in %s [line %d]:\n", inputFilename.c_str(),
           lineNum);
    printf(
        "--- convolve option needs two or three arguments in this format:\n");
    printf(
        "option convolve <kernel filename> <convolution TR> <optional tag>\n");
    return 1;
  }
...
void gSession::addDiagonal() {
  ...
  for (size_t i = 0; i < (uint32)rowNum; i++) {
    ...
    for (unsigned j = 0; j < condVec_up->getLength(); j++) {
      ...
      // scale or not?
      if (newDS.scaleFlag) newVar.varvec[j] /= (double)countNum(condVec_up, n);
    }
    // center-norm or not?
    if (newDS.centerFlag) newVar.varvec.meanCenter();
...
void gSession::chkCondition() {
  ...
  if (condStat == -2) {
    printf("Condition function error in %s [gSession starting from line %d]:\n",
           inputFilename.c_str(), startLine);
    printf(
        "--- %s has different number of condition keys in header and content\n",
        condfxn.c_str());
    validity = false;
    return;
  }
  ...
  if (condLength % totalReps != 0) {
    printf("Condition function error in %s [gSession starting from line %d]:\n",
           inputFilename.c_str(), startLine);
    printf(
        "--- number of elements (%d) in %s is NOT multiple of the number of "
        "time points defined (%d)\n",
  ...
  if ((TR / tmpResolve) % (condLength / totalReps) != 0) {
    printf("Condition function error in %s [gSession starting from line %d]:\n",
           inputFilename.c_str(), startLine);
    printf(
        "--- %s can NOT be upsampled with the current sampling rate (%dms)\n",
...
/* chkCondKey() Compare condition keys in condition function file with
 * user-defined ones in condition-label-file or condition-label-name lines */
void gSession::chkCondKey() {
  if (!condLabFile.length() && !userCondKey.size()) return;

  if (condLabFile.length() && userCondKey.size()) {
    printf("gds syntax error in %s [gSession starting from line %d]:\n",
           inputFilename.c_str(), startLine);
    printf(
        "--- condition-label-file and condition-label-name lines are "
        "exclusive\n");
    validity = false;
    return;
  }
  ...
  if (numKey != userCondKey.size()) {
    ...
    printf("gds error in %s [gSession starting from line %d]:\n",
           inputFilename.c_str(), startLine);
    printf(
        "--- The number of user-defined condition keys is not equal to that "
        "defined in condition function file\n");
    validity = false;
    return;
  }

  condKey.clear();
  for (size_t i = 0; i < numKey; i++) condKey.Add(userCondKey[i]);
}

/* chkKeyName() compares the input string with any of condition key names.
 * If the input is same as one of the keys, return the key's index; otherwise
 * returns -1 */
int gSession::chkKeyName(string inputStr) {
  for (size_t i = 0; i < condKey.size(); i++) {
    if (inputStr == condKey[i]) return i;
  }
  return -1;
}
...
void gSession::addContrast() {
  ...
  for (size_t i = 0; i < lineNum; i++) {
    ...
    if (newContrast.centerFlag) newVar.varvec.meanCenter();
    newCovMod();
    // set up varname
    if (i < covName.size())
      newVar.varname = covName[i];
      ...
      // Remove '+' at the end
      if (tmpStr[tmpStr.length() - 1] == '+') tmpStr.erase(tmpStr.length() - 1);
...
void gSession::chkTrialFile() {
  ...
  if (trialSum != totalReps * TR / 1000) {
    printf("GDS SYNTAX ERROR in %s trial-effect section [ended at line %d]:\n",
           inputFilename.c_str(), lineNum);
    printf(
        "--- sum of elements (%d) in %s doesn't match the actual length (%d)\n",
...
void gSession::addScanfx() {
  ...
  for (size_t i = 0; i < scanLenVec.size(); i++) {
    ...
    for (int j = 0; j < totalReps * TR / tmpResolve; j++) {
      if (j >= offset && j < end) newVar.varvec[j] = 1.0;
    }
    newVar.varvec.meanCenter();
    newCovMod();
    // set up varname
...
/* chkScanLen() will make sure the sum of scan-length arguments is equal to
 * total repetition */
void gSession::chkScanLen() {
  ...
  if (totalLen != totalReps) {
    printf("GDS SYNTAX ERROR in %s scan-effect section [ended at line %d]:\n",
           inputFilename.c_str(), lineNum);
    printf(
        "--- sum of scan-length arguments (%d) not equal to total number of "
        "time points (%d)\n",
        totalLen, totalReps);
    validity = false;
  }
...
void gSession::addGS() {
  ...
  for (size_t i = 0; i < teslist.size(); i++) {
    ...
    gsVec.meanCenter();
    // set up varvec
    for (int j = start; j < gsLength + start; j++) newVec[j] = gsVec[j - start];
...
/* chkGS() checks the availability of global signal file(s) based on tes
 * filename(s) */
void gSession::chkGS() {
  if (!teslist.size()) {
    validity = false;
  ...
  for (size_t i = 0; i < teslist.size(); i++) {
    ...
    if (!chkinfile(gsFile.c_str())) {
      printf(
          "GDS SYNTAX ERROR in %s global-signal section [ended at line %d]:\n",
          inputFilename.c_str(), lineNum);
...
void gSession::addMP() {
  ...
  for (size_t i = 0; i < teslist.size(); i++) {
    ...
    for (size_t j = 0; j < 6; j++) {
      ...
      // center it
      for (int k = 0; k < unitLength; k++) moveInfo[k] = mpVec[k * 7 + j];
      moveInfo.meanCenter();
      for (int k = 0; k < unitLength; k++) newVec[k + start] = moveInfo[k];
      // set up varvec
      newVar.varvec = VB_Vector(upSampling(&newVec, TR / tmpResolve));
      newCovMod();
      // set up varname
...
/* chkMP() will check availability of movement parameter file(s) based on tes
 * filename(s) */
void gSession::chkMP() {
  if (!teslist.size()) {
    validity = false;
  ...
  for (size_t i = 0; i < teslist.size(); i++) {
    ...
    if (!chkinfile(mpFile.c_str())) {
      ...
      printf("GDS SYNTAX ERROR in %s move-params section [ended at line %d]:\n",
             inputFilename.c_str(), lineNum);
      printf(
          "--- number of elements (%d) in movement parameter file %s is NOT "
          "multiple of 7\n",
...
void gSession::chkSpike(string spikeStr, int tesIndex) {
  ...
  int startPos = 0;
  for (int i = 0; i < tesIndex; i++) startPos += lenList[i];
  for (int i = 0; i < (int)localSpike.size(); i++)
...
void gSession::chkTxt() {
  ...
  if (txtRowNum != totalReps) {
    printf("GDS ERROR in %s txt-file section [ended at line %d]:\n",
           inputFilename.c_str(), lineNum);
    printf(
        "--- the number of rows in %s NOT match the total number of time "
        "points",
        txtChar);
    validity = false;
    return;
  ...
  if (readStat == 1) {
    printf("GDS ERROR in %s txt-file section [ended at line %d]:\n",
           inputFilename.c_str(), lineNum);
    printf(
        "--- %s does NOT have exactly the same number of elements in all rows",
        txtChar);
    validity = false;
    return;
  }
 ...
 * options, in which new covariate(s) will be generaged based on a certain
 * covariate. The base covariate is labelled by singleID */
void gSession::cpCov() {
...
void gSession::derivMod() {
  ...
  for (int i = 0; i < derivIndex; i++) singleOpt(newVar, option[i]);
  ...
  for (size_t k = 0; k < (uint32)derivNum; k++) {
    ...
    if (k < covName.size()) tmpVar.varname = covName[k];
...
/* getCovID() converts an input string (covariate name) into its index in
 * covList; returns -1 if the covariate name is not found at all, returns -2 if
 * more than one covariate have same name, returns the real covariate index if
 * there is one and only one covariate's full name matches the input string*/
int gSession::getCovID(string inputStr) {
  vector<unsigned> matchID;
  for (unsigned i = 0; i < covList.size(); i++) {
    if (getFullName(i) == inputStr) matchID.push_back(i);
  }
...
void gSession::writeG() {
  ...
  if (gDirFlag) {
    if (!(vb_direxists(dirname))) mkdir(dirname.c_str(), 0777);
  }
  ...
  for (int i = 0; i < colNum; i++) {
    ...
    // If meanAll is enabled, mean center all but intercept
    if (meanAll && tmpVec2.getVariance() > 1e-15) tmpVec2.meanCenter();
...
/* readG() reads the input VBMatrix and loads parameters into the current
 * session */
void gSession::readG(const char *gdsName, const char *matName) {
  ...
  if (!validateOnly) buildCovList(gdsName, matName);
}
...
bool gSession::chkG(const char *gdsName, const char *matName) {
  ...
  if (gStat == 4) {
    printf("GDS SYNTAX ERROR in %s [line %d]:\n", gdsName, lineNum);
    printf(
        "--- %s: condition function has different number of keys in header and "
        "content\n",
        matName);
    validity = false;
    return false;
  ...
  if (gStat == 5) {
    printf("GDS SYNTAX ERROR in %s [line %d]:\n", gdsName, lineNum);
    printf(
        "--- %s: condition function defined in the header has different keys "
  ...
  if (gStat == 6) {
    printf("GDS SYNTAX ERROR in %s [line %d]:\n", gdsName, lineNum);
    printf(
        "--- %s: # of condition key lines in the header is different from that "
        "in condition function\n",
        matName);
    validity = false;
    return false;
  }
  if (gStat == 7) {
    printf("GDS SYNTAX ERROR in %s [line %d]:\n", gdsName, lineNum);
    printf(
        "--- %s: # of elements in condition function is NOT a multiple of "
  ...
  if (gStat == 8) {
    printf("GDS SYNTAX ERROR in %s [line %d]:\n", gdsName, lineNum);
    printf(
        "--- %s: condition function can NOT be upsampled with the sampling "
        "rate\n",
        matName);
    validity = false;
    return false;
...
/* chkPreG() makes sure *.preG file is valid, returns true if it is, false
 * otherwise */
bool gSession::chkPreG(const char *gdsName, const char *matName) {
  ...
  if (myGInfo.colNum != pregInfo.colNum) {
    printf("GDS INFO in %s [line %d]:\n", gdsName, lineNum);
    printf(
        "--- preG file ignored: different number of columns in G and preG "
        "file\n");
    return false;
  }
  if (myGInfo.sampling != pregInfo.sampling) {
    printf("GDS INFO in %s [line %d]:\n", gdsName, lineNum);
    printf(
        "--- preG file ignored: different sampling rate in G and preG file\n");
    return false;
  }
  if (myGInfo.TR && myGInfo.sampling &&
    ...
    printf("GDS INFO in %s [line %d]:\n", gdsName, lineNum);
    printf(
        "--- preG file ignored: # of rows in preG is NOT the upsampled number "
        "based on G file\n");
    return false;
  }
  if (myGInfo.condfxn != pregInfo.condfxn) {
    printf("GDS INFO in %s [line %d]:\n", gdsName, lineNum);
    printf(
        "--- preG file ignored: different condition function filenames in G "
        "and preG file\n");
    return false;
  }
  if (myGInfo.condfxn.length() &&
      cmpElement(myGInfo.condKey, pregInfo.condKey) != 0) {
    printf("GDS INFO in %s [line %d]:\n", gdsName, lineNum);
    printf(
        "--- preG file ignored: different Condition lines in G and preG "
        "file\n");
    return false;
  }
  if (cmpElement(myGInfo.nameList, pregInfo.nameList) != 0) {
    printf("GDS INFO in %s [line %d]:\n", gdsName, lineNum);
    printf(
        "--- preG file ignored: different covariate names in G and preG "
        "file\n");
    return false;
  }
  if (cmpElement(myGInfo.typeList, pregInfo.typeList) != 0) {
    printf("GDS INFO in %s [line %d]:\n", gdsName, lineNum);
    printf(
        "--- preG file ignored: different covariate types in G and preG "
        "file\n");
    return false;
  }
...
void gSession::buildCovList(const char *gdsName, const char *matName) {
  ...
  for (int i = 0; i < myGInfo.colNum; i++) {
    ...
    if (pregStat) {
      ...
      vecSize = tmpVec.size();
      for (int j = 0; j < vecSize; j++) tmpCov.varvec[j] = tmpVec[j];
    }
    // Upsample the G file based on sampling rate
      ...
      for (int j = 0; j < vecSize; j++) downVector[j] = tmpVec[j];
...
void gSession::saveCov(int covID, const char *filename) {
  ...
  printf("gds is saving covariate %s in %s ... ",
         getFullName((unsigned)covID).c_str(), filename);
...
int gSession::chkDS(string inputStr) {
  if (inputStr == "NO" || inputStr == "No" || inputStr == "no" ||
...
bool gSession::typeMatch(char typeArg, int inputIndex) {
  if (typeArg == 'A') return true;
  if (typeArg == 'I' && covList[inputIndex].type == 'I') return true;
  if (typeArg == 'N' &&
      (covList[inputIndex].type == 'I' || covList[inputIndex].type == 'K'))
    return true;
...
void gSession::chkEffVar() {
  ...
  if (2000 % tmpResolve) {
    printf("GDS SYNTAX ERROR in %s chkeff section [ended at line %d]:\n",
           inputFilename.c_str(), lineNum);
    printf(
        "--- chkeff requires that 2000 be devided by the current sampling rate "
        "(%d)\n",
        tmpResolve);
    validity = false;
    return;
  ...
  if (effFilter.length()) {
    ...
    // In "downsample before convolution" option, is the filter vector good for
    // interpolation?
    if (dsOption == 2 && 2000 % TR) {
      printf("GDS SYNTAX ERROR in %s chkeff section [ended at line %d]:\n",
             inputFilename.c_str(), lineNum);
      printf(
          "--- \"downsample before convolution\" option requires 2000 be "
          "divided by TR (%d)\n",
          tmpResolve);
      validity = false;
      return;
  ...
  if (!validateOnly) {
    ...
    if (tmpVal == -1) {
      printf("GDS ERROR in %s [line %d]:\n", inputFilename.c_str(), lineNum);
      printf(
          "--- all elements are zero, invalid base covariate for efficiency "
          "check: %s\n",
    ...
    // Make sure the base covariate's raw efficiency is not zero
      ...
      printf("GDS ERROR in %s [line %d]:\n", inputFilename.c_str(), lineNum);
      printf(
          "--- raw efficiency is zero, invalid base covariate for efficiency "
          "check: %s\n",
          getFullName(effBaseIndex).c_str());
      validity = false;
    } else
...
void gSession::exeEff() {
  vector<int> effID;
  // collect covariate index whose VALID efficiency is less than the cutoff
  // value
  for (unsigned i = 0; i < covList.size(); i++) {
    if (!typeMatch(effType, (int)i)) continue;
    ...
    if (finalEff <= 0.01) finalEff = 0;
    if (finalEff < cutoff) effID.push_back(i);
  }
...
int gHeaderInfo::chkInfo() {
  if (typeList.size() != (uint32)colNum || nameList.size() != (uint32)colNum)
    return 1;
  if (TR && sampling && TR % sampling != 0) return 2;
...
int gHeaderInfo::chkCondfxn() {
  ...
  if (refStat == -1)  // Quit if the file isn't readable
    return 3;
  if (refStat == -2) return 4;
  if (refStat == 1) return 5;
  if (condKey.size() != condKeyInFile.size()) return 6;

  int condLen = condVector->getLength();
  // Make sure the number of elements in condition function is a multiple of
  // total time points
  if (condLen % rowNum != 0) return 7;

  // Make sure condition function can be upsampled
  if (sampling && (TR / sampling) % (condLen / rowNum) != 0) return 8;
