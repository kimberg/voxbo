...
VBRename::Rename(string fname)
{
  ...
  if (read_dicom_header(fname,dci))
    return 101;
  string newdirname=(format("%04d_%s")
                     %dci.series
                     %xstripwhitespace(dci.protocol)
                     ).str();
  string newfilename=(format("%04d_%s_%s")
                      %dci.instance
  ...
  set<string> stripvrs;
  int removed=1;
  err=anonymize_dicom_header(newname,newname,"",stripgroups,stripges,stripvrs,removed);
  return err;
}
