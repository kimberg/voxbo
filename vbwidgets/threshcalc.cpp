...
tcalc::tcalc(threshold v_in, QWidget *parent, Qt::WFlags fl)
  ...
  buttonbox->hide();
  languageChange();
  connect(txtVoxelNumber, SIGNAL(textChanged(const QString &)), this,
          SLOT(update()));
...
void tcalc::languageChange() {
  ...
  setFixedWidth(this->fontMetrics().width("W") *
                25);  // heuristic for window width
  update();
}
...
void tcalc::update() {
  ...
  if (v.denomdf > FLT_MIN) {
    f_ftest = 1;
    restext =
        (format("Critical value for F(%g,%g):") % v.effdf % v.denomdf).str();
  } else {
    restext = (format("Critical value for t(%g):") % v.effdf).str();
  }
...
double tcalc::getbfthresh() { return bfthresh; }

double tcalc::getrftthresh() { return rftthresh; }

double tcalc::getbestthresh() {
  if (!finite(bfthresh))
    return rftthresh;  // if they're both nans, this is still okay
  if (bfthresh < rftthresh) return bfthresh;
  return rftthresh;
}

threshold tcalc::getthreshdata() { return v; }

void tcalc::showbuttons() { buttonbox->show(); }

void tcalc::closeEvent(QCloseEvent *) {
  reject();
  return;
}
