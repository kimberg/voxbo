...
fileview::fileview(QWidget *parent, const char *name, bool modal, Qt::WFlags fl)
  ...
  connect(leDirectory, SIGNAL(textChanged(const QString &)), this,
          SLOT(grayDir()));
  connect(leDirectory, SIGNAL(returnPressed()), this, SLOT(populateListBox()));
  connect(leFileNamePattern, SIGNAL(textChanged(const QString &)), this,
          SLOT(populateListBox()));
  connect(lvFiles, SIGNAL(itemDoubleClicked(QTreeWidgetItem *, int)), this,
          SLOT(Selected(QTreeWidgetItem *, int)));
}

fileview::~fileview() {}

void fileview::SetDirectory(string dir) {
  return;
  leDirectory->clear();
...
void fileview::SetPattern(string pat) {
  leFileNamePattern->setText(pat.c_str());
}

void fileview::ShowImageInformation(bool x) {
  if (x)
...
vector<string> fileview::Go() {
  populateListBox();
  this->exec();
  return returnedFiles;
}

void fileview::ShowDirectoriesAlso(bool x) {
  showdirs = x;
  return;
}

void fileview::populateListBox() {
  lvFiles->clear();
  struct stat st;
  char size[STRINGLEN];
  ...
  for (size_t i = 0; i < list.size(); i++) {
    if (stat(list[i].c_str(), &st)) continue;
    if (showdirs == false) {
    ...
    if (lvFiles->columnWidth(COL_INFO) && S_ISREG(st.st_mode)) {
      ...
      if (!(cb.ReadHeader(list[i]))) {
        imageinfo =
            (format(" 3D volume: %dx%dx%d") % cb.dimx % cb.dimy % cb.dimz)
                .str();
        protocol = cb.GetHeader("Protocol:");
      } else if (!(ts.ReadHeader(list[i]))) {
        imageinfo = (format(" 4D volume: %dx%dx%d, %d volumes") % ts.dimx %
                     ts.dimy % ts.dimz % ts.dimt)
                        .str();
        protocol = cb.GetHeader("Protocol:");
      } else if (!mat.ReadHeader(list[i])) {
        imageinfo = (format(" 2D matrix: %dx%d") % mat.m % mat.n).str();
      } else if (!vec.ReadFile(list[i])) {
        imageinfo = (format(" 1D vector: %d elements (mean %d)") % vec.size() %
                     vec.getVectorMean())
                        .str();
      }

      if (protocol.size()) imageinfo += " (" + protocol + ")";
...
void fileview::Selected(QTreeWidgetItem *item, int) {
  ...
  string newdir =
      (string)leDirectory->text().latin1() + "/" + item->text(0).latin1();
...
vector<string> fileview::returnSelectedFiles() {
...
void fileview::HandleHome() {
  leDirectory->setText(getenv("HOME"));
  populateListBox();
}

void fileview::HandleUp() {
  leDirectory->setText(xdirname(leDirectory->text().ascii()).c_str());
  populateListBox();
}

void fileview::HandleRoot() {
  leDirectory->setText("/");
  populateListBox();
}

void fileview::grayDir() {
  leDirectory->setPaletteBackgroundColor(qRgb(220, 160, 160));
}

void fileview::HandleNewWD() {
  QString s = Q3FileDialog::getExistingDirectory(leDirectory->text(), this,
                                                 "xxx", "new dir?");
  leDirectory->setText(s.ascii());
  populateListBox();
}

bool fileview::Okayed() { return okayed; }
