...
class PlotScreen : public QScrollArea {
 ...
 public:
  ...
  void setFixedSize(unsigned outWidth, unsigned outHeight, unsigned inWidth,
                    unsigned inHeight);
  ...
  int addVector(const VB_Vector &, QColor inputColor = "green");
  int addVector(const VB_Vector *, double xMarkMin, double xLength,
                QColor plotColor, unsigned mode);
  int addVector(const VB_Vector &, double xMarkMin, double xLength,
                QColor plotColor, unsigned mode);
  int addVecFile(const char *filename, QColor inputColor = "green");
  int addVecFile(const char *filename, double xMarkMin, double xLength,
                 QColor plotColor, unsigned mode);
  ...
  /* The functions below are specilly written to manipulate the first vector.
   * They are mainly used for Gdw interface, where only one covariate shows up
   * on the screen. */
...
class PlotWidget : public QFrame {
 ...
 private:
  ...
  unsigned windowWidth, windowHeight,
      frameWidth;                  // outside window size and the frame width
  ...
  std::vector<double>
      xMinList;  // vector to hold each curve's minimum on X axis
  std::vector<double>
      xMaxList;  // vector to hold each curve's maximum on X axis
  std::vector<double>
      yMinList;  // vector to hold each curve's minimum on Y axis
  std::vector<double>
      yMaxList;  // vector to hold each curve's maximum on Y axis
  std::vector<QColor> colorList;  // vector to hold each curve's plot colors
  std::vector<unsigned>
      plotModeList;  // vector to hold each vector's plot mode (1, 2, 3 or 4)
  std::vector<double> xStartPost;
  std::vector<double> xLengthInPixel;

  double xInPixel;      // number of pixels between two nearby elements in the
                        // vector on X axis
  double plotXMarkMin;  // minimum X axis mark value (assigned for zoom in/out
                        // functionality)
  double plotXLength;   // length shown on X axis, not the number of points in
                        // input VB_Vector
  double plotYMarkMin;  // minimum mark value on Y axis
  double plotYLength;   // length shown on Y Axis
  int plotMode;         // mode to draw graph: 1 indicates drawGraphMode1(), 2
                        // drawGraphMode2(), etc
