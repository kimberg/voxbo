...
void Extractor::init(tokenlist &args) {
  // defaults
  xvflag = 0;
  newdim[0] = 41;
  newdim[1] = 51;
  ...
  for (int i = 0; i < newcube.dimx; i++) {
    for (int j = 0; j < newcube.dimy; j++) {
      for (int k = 0; k < newcube.dimz; k++) {
        ...
        if (xvflag == 0 || val == xval) newcube.SetValue(i, j, k, val);
      }
    }
  }
  newcube.WriteFile();
...
double Extractor::regioncode(int x, int y, int z) {
  ...
  for (int i = xx0; i < xx1; i++) {
    if (i < 0 || i >= original.dimx) continue;
    for (int j = yy0; j < yy1; j++) {
      if (j < 0 || j >= original.dimy) continue;
      for (int k = zz0; k < zz1; k++) {
        if (k < 0 || k >= original.dimz) continue;
...
void extractmask_help() { cout << boost::format(myhelp) % vbversion; }
