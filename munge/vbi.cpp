...
int main(int argc, char *argv[]) {
  ...
  for (size_t i = 0; i < filelist.size(); i++) {
    if (mytes.ReadHeader(filelist[i]) == 0) {
      if (f_loaddata) mytes.ReadFile(filelist[i]);
      if (f_long)
        mytes.print();
      else {
        printf("[I] ");
      ...
      if (f_loaddata) mycube.ReadFile(filelist[i]);
      if (f_long)
        mycube.print();
      else {
        printf("[I] ");
      ...
      cout << format("[I] %s: %d elements, mean of %g, variance of %g\n") %
                  filelist[i] % myvec.size() % myvec.getVectorMean() %
                  myvec.getVariance();
...
void vbi_help() { cout << boost::format(myhelp) % vbversion; }

void vbi_version() { printf("VoxBo vbi (v%s)\n", vbversion.c_str()); }
