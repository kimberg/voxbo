...
int vbsmooth_smooth(tokenlist &args) {
  ...
  for (int i = 0; i < (int)filelist.size(); i++) {
    if (smooth_file(filelist[i], outfile)) err++;
  }
  return err;
}
...
int smooth_file(string infile, string ofile) {
  ...
  if (filetypes[0].getDimensions() == 3) {
    ...
    // if it's "fancy" remask, quantize to 1.0, smooth, thresh at 0.5, and
    // quantize again
    if (sflag) {
      remask.quantize(1.0);
    ...
    tmps.str("");
    tmps << "SpatialSmooth: " << timedate() << " " << sx << " " << sy << " "
         << sz;
    cube.AddHeader(tmps.str());
    cube.SetFileName(outfile);
    if (remask.data) cube.intersect(remask);
    if (cube.WriteFile()) {
    ...
    for (int i = 0; i < tes.dimt; i++) {
      ...
      if (remask.data) cb.intersect(remask);
      tes.SetCube(i, &cb);
    }
    tmps.str("");
    tmps << "SpatialSmooth: " << timedate() << " " << sx << " " << sy << " "
...
int vbsmooth_estimate(tokenlist &) {
  printf("[E] vbsmooth: estimation not yet implemented\n");
  return 101;
}

void vbsmooth_help() { cout << boost::format(myhelp) % vbversion; }

void vbsmooth_version() { printf("VoxBo vbsmooth (v%s)\n", vbversion.c_str()); }
