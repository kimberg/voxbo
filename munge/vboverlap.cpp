...
void calc_overlap(Cube &atlas, Cube &mask, int maskval, int atlasval) {
  if (atlas.dimx != mask.dimx || atlas.dimy != mask.dimy ||
      atlas.dimz != mask.dimz) {
  ...
  for (MI mm = breakdown.begin(); mm != breakdown.end(); mm++) {
    ...
    if (atlas.maskspecs.count(mm->first))
      ...
      rname = "anon";
    printf(
        "[I]   mask %s, region %d (%s): mask includes %d of %d voxels (%.4f)\n",
        mask.GetFileName().c_str(), mm->first, rname.c_str(),
        mm->second.overlap, mm->second.total,
        (float)mm->second.overlap / (float)mm->second.total);
  }
}

void vboverlap_help() { cout << boost::format(myhelp) % vbversion; }
