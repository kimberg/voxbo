...
int main(int argc, char *argv[]) {
  ...
  if (args.size() == 3) ind = strtol(args[2]);
  tes = new Tes();
  ...
  if (ind > tes->dimt) {
    tmps.str("");
    tmps << "tes2cub: index (" << ind << ") is beyond the last image ("
         << tes->dimt << ").";
    printErrorMsg(VB_ERROR, tmps.str());
    exit(5);
  }
  ...
  if (!cub->data_valid) {
    tmps.str("");
    tmps << "tes2cub: error extracting the cube from the 4D file (shouldn't "
            "happen!).";
    printErrorMsg(VB_ERROR, tmps.str());
    exit(5);
  }
...
void tes2cub_help() { cout << boost::format(myhelp) % vbversion; }
