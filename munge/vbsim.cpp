...
int
VBSim::AddUniform(Cube &cb,double low,double range,double sx,double sy,double sz)
{
  Cube cb2=cb;
  cb2.zero();
  double val;
  ...
  if (sx != 0.0 || sy != 0.0 || sz != 0.0)
    smoothCube(cb2,sx,sy,sz,1);;
  cb+=cb2;
  return 0;
}

int
VBSim::AddGaussian(Cube &cb,double mu,double variance,double sx,double sy,double sz)
{
...
double
VBSim::gaussian_random(double sigma)
{
